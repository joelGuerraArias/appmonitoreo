...
try:
    from clip_intro import INTRO_CLIP_ENABLED, prefijar_intro_coincidencia_si
except ImportError:
    INTRO_CLIP_ENABLED = False

    def prefijar_intro_coincidencia_si(clip_path, termino, nombre_archivo, **kwargs):
        return clip_path, False, "clip_intro no disponible"
...
def _descubrir_bin_ffmpeg_winget():
    """
    Si FFmpeg (Gyan) está instalado vía winget, devuelve la carpeta .../bin con ffmpeg.exe/ffprobe.exe.
    Así funciona sin FFMPEG_BIN en .env en muchas instalaciones Windows.
    """
    try:
        lad = os.environ.get("LOCALAPPDATA", "")
        if not lad:
            return ""
        pkg = Path(lad) / "Microsoft" / "WinGet" / "Packages"
        if not pkg.is_dir():
            return ""
        for pat in ("Gyan.FFmpeg.Essentials*", "Gyan.FFmpeg*"):
            try:
                for root_folder in sorted(pkg.glob(pat), key=lambda p: str(p)):
                    ...
                    for sub in root_folder.iterdir():
                        if not sub.is_dir():
                            continue
                        b_dir = sub / "bin"
                        if (b_dir / "ffmpeg.exe").is_file() and (b_dir / "ffprobe.exe").is_file():
...
# FFmpeg/ffprobe: FFMPEG_BIN (.env), o auto winget (Gyan); anteponer al PATH para subprocess.
_ffmpeg_bin_prepend = os.getenv("FFMPEG_BIN", "").strip()
if _ffmpeg_bin_prepend:
    _ffmpeg_bin_norm = os.path.normpath(_ffmpeg_bin_prepend.replace("/", os.sep))
    _ffmpeg_bin_prepend = _ffmpeg_bin_norm if os.path.isdir(_ffmpeg_bin_norm) else ""
if not _ffmpeg_bin_prepend:
    _wg_bin = _descubrir_bin_ffmpeg_winget()
    if _wg_bin:
        _ffmpeg_bin_prepend = _wg_bin
        logger.info(f"FFmpeg detectado (WinGet): PATH + {_ffmpeg_bin_prepend}")
if _ffmpeg_bin_prepend and os.path.isdir(_ffmpeg_bin_prepend):
    _sep = os.pathsep
    if _ffmpeg_bin_prepend not in os.environ.get("PATH", ""):
        os.environ["PATH"] = _ffmpeg_bin_prepend + _sep + os.environ.get("PATH", "")
    os.environ.setdefault("FFMPEG_BIN", _ffmpeg_bin_prepend)
...
# DeepSeek — enriquecimiento opcional de motivos tangenciales (API compatible OpenAI)
DEEPSEEK_API_KEY = (
...
if DEEPSEEK_API_KEY:
    try:
        ...
        logging.info("[OK] Cliente DeepSeek inicializado (tangenciales en appMonitoreo)")
    except Exception as _e_ds_init:
...
# Drive: carpeta tangenciales por defecto; coincidencias Intrant ([videos intrant](https://drive.google.com/drive/folders/1GrbA0keR8AD4sG1O3e4xAOAYpYJj1BnH)) y Edesur ([edesurVideos](https://drive.google.com/drive/folders/10wJNNTmE9aO1cI98N7oKGmE-vJAUFrjg))
GOOGLE_DRIVE_FOLDER_TANGENCIALES_DEFAULT = os.getenv(
    'GOOGLE_DRIVE_FOLDER_TANGENCIALES_ID',
    '1Jzsrkx2YRYfyqgxKQ1oW1PHmYuZCKKXf',
).strip()
GOOGLE_DRIVE_INTRANT_COINCIDENCIAS_FOLDER_ID = os.getenv(
    'GOOGLE_DRIVE_INTRANT_COINCIDENCIAS_FOLDER_ID',
    '1GrbA0keR8AD4sG1O3e4xAOAYpYJj1BnH',
).strip()
GOOGLE_DRIVE_EDESUR_COINCIDENCIAS_FOLDER_ID = os.getenv(
    'GOOGLE_DRIVE_EDESUR_COINCIDENCIAS_FOLDER_ID',
    '10wJNNTmE9aO1cI98N7oKGmE-vJAUFrjg',
).strip()
# Drive + Sheets: mismo refresh token; al ampliar scopes hay que reautorizar en Google Cloud y regenerar token
GOOGLE_OAUTH_SCOPES = [
...
# === CONFIGURACIÓN ===
# Salida por defecto: carpeta "videos procesados" junto a este script (ruta absoluta).
# Ej.: C:\Users\...\grabaciones\videos procesados — no depende del directorio de trabajo actual.
_DIR_SCRIPT = Path(__file__).parent.resolve()
_CARPETA_BASE = _DIR_SCRIPT / "videos procesados"
_CARPETA_PROCESADOS_DEFAULT = str(_CARPETA_BASE.resolve())
CARPETA_VIDEOS = os.getenv('CARPETA_VIDEOS', _CARPETA_PROCESADOS_DEFAULT)
CARPETA_PROCESADOS = os.getenv('CARPETA_PROCESADOS', _CARPETA_PROCESADOS_DEFAULT)
...
# Siempre junto a este script (no dependen del cwd al lanzar Streamlit)
TERMINOS_CONFIG = str(_DIR_SCRIPT / "terminos_guardados.json")
...
try:
    DURACION_MINIMA_SEGUNDOS_PROCESAR = max(0, int(os.getenv("VIDEO_DURACION_MINIMA_SEGUNDOS", "0")))
except ValueError:
    DURACION_MINIMA_SEGUNDOS_PROCESAR = 0
# Subcarpetas dentro de CARPETA_PROCESADOS (todo lo generado queda bajo "videos procesados")
CARPETA_VIDEOSCHECK = os.path.join(CARPETA_PROCESADOS, "videoscheck")
CARPETA_ARCHIVOS_FALLIDOS = os.path.join(CARPETA_PROCESADOS, "archivos_fallidos")
FALLIDOS_TXT = os.path.join(CARPETA_PROCESADOS, "fallidos.txt")
VIDEOSCHECK_INDEX_CSV = os.path.join(CARPETA_VIDEOSCHECK, "index.csv")
ENVIOS_COINCIDENCIAS_DEDUPE_JSON = os.path.join(CARPETA_PROCESADOS, "envios_coincidencias_dedupe.json")
# Informe acumulativo en Escritorio/informes (o INFORMES_GENERAL_DIR / INFORME_GENERAL_MD)
_INFORMES_GENERAL_DIR = os.getenv(
    "INFORMES_GENERAL_DIR",
    os.path.join(os.path.expanduser("~"), "Desktop", "informes"),
)
INFORME_GENERAL_PATH = os.getenv(
    "INFORME_GENERAL_MD",
    os.path.join(_INFORMES_GENERAL_DIR, "informe_general.md"),
)
# Crear carpetas necesarias
os.makedirs(CARPETA_PROCESADOS, exist_ok=True)
os.makedirs(os.path.dirname(INFORME_GENERAL_PATH), exist_ok=True)
DEDUPE_ENVIO_TTL_DIAS = 14
DEDUPE_ENVIO_MAX_ENTRADAS = 8000
TELEGRAM_CAPTION_MAX = 1024
TELEGRAM_SENDVIDEO_DIRECT_MAX_MB = 50

def _dedupe_normalizar_texto(texto):
    if not texto:
        return ""
    t = re.sub(r"\s+", " ", str(texto).strip().lower())
    return t[:6000]

def calcular_clave_dedupe_envio(cliente, termino_encontrado, timestamp_seg, contexto_termino, idea_general=None, transcripcion_segmento=""):
    """Huella estable sin depender del nombre de archivo (reprocesos/renombres)."""
    cid = str(cliente.get("id", cliente.get("nombre", "default")))
    term = (termino_encontrado or "").strip().lower()
    try:
        ts = round(float(timestamp_seg), 1)
    except (TypeError, ValueError):
        ts = 0.0
    ctx = _dedupe_normalizar_texto(contexto_termino)
    if not ctx:
        ctx = _dedupe_normalizar_texto(idea_general or "")
    seg = _dedupe_normalizar_texto(transcripcion_segmento)[:1200]
    payload = f"{cid}|{term}|{ts}|{ctx}|{seg}"
    return hashlib.sha256(payload.encode("utf-8")).hexdigest()

def _dedupe_cargar_store():
    try:
        if os.path.exists(ENVIOS_COINCIDENCIAS_DEDUPE_JSON):
            with open(ENVIOS_COINCIDENCIAS_DEDUPE_JSON, "r", encoding="utf-8") as f:
                data = json.load(f)
            if isinstance(data, dict) and "envios" in data:
                return data
    except Exception:
        pass
    return {"version": 1, "envios": {}}

def _dedupe_podar_envios(envios):
    """Quita entradas más viejas que TTL y recorta a MAX entradas."""
    if not isinstance(envios, dict):
        return {}
    cutoff = datetime.now() - timedelta(days=DEDUPE_ENVIO_TTL_DIAS)
    nuevos = {}
    for k, meta in envios.items():
        if not isinstance(meta, dict):
            continue
        try:
            ts = datetime.fromisoformat(meta.get("cuando", "2000-01-01T00:00:00"))
        except ValueError:
            continue
        if ts >= cutoff:
            nuevos[k] = meta
    while len(nuevos) > DEDUPE_ENVIO_MAX_ENTRADAS:
        oldest_key = min(nuevos.keys(), key=lambda x: nuevos[x].get("cuando", ""))
        del nuevos[oldest_key]
    return nuevos

def _dedupe_guardar_store(data):
    try:
        os.makedirs(CARPETA_PROCESADOS, exist_ok=True)
        with open(ENVIOS_COINCIDENCIAS_DEDUPE_JSON, "w", encoding="utf-8") as f:
            json.dump(data, f, ensure_ascii=False, indent=0)
    except Exception as e:
        log_warning(f"No se pudo guardar dedupe envíos: {e}", "_dedupe_guardar_store")

def dedupe_envio_ya_registrado(clave_hex):
    if not clave_hex:
        return False
    data = _dedupe_cargar_store()
    raw = data.get("envios") or {}
    envios = _dedupe_podar_envios(raw)
    if len(envios) != len(raw):
        data["envios"] = envios
        _dedupe_guardar_store(data)
    return clave_hex in envios

def dedupe_registrar_envio_exitoso(clave_hex, termino, cliente_nombre):
    if not clave_hex:
        return
    data = _dedupe_cargar_store()
    envios = _dedupe_podar_envios(data.get("envios") or {})
    envios[clave_hex] = {
...
def guardar_video_y_minuto_coincidencia(video_path, minuto_detectado, termino_detectado="", mover_video=False, prefijo_carpeta=""):
    """
    Guarda/copia (o mueve) el video en videos procesados/videoscheck
    y registra el minuto detectado en un TXT del mismo nombre base.
    """
    func_name = "guardar_video_y_minuto_coincidencia"
    try:
        if not video_path or not os.path.exists(video_path):
            return None, None, "Ruta de video inválida o no existe"

        ruta_origen_abs = os.path.abspath(video_path)
        nombre_video = os.path.basename(video_path)
        base_sin_ext, _ = os.path.splitext(nombre_video)

        # Guardar en subcarpeta por video para evitar colisiones y facilitar revisión manual.
        nombre_carpeta_video = f"{prefijo_carpeta}{base_sin_ext}" if prefijo_carpeta else base_sin_ext
        carpeta_destino = os.path.join(CARPETA_VIDEOSCHECK, nombre_carpeta_video)
        os.makedirs(carpeta_destino, exist_ok=True)

        destino_video = os.path.join(carpeta_destino, nombre_video)
        estado_copia = "ya_existia"

        if not os.path.exists(destino_video):
            if mover_video:
                shutil.move(video_path, destino_video)
                accion = "movido"
            else:
                # Por defecto se copia para conservar el video fuente original.
                shutil.copy2(video_path, destino_video)
                accion = "copiado"
            estado_copia = accion
            log_info(f"Video {accion} a videos procesados/videoscheck: {destino_video}", func_name)
        else:
            log_info(f"Video ya existía en videoscheck: {destino_video}", func_name)

        # Verificación real del archivo copiado/movido.
        if not os.path.exists(destino_video):
            return None, None, "error_copia"
        try:
            size_destino = os.path.getsize(destino_video)
            size_origen = os.path.getsize(ruta_origen_abs) if os.path.exists(ruta_origen_abs) else None
            if size_destino <= 0:
                return None, None, "error_copia_destino_vacio"
            # Si fue copia (no move), validar tamaño igual para confirmar integridad básica.
            if not mover_video and size_origen is not None and size_destino != size_origen:
                return None, None, f"error_copia_tamano_mismatch({size_origen}!={size_destino})"
        except Exception as e_size:
            return None, None, f"error_verificacion_copia({e_size})"

        txt_minutos = os.path.join(carpeta_destino, f"{base_sin_ext}.txt")
        marca_tiempo = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

        minuto_entero = int(minuto_detectado // 60) if minuto_detectado is not None else 0
        segundo_entero = int(minuto_detectado % 60) if minuto_detectado is not None else 0

        linea = (
            f"{marca_tiempo} | minuto {minuto_entero}:{segundo_entero:02d} "
            f"({float(minuto_detectado):.1f}s) | termino: {termino_detectado}\n"
        )

        with open(txt_minutos, "a", encoding="utf-8") as ftxt:
            ftxt.write(linea)

        # Registrar índice global para trazabilidad (origen -> destino).
        os.makedirs(CARPETA_VIDEOSCHECK, exist_ok=True)
        existe_index = os.path.exists(VIDEOSCHECK_INDEX_CSV)
        tipo_registro = "tangencial" if prefijo_carpeta.startswith("_tangencial_") else ("coincidencia" if prefijo_carpeta.startswith("_coincidencia_") else "otro")
        with open(VIDEOSCHECK_INDEX_CSV, "a", newline="", encoding="utf-8") as fcsv:
            ...
            writer.writerow([
                ...
                tipo_registro,
                termino_detectado,
                f"{minuto_entero}:{segundo_entero:02d}",
                f"{float(minuto_detectado):.1f}" if minuto_detectado is not None else "0.0",
                estado_copia,
                ruta_origen_abs,
                os.path.abspath(destino_video),
                os.path.abspath(txt_minutos),
            ])

        return destino_video, txt_minutos, estado_copia
    except Exception as e:
        log_warning(f"No se pudo guardar en videos procesados/videoscheck: {e}", func_name)
...
def migrar_config_actual_a_cliente():
    ...
    bunny_config = cargar_bunny_config()
    ...
    return {
        ...
        'brevo': {
            ...
            'sender_name': brevo_config.get('sender_name', 'Sistema de Análisis de Videos'),
        ...
        'bunny': {
            'enabled': bool(bunny_config.get('enabled')) and bool(bunny_config.get('storage_zone')) and bool(bunny_config.get('access_key')),
            'storage_zone': bunny_config.get('storage_zone', ''),
            'access_key': bunny_config.get('access_key', ''),
            'folder': bunny_config.get('folder', 'video_analyzer_clips'),
            'cdn_base_url': bunny_config.get('cdn_base_url', '')
        },
        'r2': {
            'enabled': True,
            'folder': cloudinary_config.get('folder', 'video_analyzer_clips'),
            ...
            with open(TERMINOS_CONFIG, 'r', encoding='utf-8') as f:
                ...
                for t in terminos:
                    if isinstance(t, dict):
                        raw = (t.get('termino') or '').strip().lower()
                        if raw == term_norm:
...
def filtrar_terminos_por_analisis_clientes(terminos_list):
    ...
    if not terminos_list:
        return []
    out = []
    for t in terminos_list:
        cid = 'default'
        if isinstance(t, dict):
            cid = t.get('cliente_id', 'default')
        if cliente_incluye_en_analisis(cid):
            out.append(t)
    return out

# === FUNCIONES DE ENVÍO POR CLIENTE ===
def enviar_webhook_cliente(cliente, video_path, resumen, terminos, data_extra=None, video_url=None, video_url_bunny=None, video_url_r2=None):
    ...
    if video_url_bunny:
        data['video_url_bunny'] = video_url_bunny
    if video_url_r2:
        data['video_url_r2'] = video_url_r2
...
def enviar_telegram_cliente(cliente, mensaje, video_path=None, parse_mode='Markdown', video_url=None, video_url_bunny=None, video_url_r2=None):
    """Envía mensaje y/o video a Telegram usando credenciales del cliente"""
    ...
    resultados = []
    full_caption_text = escape_telegram_text(mensaje)
    caption_limpio = full_caption_text[:TELEGRAM_CAPTION_MAX]
    caption_truncado = len(full_caption_text) > TELEGRAM_CAPTION_MAX
    if caption_truncado:
        log_info(f"Caption de video limitado a {TELEGRAM_CAPTION_MAX} caracteres (API Telegram sendVideo)", func_name)
    ...
    if caption_truncado:
        video_ok = any(n == 'Video' and ex for n, ex, _ in resultados)
        if video_ok:
            nota = (
                f"Nota: Telegram limita el caption del video a {TELEGRAM_CAPTION_MAX} caracteres. "
                "El mensaje de texto anterior incluye el análisis completo."
            )
            ex_n, ms_n = enviar_mensaje_telegram(nota, chat_id, bot_token, None)
            resultados.append(('Caption', ex_n, ms_n))
...
# --- Tangenciales: ítems homogéneos + DeepSeek (todo en este archivo; no depender de radioAnalizer.py) ---

def motivo_display_tangencial(item) -> str:
    """Texto de motivo seguro para UI, MD y correo (nunca vacío)."""
...
def motivo_tangencial_una_frase(item) -> str:
    """
    Versión corta para correo: tras DeepSeek, `motivo` ya suele ser una sola frase;
    antes del enriquecimiento, primera oración del motivo técnico o recorte hasta límite.
    """
...
def tangencial_tooltip_referencia_completa(item) -> str:
    """Para tooltips HTML: motivo técnico guardado en `motivo_sistema`; si no, el motivo disponible."""
    if not isinstance(item, dict):
        return ""
    ref = (item.get("motivo_sistema") or "").strip()
    if ref:
        return ref
    return motivo_display_tangencial(item)


def tangencial_formato_momento_mencion_termino(item) -> tuple:
    """
    Instante dentro del archivo de video donde cayó la detección del término.
    Devuelve (texto_compacto XmYs o XmY,Zs, segundos_exactos_float).
    """
    if not isinstance(item, dict):
        return ("—", 0.0)
    try:
    ...
    mi = int(t // 60)
    fr = max(0.0, t - mi * 60)
    cer = abs(fr - round(fr)) < 0.06
    s_part = f"{round(fr):02d}s" if cer else f"{fr:.1f}s"
    return (f"{mi}m{s_part}", t)


def crear_item_tangencial(
    rel,
    termino,
    motivo,
    momento_termino,
    texto_evidencia="",
    url_drive_video="",
    transcripcion_extracto=None,
):
    """
    Registro homogéneo para menciones tangenciales: UI, Analisishoy, Brevo y DeepSeek.
    - texto_evidencia: contexto largo para el modelo (hasta 12k en ítem).
    - transcripcion_extracto: resumen corto para tabla de correo (si None, se deriva de texto_evidencia).
    """
    hora_dt = datetime.now()
    te = (texto_evidencia or "").strip()
    if transcripcion_extracto is None:
        tx = te[:2500] if te else ""
    else:
        tx = (transcripcion_extracto or te)[:2500]
    ud = (url_drive_video or "").strip()
    mot = (motivo or "").strip() or "Mención tangencial sin desarrollo"
    return {
        "archivo": rel,
        "termino": termino,
        "motivo": mot,
        ...
        "texto_evidencia": te[:12000],
        "transcripcion_extracto": tx,
        "url_drive_video": ud,
        "video_url_drive": ud,
    }
...
def extraer_texto_transcripcion_ventana(segments_timestamps, centro_seg, ventana_seg=120, duracion_audio=None):
    """Concatena textos de segmentos que solapan [centro - ventana, centro + ventana]."""
...
def deepseek_tangenciales_activo() -> bool:
    """True si hay cliente DeepSeek y no está desactivado por variables de entorno."""
    if not deepseek_client:
        return False
    for name in ("VIDEO_DEEPSEEK_TANGENCIALES", "DEEPSEEK_TANGENCIALES_ENABLED", "RADIO_DEEPSEEK_TANGENCIALES"):
...
def enriquecer_motivos_tangenciales_deepseek(items_list, func_name="enriquecer_motivos_tangenciales_deepseek"):
    ...
    y del motivo técnico previo del análisis.

    Conserva ese motivo técnico en `motivo_sistema` cuando tiene éxito. Los ítems que ya
    tienen `motivo_sistema` no se reenvían (p. ej. ya enriquecidos antes del correo inmediato).
    """
    if not items_list:
        return
    ...
    if not deepseek_tangenciales_activo():
        if items_list and not deepseek_client:
            ...
            log_info("DeepSeek tangenciales desactivado (VIDEO_/DEEPSEEK_/RADIO_*_TANGENCIALES)", func_name)
        return
    ...
    for loc_idx, orig_idx in enumerate(pend_orig_idx):
        ...
        if isinstance(it, dict):
            ...
            ms_tecnico = ((it.get("motivo_sistema") or "").strip() or motivo_display_tangencial(it))
...
def crear_plantilla_email_tangenciales_html(cliente, items_list, hora_ciclo, aviso_inmediato=False):
    """
    Plantilla HTML para correo de menciones tangenciales (estilo alineado a crear_plantilla_email_html).
    Si aviso_inmediato=True, el encabezado indica aviso al momento (el resumen completo del ciclo se envía igual al final).
    """
    primary = cliente.get('color') or '#667eea'
    secondary = '#764ba2'
    nombre_cliente = html_module.escape(cliente.get('nombre', 'Cliente'))
    hora_safe = html_module.escape(hora_ciclo)
    etiqueta_tiempo = "Aviso inmediato" if aviso_inmediato else "Cierre de ciclo"
    nota_fin_ciclo_html = (
        ' Al cierre del ciclo de escaneo recibirás un correo adicional con el <strong>resumen de todas</strong> las tangenciales acumuladas.'
        if aviso_inmediato else ""
    )

    filas = []
    for i, tang in enumerate(items_list, 1):
        tiempo_compacto, t_sec_exacto = tangencial_formato_momento_mencion_termino(tang)
        if t_sec_exacto and t_sec_exacto > 0:
            momento_title = html_module.escape(f"Momento dentro del archivo donde se menciona el término: {tiempo_compacto} ({t_sec_exacto:g} s desde el inicio).")
        else:
            momento_title = html_module.escape("Momento dentro del archivo: no disponible.")
        arch_raw = str(tang.get('archivo', '') or '')
        leg, ref_t = archivo_broadcast_legible_y_referencia(arch_raw)
        arch_main = html_module.escape(leg)
        if ref_t:
            arch_cell = (
                f'<div style="font-weight:600;color:#343a40;line-height:1.35;">{arch_main}</div>'
                f'<div style="font-size:11px;color:#868e96;word-break:break-all;margin-top:4px;">'
                f'{html_module.escape(ref_t)}</div>'
            )
        else:
            arch_cell = f'<span style="font-size:13px;word-break:break-word;color:#495057;">{arch_main}</span>'
        term = html_module.escape(str(tang.get('termino', '')))
        ref_tooltip = tangencial_tooltip_referencia_completa(tang)
        motivo_title = html_module.escape((ref_tooltip or "")[:2800])
        frase_una = motivo_tangencial_una_frase(tang)
        frase_cell = html_module.escape(frase_una)
        ext_raw = str(tang.get('transcripcion_extracto', '') or '')
        ext_show = ext_raw if len(ext_raw) <= 600 else (ext_raw[:600] + "…")
        ext_cell = html_module.escape(ext_show) if ext_show else "—"
        url_d = (tang.get('url_drive_video') or tang.get('video_url_drive') or '').strip()
        if url_d:
            link_cell = f'<a href="{html_module.escape(url_d)}" style="color:#1a73e8;font-weight:600;">Abrir en Drive</a>'
        else:
            link_cell = '<span style="color:#adb5bd;">—</span>'
        bg = '#f8f9fa' if i % 2 == 1 else '#ffffff'
        filas.append(f"""
            <tr style="background:{bg};">
                <td style="padding:14px 12px;border-bottom:1px solid #e9ecef;font-weight:600;color:#495057;">{i}</td>
                <td style="padding:14px 12px;border-bottom:1px solid #e9ecef;font-size:13px;word-break:break-word;">{arch_cell}</td>
                ...
                <td style="padding:14px 12px;border-bottom:1px solid #e9ecef;font-size:15px;line-height:1.45;color:#1a237e;font-weight:600;" title="{motivo_title}">{frase_cell}</td>
                <td style="padding:14px 12px;border-bottom:1px solid #e9ecef;font-size:12px;color:#6c757d;word-break:break-word;">{ext_cell}</td>
                <td style="padding:14px 12px;border-bottom:1px solid #e9ecef;white-space:nowrap;font-size:13px;">{link_cell}</td>
                <td style="padding:14px 12px;border-bottom:1px solid #e9ecef;white-space:nowrap;font-family:Consolas,monospace;" title="{momento_title}">{html_module.escape(tiempo_compacto)}</td>
            </tr>""")
    ...
    if len(items_list) == 1:
        t0 = items_list[0]
        tf = html_module.escape(motivo_tangencial_una_frase(t0))
        momento_uno_compacto, t0_sec = tangencial_formato_momento_mencion_termino(t0)
        if t0_sec and t0_sec > 0:
            momento_big = html_module.escape(f"{momento_uno_compacto} ({t0_sec:g} s desde el inicio)")
            momento_txt_muted = html_module.escape(
                "Punto dentro del archivo de video donde se detectó la mención del término; úsalo para localizarla en reproducción o en el clip en Drive."
            )
        else:
            momento_big = html_module.escape("—")
            momento_txt_muted = html_module.escape("Marca temporal no disponible para esta mención.")
        callout_una_frase = f"""
                <div style="margin:0 0 22px 0;padding:18px 22px;background:linear-gradient(135deg,{primary}14 0%,{secondary}12 100%);border-left:5px solid {primary};border-radius:12px;box-sizing:border-box;">
                    <p style="margin:0 0 8px 0;font-size:12px;text-transform:uppercase;letter-spacing:0.07em;color:#495057;font-weight:700;">Momento de mención del término</p>
                    <p style="margin:0 0 16px 0;font-size:22px;line-height:1.35;color:#0d1740;font-weight:700;font-family:Consolas,monospace;letter-spacing:0.03em;">{momento_big}</p>
                    <p style="margin:0;font-size:12px;color:#6c757d;line-height:1.45;">{momento_txt_muted}</p>
                    <hr style="border:none;border-top:1px solid #dee2e6;margin:14px 0 12px 0;" />
                    <p style="margin:0 0 8px 0;font-size:12px;text-transform:uppercase;letter-spacing:0.07em;color:#495057;font-weight:700;">¿Por qué es tangencial?</p>
                    <p style="margin:0;font-size:18px;line-height:1.5;color:#0d1740;font-weight:600;">{tf}</p>
                    <p style="margin:10px 0 0 0;font-size:12px;color:#6c757d;">La frase de una línea sintetiza la transcripción y el motivo técnico del análisis; es la misma que ves en la columna de la tabla.</p>
                </div>"""
    ...
    <div style="max-width:820px;margin:0 auto;padding:24px;">
        <div style="background:#fff;border-radius:16px;box-shadow:0 8px 32px rgba(0,0,0,0.08);overflow:hidden;">
            <div style="background:linear-gradient(135deg, {primary} 0%, {secondary} 100%);color:#fff;padding:28px 32px;text-align:center;">
                <p style="margin:0 0 8px 0;font-size:13px;letter-spacing:0.08em;opacity:0.95;text-transform:uppercase;">Monitoreo de medios</p>
                ...
                <p style="margin:8px 0 0 0;font-size:14px;opacity:0.88;">{html_module.escape(etiqueta_tiempo)}: {hora_safe}</p>
            </div>
            <div style="padding:28px 24px 8px 24px;">
                <p style="margin:0 0 16px 0;color:#6c757d;font-size:14px;line-height:1.6;">
                    A continuación, los segmentos <strong>no enviados como alerta con clip</strong>: descartes por relevancia baja o mención tangencial sin desarrollo suficiente.{nota_fin_ciclo_html}
                </p>
                <p style="margin:0 0 18px 0;color:#495057;font-size:13px;line-height:1.55;"><strong>Tabla:</strong> la última columna es el <strong>momento dentro del archivo</strong> en que se ubicó la mención del término (pasar el cursor para ver segundo exacto desde el inicio). «Por qué es tangencial (1 frase)» resume en una línea el descarte (transcripción y motivo técnico del análisis cuando aplica); el tooltip de esa celda muestra el detalle de referencia si está guardado.</p>
                {callout_una_frase}
                    ...
                    <table style="width:100%;border-collapse:collapse;font-size:14px;">
                        <thead>
                            <tr style="background:linear-gradient(180deg,#f8f9fa 0%,#eef1f5 100%);">
                                <th style="padding:12px;text-align:left;border-bottom:2px solid {primary};color:#343a40;width:40px;">#</th>
                                <th style="padding:12px;text-align:left;border-bottom:2px solid {primary};color:#343a40;">Emisión / archivo</th>
                                <th style="padding:12px;text-align:left;border-bottom:2px solid {primary};color:#343a40;">Término</th>
                                <th style="padding:12px;text-align:left;border-bottom:2px solid {primary};color:#343a40;">Por qué es tangencial (1 frase)</th>
                                <th style="padding:12px;text-align:left;border-bottom:2px solid {primary};color:#343a40;">Extracto</th>
                                <th style="padding:12px;text-align:left;border-bottom:2px solid {primary};color:#343a40;">Drive</th>
                                <th style="padding:12px;text-align:left;border-bottom:2px solid {primary};color:#343a40;">Momento mención<br/><span style="font-size:11px;font-weight:500;color:#6c757d;">del término en el archivo</span></th>
                            </tr>
                        </thead>
                        <tbody>
                            {tbody}
            ...
            <div style="background:#343a40;color:#ced4da;padding:20px 28px;text-align:center;font-size:12px;line-height:1.6;">
                <p style="margin:0;">Correo automático del sistema de análisis de videos. No responda a este mensaje.</p>
...
def enviar_brevo_tangencial_inmediato(cliente, item_tangencial):
    """
    Envía un correo Brevo en el acto con una sola tangencial y su motivo.
    Al final del ciclo sigue enviándose el resumen completo vía enviar_correos_tangenciales_fin_ciclo.
    """
    ...
    tiempo_mencion_compacto, t_vid = tangencial_formato_momento_mencion_termino(item_tangencial)
    linea_momento = (
        f"Momento de mención del término en el archivo: {tiempo_mencion_compacto} ({t_vid:g} s desde el inicio)"
        if (t_vid and t_vid > 0)
        else "Momento de mención del término en el archivo: no disponible"
    )
    udv = (item_tangencial.get('url_drive_video') or item_tangencial.get('video_url_drive') or '').strip()
    ext = (item_tangencial.get('transcripcion_extracto') or '')[:600]
    frase_corr = motivo_tangencial_una_frase(item_tangencial)
    ref_analisis = ((item_tangencial.get("motivo_sistema") or "").strip() or motivo_display_tangencial(item_tangencial))
    lineas_txt = [
        ...
        ref_analisis,
        "",
        archivo_broadcast_lineas_correo_plano(str(item_tangencial.get('archivo', '') or '')),
        f"Término: {term_label}",
    ]
    if udv:
        lineas_txt.append(f"Drive: {udv}")
    if ext:
        lineas_txt.append(f"Extracto: {ext}")
    lineas_txt.extend([
        "",
        "Al cierre del ciclo de escaneo recibirás un correo adicional con el resumen de todas las tangenciales del lote.",
        "",
        f"— {cliente_nombre} | Sistema de análisis de videos",
    ])
    text_content = "\n".join(lineas_txt)
    asunto = f"Tangencial (inmediata) — {cliente_nombre} — «{term_label}» — {hora}"
...
def notificar_brevo_tangencial_inmediato_si(cliente, item_tangencial, caller_func_name):
    """Intenta enviar Brevo inmediato; no interrumpe el bucle de análisis si falla."""
    if not item_tangencial:
        return
    try:
...
def enviar_brevo_menciones_tangenciales_cliente(cliente, items_list, hora_ciclo=None):
    ...
    lineas_txt = [
        ...
        "",
        "Listado (no incluye clips; son segmentos descartados por relevancia baja o mención tangencial sin desarrollo):",
        "",
    ]
    for i, tang in enumerate(items_list, 1):
        tiempo_menc_compacto, t_vid = tangencial_formato_momento_mencion_termino(tang)
        momento_str = (
            f"{tiempo_menc_compacto} ({t_vid:g} s desde el inicio del archivo)"
            if (t_vid and t_vid > 0)
            else "—"
        )
        udv = (tang.get('url_drive_video') or tang.get('video_url_drive') or '').strip()
        ext = (tang.get('transcripcion_extracto') or '')[:400]
        ref_tecnico = ((tang.get("motivo_sistema") or "").strip())
        motivo_para_ref = ref_tecnico if ref_tecnico else motivo_display_tangencial(tang)
        _ar = str(tang.get('archivo', '') or '')
        linea = (
            f"{i}. "
            + archivo_broadcast_lineas_correo_plano(_ar).replace("\n", " — ")
            + f" | término: {tang.get('termino', '')}"
        )
        linea += f"\n   Momento de mención del término en el archivo: {momento_str}"
        linea += f"\n   ¿Por qué tangencial? {motivo_tangencial_una_frase(tang)}"
        linea += f"\n   Motivo de referencia (análisis técnico): {motivo_para_ref}"
        if udv:
            linea += f"\n   Drive: {udv}"
        if ext:
            linea += f"\n   Extracto: {ext}"
        lineas_txt.append(linea)
    lineas_txt.extend(["", f"— {cliente_nombre} | Sistema de análisis de videos"])
    text_content = "\n".join(lineas_txt)

    asunto = f"Menciones tangenciales (monitoreo) — {cliente_nombre} — {hora_ciclo}"
    ...
    try:
        ...
        log_info(f"Enviando correo tangenciales Brevo: {cliente_nombre} → {len(correos_destinatarios)} dest.", func_name)
        with smtplib.SMTP(smtp_server, smtp_port) as server:
            server.starttls()
            server.login(smtp_user, api_key)
            server.send_message(msg)
...
def enviar_correos_tangenciales_fin_ciclo(menciones_tangenciales_data):
    ...
    Incluye todas las tangenciales del ciclo (también las ya notificadas por correo inmediato).
...
def _es_cliente_intrant_para_drive_coincidencias(cliente) -> bool:
    """Cliente Intrant: coincidencias (clip + txt) van a carpeta dedicada en Drive."""
    if not cliente:
        return False
...
def _es_cliente_edesur_para_drive_coincidencias(cliente) -> bool:
    """Cliente Edesur / Sistema principal (`default`): coincidencias a carpeta edesurVideos en Drive."""
    if not cliente:
        return False
    cid = str(cliente.get('id') or '').strip().lower()
    if cid in ('intrant', 'minerd'):
        return False
    if cid in ('default', 'edesur'):
        return True
    nom = str(cliente.get('nombre') or '').strip().upper()
    return 'EDESUR' in nom or 'SISTEMA PRINCIPAL' in nom
...
def enviar_gdrive_cliente(cliente, archivo_path, nombre_archivo=None):
    ...
    if _es_cliente_intrant_para_drive_coincidencias(cliente) and GOOGLE_DRIVE_INTRANT_COINCIDENCIAS_FOLDER_ID:
        folder_id = GOOGLE_DRIVE_INTRANT_COINCIDENCIAS_FOLDER_ID
        log_info(
            "☁️ Drive coincidencias Intrant: usando carpeta dedicada "
            f"({GOOGLE_DRIVE_INTRANT_COINCIDENCIAS_FOLDER_ID[:18]}…)",
            func_name,
        )
    elif _es_cliente_edesur_para_drive_coincidencias(cliente) and GOOGLE_DRIVE_EDESUR_COINCIDENCIAS_FOLDER_ID:
        folder_id = GOOGLE_DRIVE_EDESUR_COINCIDENCIAS_FOLDER_ID
        log_info(
            "☁️ Drive coincidencias Edesur: usando carpeta dedicada "
            f"({GOOGLE_DRIVE_EDESUR_COINCIDENCIAS_FOLDER_ID[:18]}…)",
            func_name,
        )
    ...
    try:
        ...
        if resultado:
            return True, mensaje, (resultado.get('webViewLink') or resultado.get('id'))
...
def obtener_google_drive_folder_id_tangenciales(cliente):
    """Carpeta raíz Drive para tangenciales: JSON `folder_id_tangenciales` o valor por defecto (compartida tangenciales)."""
    gdrive_config = cliente.get('google_drive', {})
    if not gdrive_config.get('enabled', False):
        return ''
    explicit = (gdrive_config.get('folder_id_tangenciales') or '').strip()
    if explicit:
        return explicit
    if GOOGLE_DRIVE_FOLDER_TANGENCIALES_DEFAULT:
        return GOOGLE_DRIVE_FOLDER_TANGENCIALES_DEFAULT
    return ''


def _slug_tangencial_cliente(cliente):
    """Nombre de subcarpeta en Drive para tangenciales (`default` → edesur)."""
    cid = (cliente or {}).get('id', 'default') or 'default'
    if cid == 'default':
        return 'edesur'
    return str(cid).strip().lower() or 'edesur'


def drive_obtener_o_crear_subcarpeta(parent_folder_id, nombre_carpeta):
    """Devuelve el id de una subcarpeta bajo parent_folder_id, creándola si no existe."""
    func_name = "drive_obtener_o_crear_subcarpeta"
    if not parent_folder_id or not nombre_carpeta:
        return None
    safe_q = nombre_carpeta.replace("'", "\\'")
    try:
        service, msg = crear_servicio_google_drive()
        if not service:
            log_warning(f"{func_name}: {msg}", func_name)
            return None
        q = (
            f"mimeType='application/vnd.google-apps.folder' and trashed=false "
            f"and '{parent_folder_id}' in parents and name='{safe_q}'"
        )
        ...
        fid = f.get('id')
        log_info(f"Carpeta tangenciales en Drive: {nombre_carpeta} → {fid}", func_name)
        return fid
    except Exception as e:
        log_exception(func_name, e)
        return None
...
def resolver_carpeta_drive_destino_tangenciales(cliente):
    """Raíz tangenciales + subcarpeta por entidad (intrant, edesur, minerd) si aplica."""
    base = obtener_google_drive_folder_id_tangenciales(cliente)
    if not base:
        return ''
    gdrive = (cliente or {}).get('google_drive', {})
    usar_sub = bool(gdrive.get('tangenciales_usar_subcarpeta_cliente'))
    # Raíz compartida por defecto: una subcarpeta por cliente evita mezclar tangenciales
    if not usar_sub and base.strip() == GOOGLE_DRIVE_FOLDER_TANGENCIALES_DEFAULT.strip():
        usar_sub = True
    if not usar_sub:
        return base
    sub = drive_obtener_o_crear_subcarpeta(base, _slug_tangencial_cliente(cliente))
    return sub or ''


def subir_tangencial_videoscheck_a_google_drive(cliente, ruta_video, ruta_txt, transcripcion_contexto=""):
    """
    Parte A tangenciales: tras guardar en videoscheck local, sube video + txt de minutos
    y un .txt con transcripción/contexto si hay texto. No altera el flujo local si falla Drive.

    Returns:
        (ok: bool, mensaje: str, web_view_link_video: str) — enlace de visualización del MP4 en Drive si hubo subida.
    """
    func_name = "subir_tangencial_videoscheck_a_google_drive"
    try:
        target = resolver_carpeta_drive_destino_tangenciales(cliente)
        if not target:
            return False, "omitido (sin carpeta tangenciales)", ""
        if not ruta_video or not os.path.exists(ruta_video):
            return False, "omitido (sin archivo de video)", ""

        subidos = []
        link_video_drive = ""
        r1, m1 = subir_archivo_google_drive(ruta_video, folder_id=target)
        if not r1:
            log_warning(f"{func_name}: video — {m1}", func_name)
            return False, m1, ""
        subidos.append(os.path.basename(ruta_video))
        link_video_drive = (r1.get("webViewLink") or "").strip()

        if ruta_txt and os.path.exists(ruta_txt):
            r2, m2 = subir_archivo_google_drive(ruta_txt, folder_id=target)
            if not r2:
                log_warning(f"{func_name}: txt minutos — {m2}", func_name)
            else:
                subidos.append(os.path.basename(ruta_txt))

        base = os.path.splitext(os.path.basename(ruta_video))[0]
        bloques_tc = []
        tc_ctx = (transcripcion_contexto or '').strip()
        if tc_ctx:
            bloques_tc.append(tc_ctx)
        if ruta_txt and os.path.exists(ruta_txt):
            try:
                with open(ruta_txt, 'r', encoding='utf-8', errors='replace') as tf:
                    bloques_tc.append('--- TXT minuto / registro local ---\n' + (tf.read()[:20000]))
            except Exception as e_txt:
                log_warning(f"{func_name}: leer txt local — {e_txt}", func_name)
        texto_drive = '\n\n'.join(bloques_tc).strip() if bloques_tc else ''
        if len(texto_drive) < 40:
            texto_drive = (
                '(Transcripción ampliada no disponible en este envío.)\n\n' + texto_drive
            ).strip() or 'Tangencial: sin bloque de texto adicional en este envío.'
        nombre_tx = f"{base}_transcripcion_contexto.txt"
        r3, m3 = subir_texto_google_drive(texto_drive, nombre_tx, folder_id=target)
        if not r3:
            log_warning(f"{func_name}: transcripción consolidada — {m3}", func_name)
        else:
            subidos.append(nombre_tx)

        cn = (cliente or {}).get('nombre', '?')
        log_info(f"Tangenciales Drive OK ({cn}): {', '.join(subidos)} | link_video={bool(link_video_drive)}", func_name)
        return True, f"subido: {', '.join(subidos)}", link_video_drive
    except Exception as e:
        log_exception(func_name, e)
        ...
        if supabase:
            ...
            try:
                for item in coincidencias_items:
                    data = {
                        'termino_detectado': item.get('termino', ''),
                        'nombre_archivo': nombre_archivo,
...
def enviar_coincidencia_a_cliente(cliente, nombre_archivo, termino_encontrado, contexto_termino, tipo_archivo, clip_path=None, transcripcion_completa="", timestamp=None, idea_general=None, video_url=None, video_url_bunny=None, video_url_r2=None, video_path=None, transcripcion_segmento=""):
    """
    ...
    clave_dedupe = calcular_clave_dedupe_envio(
        cliente, termino_encontrado, timestamp, contexto_termino,
        idea_general=idea_general, transcripcion_segmento=transcripcion_segmento
    )
    if dedupe_envio_ya_registrado(clave_dedupe):
        log_info(f"⏭️ Envío omitido (dedupe): ya notificado | {termino_encontrado}", func_name)
        st.info("⏭️ Esta coincidencia ya fue enviada (mismo término, instante y contexto). No se reenvía.")
        with st.expander("📋 Flujo de envío — paso a paso", expanded=True):
            st.markdown(
                "**0. Dedupe de envíos** — ⏭️ **Omitido**: esta huella ya está en "
                f"`envios_coincidencias_dedupe.json` (no se repiten Telegram, Drive, correo, etc.)."
            )
            st.caption(
                "Si esperabas ver algo nuevo en Drive y no aparece, puede ser un reproceso "
                "del mismo instante/contexto; borra la entrada del dedupe o cambia el archivo/timestamp."
            )
    ...
    if clip_path and os.path.exists(clip_path):
        try:
            ...
            if not nombre_base.startswith(prefijo):
                ...
                try:
                    nombre_src = os.path.splitext(os.path.basename(nombre_archivo))[0]
                    patron_fh = r'(\d{4})-(\d{2})-(\d{2})_(\d{2})-(\d{2})-(\d{2})'
                    match_nom = re.search(patron_fh, nombre_src)
                    if match_nom:
                        canal_raw = nombre_src[: match_nom.start()].strip("_").split("_")[0]
                    else:
                        canal_raw = nombre_src.split("_")[0]
                    canal = re.sub(r"[^\w]", "", canal_raw) or "Medio"
                except Exception:
                    canal = re.sub(r"[^\w]", "", nombre_base.split("_")[0]) or "Medio"
    ...
    # Encabezado legible tipo «Canal 13 de mayo a las 7:28» cuando el archivo sigue `_NNNp_fecha_hora_seg`
    _emision_bn = os.path.basename(nombre_archivo)
    _titulo_emision_mostrar, _ref_arch_emit_tecnico = archivo_broadcast_legible_y_referencia(_emision_bn)
    _bloque_archivo_emit = ""
    if _ref_arch_emit_tecnico:
        _bloque_archivo_emit = f"\n\n> **Nombre de archivo:** `{_emision_bn}`\n\n"
    ...
    mensaje_completo = f"# 📊 ANÁLISIS COMPLETO: {_titulo_emision_mostrar}{_bloque_archivo_emit}{resumen_ejecutivo}"
    
    # Para el correo: incluir resumen + transcripción siempre que haya transcripción (cualquier longitud)
    _tx_envio = (transcripcion_completa or "").strip()
    resumen_para_email = (
    ...
    _n_tx = len((transcripcion_completa or "").strip())
    _clip_ui = clip_path and os.path.isfile(clip_path)
    st.markdown("### 📋 Contexto del envío (qué hay disponible)")
    st.markdown(
        f"- **Archivo:** `{nombre_archivo}` "
        + (f"→ **emisión:** {_titulo_emision_mostrar}" if _ref_arch_emit_tecnico else "")
    )
    st.markdown(
        f"- **Clip (.mp4)** para CDN / Telegram / Drive vídeo: {'✅ `' + os.path.basename(clip_path) + '`' if _clip_ui else '❌ Sin archivo local — el vídeo no irá a Drive/Telegram como archivo; suele pasar si el clip es &lt;90s (solo texto), verificación fallida o clip borrado.'}"
    )
    st.markdown(
        f"- **Transcripción** pasada a este envío: **{_n_tx}** caracteres"
        + (" — el TXT en Drive y el correo incluyen la transcripción **si hay al menos un carácter** (tras quitar espacios)." if _n_tx else " — **sin transcripción** en este envío; el TXT no llevará bloque TRANSCRIPCIÓN COMPLETA.")
    )
    st.markdown(
        f"- **Google Drive** para este cliente (`google_drive.enabled`): **{'✅ encendido' if cliente.get('google_drive', {}).get('enabled') else '❌ apagado — no sube TXT ni MP4'}**"
    )
    st.divider()
    ...
    if cliente.get('telegram', {}).get('enabled'):
        try:
            st.markdown("##### 1️⃣ Telegram")
            with st.spinner(f"📱 Enviando a Telegram de {cliente_nombre}..."):
                exito, msg = enviar_telegram_cliente(
                    cliente, mensaje_completo, clip_path, video_url=video_url, video_url_bunny=video_url_bunny, video_url_r2=video_url_r2
                )
        ...
        st.markdown("##### 1️⃣ Telegram")
        st.caption("⏭️ Omitido — `telegram.enabled` es falso para este cliente.")

    # === 2. ENVIAR A WEBHOOK ===
    if cliente.get('webhook', {}).get('enabled'):
        try:
            st.markdown("##### 2️⃣ Webhook")
            with st.spinner(f"🌐 Enviando a Webhook de {cliente_nombre}..."):
                exito, msg = enviar_webhook_cliente(
                    cliente, clip_path or nombre_archivo, mensaje_completo, [termino_encontrado],
                    video_url=video_url, video_url_bunny=video_url_bunny, video_url_r2=video_url_r2
                )
    ...
    if cliente.get('brevo', {}).get('enabled'):
        ...
        st.markdown("##### 3️⃣ Correo (Brevo)")
        st.caption("⏭️ Omitido — `brevo.enabled` es falso para este cliente.")

    # === 4. ENVIAR A GOOGLE DRIVE ===
    if cliente.get('google_drive', {}).get('enabled'):
        try:
            st.markdown("##### 4️⃣ Google Drive (TXT resumen + transcripción si aplica + .mp4 si hay clip)")
            with st.spinner(f"☁️ Subiendo a Google Drive de {cliente_nombre}..."):
                # Generar nombre base compartido para que el TXT y el video hagan match
                ...
                nombre_base_gdrive = f"{termino_encontrado}_{nombre_video_limpio}_{gdrive_timestamp}"
                
                exito_txt, msg_txt = False, "TXT no procesado"
                ...
                try:
                    ...
                    contenido_drive += f"Término: {termino_encontrado}\n"
                    contenido_drive += f"Fecha: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
                    contenido_drive += f"Emisión (legible): {_titulo_emision_mostrar}\n"
                    if _ref_arch_emit_tecnico:
                        contenido_drive += f"Nombre de archivo (técnico): {_emision_bn}\n"
                    contenido_drive += "\n"
                    contenido_drive += f"{'='*50}\nRESUMEN EJECUTIVO:\n{'='*50}\n\n{resumen_ejecutivo}\n\n"
                    if contexto_termino:
                        contenido_drive += f"{'='*50}\nCONTEXTO:\n{'='*50}\n\n{contexto_termino}\n\n"
                    if (transcripcion_completa or "").strip():
                        contenido_drive += f"{'='*50}\nTRANSCRIPCIÓN COMPLETA:\n{'='*50}\n\n{transcripcion_completa}\n"
                    with open(txt_path, 'w', encoding='utf-8') as f:
                        f.write(contenido_drive)
                    exito_txt, msg_txt, _ = enviar_gdrive_cliente(cliente, txt_path, f"{nombre_base_gdrive}.txt")
                    os.remove(txt_path)
                except Exception as e_txt_gd:
                    exito_txt, msg_txt = False, f"Error creando TXT: {e_txt_gd}"
                    log_warning(f"Drive TXT coincidencia falló al crear temporal o subir: {e_txt_gd}", func_name)
                
                # Subir video clip con el mismo nombre base
                exito_video = False
                msg_video = "Sin video"
                if clip_path and os.path.exists(clip_path):
                    exito_video, msg_video, link_drive_video_coincidencia = enviar_gdrive_cliente(
                        cliente, clip_path, f"{nombre_base_gdrive}.mp4"
                    )
                else:
                    exito_video = False
                    msg_video = "Sin video (sin ruta de clip o archivo no existe en disco en este paso)"

                resultados['google_drive'] = (exito_txt or exito_video, f"TXT: {msg_txt} | Video: {msg_video}")
                if exito_txt or exito_video:
                    st.success(f"☁️ Google Drive: TXT={'✅' if exito_txt else '❌'} Video={'✅' if exito_video else '❌'}")
                else:
                    st.warning(f"☁️ Google Drive: {msg_txt} | {msg_video}")
                if link_drive_video_coincidencia:
                    st.caption(f"Link Drive del clip: {link_drive_video_coincidencia}")
                st.caption(f"Detalle Drive — TXT: {msg_txt} · Video: {msg_video}")
        except Exception as e_gd:
            log_error_critico(func_name, f"Error inesperado Google Drive (se continúa con otros destinos): {e_gd}")
            resultados['google_drive'] = (False, str(e_gd))
            st.warning(f"☁️ Google Drive: {e_gd}")
    else:
        st.markdown("##### 4️⃣ Google Drive")
        st.caption("⏭️ Omitido — `google_drive.enabled` es falso para este cliente.")
    
    # === 5. ENVIAR A SUPABASE ===
    if cliente.get('supabase', {}).get('enabled'):
        try:
            st.markdown("##### 5️⃣ Supabase")
            ...
            log_error_critico(func_name, f"Error inesperado Supabase (se continúa con Sheets si aplica): {e_sb}")
            resultados['supabase'] = (False, str(e_sb))
            st.warning(f"🗄️ Supabase: {e_sb}")
    else:
        st.markdown("##### 5️⃣ Supabase")
        st.caption("⏭️ Omitido — `supabase.enabled` es falso para este cliente.")
    
    # === 6. GOOGLE SHEETS (una hoja por cliente, columnas fijas) ===
    gs = cliente.get("google_sheets") or {}
    if gs.get("enabled") and (gs.get("spreadsheet_id") or "").strip():
        try:
            st.markdown("##### 6️⃣ Google Sheets")
            with st.spinner(f"📊 Enviando fila a Google Sheets de {cliente_nombre}..."):
                log_info(
                    f"Google Sheets: cliente={cliente_nombre} spreadsheet={gs.get('spreadsheet_id','')[:20]}… rango={gs.get('range')}",
                    func_name,
                )
                ...
                log_info(
                    f"Sheets coincidencia: cliente={cliente_nombre} formato={formato} incluir_indice={incluir_indice}",
                    func_name,
                )
                ok_gs, msg_gs = append_fila_google_sheet(
                    gs["spreadsheet_id"].strip(),
                    rango,
                    fila,
                    incluir_indice=incluir_indice,
                )
        ...
        st.markdown("##### 6️⃣ Google Sheets")
        st.caption("⏭️ Omitido — sin hoja configurada o `google_sheets.enabled` es falso.")
    
    # === GENERAR MD ANALISISHOY ===
    # Guardar todas las coincidencias de la sesión en Analisishoy_YYYYMMDD.md
    st.markdown("##### 7️⃣ AnalisisHoy (MD local)")
    try:
        ...
        if ok_md:
            ...
            st.caption(f"Informe general: `{INFORME_GENERAL_PATH}`")
        else:
    ...
    if total_exitosos > 0:
        dedupe_registrar_envio_exitoso(clave_dedupe, termino_encontrado, cliente_nombre)
...
def enviar_coincidencia_inmediata(nombre_archivo, termino_encontrado, contexto_termino, tipo_archivo, clip_path=None, transcripcion_completa="", timestamp=None, idea_general=None, video_url=None, video_url_bunny=None, transcripcion_segmento="", video_url_r2_precargada=None):
    """
    ...
    try:
        ...
        cliente_nombre = cliente.get('nombre', 'Sistema Principal')
        ...
        if (not video_url_bunny) and clip_path and os.path.exists(clip_path):
            ...
            if bunny_eff.get('enabled'):
                try:
                    st.info("🐰 Subiendo video a Bunny Storage...")
                    video_url_bunny, bunny_msg = subir_video_bunny(
                        clip_path, termino_encontrado, "", bunny_cfg=cliente.get('bunny') or {}
                    )
                    if video_url_bunny:
                        log_info(f"✅ Video subido a Bunny: {video_url_bunny}", func_name)
                        st.success("🐰 Video subido a Bunny Storage")
                    else:
                        log_warning(f"⚠️ Error subiendo a Bunny: {bunny_msg}", func_name)
                except Exception as e:
        ...
        if isinstance(resultados, dict) and resultados.get("_dedupe_skip"):
            razon = resultados["_dedupe_skip"][1]
            st.info(f"⏭️ {razon}")
            log_info(f"Dedupe envío: {razon}", func_name)
            return True, razon, video_url_cloudinary or video_url, video_url_r2
        ...
        return True, f"Enviado a cliente: {cliente_nombre}", video_url_cloudinary or video_url, video_url_r2
...
def cargar_bunny_config():
    """Carga configuración de Bunny Storage (prioriza variables de entorno)."""
    default_config = {
        'enabled': False,
        'storage_zone': os.getenv('BUNNY_STORAGE_ZONE', ''),
        'access_key': os.getenv('BUNNY_STORAGE_ACCESS_KEY', ''),
        'folder': 'video_analyzer_clips',
        'cdn_base_url': os.getenv('BUNNY_CDN_BASE_URL', ''),
    }
    try:
        if os.path.exists(BUNNY_CONFIG):
            with open(BUNNY_CONFIG, 'r', encoding='utf-8') as f:
                config = json.load(f)
                default_config.update(config)
    except Exception as e:
        st.warning(f"⚠️ Error cargando configuración Bunny: {e}")
    return default_config

def guardar_bunny_config(config):
    """Guarda configuración de Bunny Storage."""
    try:
        with open(BUNNY_CONFIG, 'w', encoding='utf-8') as f:
            json.dump(config, f, indent=2, ensure_ascii=False)
        return True
    except Exception as e:
        st.error(f"❌ Error guardando configuración Bunny: {e}")
        return False

def _bunny_config_resuelta(bunny_cfg):
    """Fusiona dict de cliente con config global (campos vacíos heredan globales)."""
    base = cargar_bunny_config()
    if not bunny_cfg:
        return base
    out = base.copy()
    for k, v in bunny_cfg.items():
        if v is not None and v != '':
            out[k] = v
    if bunny_cfg.get('enabled') is True or bunny_cfg.get('enabled') is False:
        out['enabled'] = bunny_cfg['enabled']
    return out
...
def subir_video_bunny(video_path, termino="", timestamp="", bunny_cfg=None):
    ...
    try:
        cfg = _bunny_config_resuelta(bunny_cfg) if bunny_cfg is not None else cargar_bunny_config()
        if not cfg.get('enabled'):
            return None, "Bunny deshabilitado"
        zone = (cfg.get('storage_zone') or '').strip()
        key = (cfg.get('access_key') or '').strip()
        if not zone or not key:
            return None, "Bunny no configurado (storage_zone o access_key)"
        if not video_path or not os.path.isfile(video_path):
            return None, "Archivo de video no encontrado"

        folder = (cfg.get('folder') or 'video_analyzer_clips').strip().strip('/')
        nombre_base = os.path.splitext(os.path.basename(video_path))[0]
        ext = os.path.splitext(video_path)[1] or '.mp4'
        if termino and timestamp:
            remote_name = f"{termino}_{timestamp}_{nombre_base}{ext}"
        elif termino:
            remote_name = f"{termino}_{nombre_base}{ext}"
        else:
            remote_name = f"{nombre_base}{ext}"
        path_parts = [quote(p, safe='-_.') for p in folder.split('/') if p] + [quote(remote_name, safe='-_.')]
        object_path = '/'.join(path_parts)

        url_put = f"https://storage.bunnycdn.com/{quote(zone, safe='')}/{object_path}"
        ...
        resp = requests.put(
            ...
            timeout=max(60, int(len(body) / (1024 * 1024) * 3) or 120),
        )
        if resp.status_code not in (200, 201, 204):
            return None, f"Bunny HTTP {resp.status_code}: {resp.text[:120]}"

        cdn = (cfg.get('cdn_base_url') or '').strip().rstrip('/')
        if not cdn:
            cdn = f"https://{zone}.b-cdn.net"
        public_url = f"{cdn}/{object_path}"
        return public_url, "Video subido a Bunny exitosamente"
    except Exception as e:
        return None, f"Error subiendo a Bunny: {str(e)[:100]}"
...
def _r2_upload_disponible():
    return r2_storage is not None and r2_storage.r2_ready()


def _cliente_debe_subir_r2(cliente):
    """True si hay R2 en .env y el cliente no lo desactiva explícitamente (enabled: false)."""
    if not _r2_upload_disponible():
        return False
    r2c = (cliente or {}).get("r2") or {}
    if r2c.get("enabled") is False:
        return False
    return True


def subir_video_r2(video_path, termino="", timestamp=None, r2_cfg=None):
    """
    Sube un clip a Cloudflare R2 con convención de clave alineada a Cloudinary/Bunny.
    Credenciales globales en .env (R2_*); prefijo de carpeta en cliente r2.folder o Cloudinary folder.
    """
    func_name = "subir_video_r2"
    if not _r2_upload_disponible():
        return None, "R2 no disponible (pip install boto3 + variables R2_* o R2_ENABLED=off)"

    cfg = r2_cfg if isinstance(r2_cfg, dict) else {}
    folder = (cfg.get("folder") or "").strip().strip("/")
    if not folder:
        folder = (cargar_cloudinary_config().get("folder") or "video_analyzer_clips").strip().strip("/")

    nombre_base = os.path.splitext(os.path.basename(video_path))[0]
    ext = os.path.splitext(video_path)[1].lower() or ".mp4"
    ts_suffix = ""
    if timestamp is not None and str(timestamp).strip() != "":
        ts_suffix = str(timestamp).replace(".", "_")

    if termino and ts_suffix:
        remote = f"{termino}_{ts_suffix}_{nombre_base}{ext}"
    elif termino:
        remote = f"{termino}_{nombre_base}{ext}"
    else:
        remote = f"{nombre_base}{ext}"

    key = f"{folder}/{remote}".lstrip("/")
    try:
        url, uploaded_key = r2_storage.upload_local_file(video_path, key)
        log_info(f"✅ R2: {uploaded_key}", func_name)
        return url, f"Video subido a R2 ({uploaded_key})"
    except Exception as e:
        log_warning(f"{func_name}: {e}", func_name)
...
def crear_servicio_google_sheets():
    """Crea el cliente de Google Sheets API v4 (mismas credenciales OAuth que Drive)."""
    try:
        creds = obtener_credenciales_google_drive()
        if not creds:
...
def extraer_fecha_ddmmyyyy_desde_archivo(nombre_archivo):
    """Deriva DD/MM/YYYY desde un patrón de fecha en el nombre del archivo (emisión)."""
...
def extraer_nombre_medio_corto_desde_archivo(nombre_archivo):
    """Nombre corto del medio/canal a partir del nombre de archivo (p. ej. antes de _720p_ y la fecha)."""
    try:
...
def archivo_broadcast_legible_y_referencia(nombre_archivo):
    """
    Archivos tipo: EnTelevision_720p_2026-05-13_07-28-17_seg013(.mp4)
    → texto legible: «EnTelevision 13 de Mayo a las 7:28»
    • Parte antes de _720p / _480p / _NNNp_ = medio/emisor.
    • Tras eso viene fecha AAA-MM-DD_HH-MM-SS y opcional _segNNN.

    Returns:
        tuple: (titulo_legible, nombre_tecnico_basename_o_None)
        Si no coincide el patrón: (nombre con espacios en lugar de '_', None).
    """
    MESES_ES = (
    ...
    try:
        base = os.path.basename((nombre_archivo or "").strip())
        nombre_sin_ext, _ = os.path.splitext(base)
        if not nombre_sin_ext:
            return (base or ""), None
        rx = re.compile(
            r"^(?P<medio>.+?)_(?P<rx>\d+p)_(?P<y>\d{4})-(?P<mo>\d{2})-(?P<d>\d{2})_"
            r"(?P<h>\d{2})-(?P<mi>\d{2})-(?P<s>\d{2})(?:_seg\d+)?$",
            re.IGNORECASE,
        )
        m = rx.match(nombre_sin_ext)
        if not m:
            fb = nombre_sin_ext.replace("_", " ").strip() or base
            return fb, None

        medio_disp = m.group("medio").replace("_", " ").strip()
        dia = int(m.group("d"))
        mes_ix = int(m.group("mo"))
        anio = int(m.group("y"))
        hh = int(m.group("h"))
        mint = int(m.group("mi"))
        if mes_ix < 1 or mes_ix > 12:
            fb = nombre_sin_ext.replace("_", " ").strip() or base
            return fb, None

        mes_nom = MESES_ES[mes_ix - 1].capitalize()
        hora_txt = f"{hh}:{mint:02d}"
        año_actual = datetime.now().year
        if anio != año_actual:
            legible = f"{medio_disp} {dia} de {mes_nom} de {anio} a las {hora_txt}"
        else:
            legible = f"{medio_disp} {dia} de {mes_nom} a las {hora_txt}"
        return legible, (base if base else None)
    except Exception:
        fb = os.path.splitext(os.path.basename(nombre_archivo or ""))[0].replace("_", " ").strip()
        return fb or (nombre_archivo or ""), None


def archivo_broadcast_lineas_correo_plano(nombre_archivo):
    """Texto plano para correos: línea legible + referencia técnica si aplica."""
    leg, ref = archivo_broadcast_legible_y_referencia(nombre_archivo)
    if ref:
        return f"Emisión (legible): {leg}\nNombre de archivo: {ref}"
    return f"Archivo: {leg}"
...
def analizar_sentimiento_mencion_heuristica(texto):
    ...
    negative_words = [
        ...
        "apagones", "corte", "interrupción", "daño", "roto", "no funciona", "pésimo", "horrible",
...
def _es_cliente_intrant_para_sheet(cliente):
    """True si el formato de Sheet del cliente es el de Intrant (sin # y con título)."""
    if not cliente:
        return False
    cid = str(cliente.get("id") or "").strip().lower()
    if cid == "intrant":
    ...
    return "intrant" in nombre


def _compactar_texto_sheet(texto, max_len=120):
    """Compacta espacios y limita longitud para títulos en Sheets."""
    raw = re.sub(r"\s+", " ", str(texto or "")).strip()
    if not raw:
        return ""
    if len(raw) <= max_len:
        return raw
...
def _titulo_intrant_sheet(termino_encontrado="", texto_gs="", es_tangencial=False, motivo_tangencial=""):
    """Arma título para estructura Intrant."""
    termino = _compactar_texto_sheet(termino_encontrado, max_len=45) or "Monitoreo"
    if es_tangencial:
        base = _compactar_texto_sheet(motivo_tangencial or texto_gs, max_len=90)
        if base:
            return _compactar_texto_sheet(f"[Tangencial] {termino}: {base}", max_len=150)
        return _compactar_texto_sheet(f"[Tangencial] {termino}", max_len=150)
    base = _compactar_texto_sheet(texto_gs, max_len=100)
    if base:
        return _compactar_texto_sheet(f"{termino}: {base}", max_len=150)
    return termino


def construir_fila_google_sheet_cliente(
    cliente,
    nombre_archivo,
    texto_gs,
    sent_gs,
    ...
    """
    Construye fila de Sheets según el formato del cliente.
    - Edesur/default: con índice (#) + [fecha, periodista, texto, medio, sentimiento, url]
    - Intrant: sin índice + [fecha, periodista, titulo, texto, medio, sentimiento, url]
    """
    ...
    texto_norm = (texto_gs or "").strip() or "Sin texto"
    sent_norm = (sent_gs or "").strip() or "Neutral"
    url_norm = (url_gs or "").strip()

    if _es_cliente_intrant_para_sheet(cliente):
        titulo_intrant = _titulo_intrant_sheet(
            termino_encontrado=termino_encontrado,
            texto_gs=texto_norm,
            es_tangencial=es_tangencial,
            motivo_tangencial=motivo_tangencial,
        )
        fila = [fecha_gs, "Redaccion", titulo_intrant, texto_norm, medio_gs, sent_norm, url_norm]
        return fila, False, "intrant"

    periodista = "TV Tangencial" if es_tangencial else "TV"
    fila = [fecha_gs, periodista, texto_norm, medio_gs, sent_norm, url_norm]
    return fila, True, "default"


def append_fila_google_sheet(spreadsheet_id, range_a1, fila_valores, incluir_indice=True):
    """
    Añade una fila al spreadsheet.
    Inserta una fila en el rango destino.
    - incluir_indice=True: antepone # en col A (formato Edesur/default).
    - incluir_indice=False: inserta tal cual (formato Intrant).
    """
    ...
    try:
        ...
        if not isinstance(fila_valores, (list, tuple)) or len(fila_valores) == 0:
            return False, "Sheets: fila inválida (vacía o tipo no soportado)"
        service, msg = crear_servicio_google_sheets()
        if not service:
            return False, msg
        fila_final = list(fila_valores)
        siguiente = None
        if incluir_indice:
            titulo = titulo_hoja_desde_range_a1(range_a1)
            siguiente = siguiente_indice_columna_a(service, spreadsheet_id, titulo)
            fila_final = [siguiente] + fila_final
        body = {"values": [fila_final]}
        ...
        log_info(
            f"Fila añadida a Sheet (sin #) id={spreadsheet_id} rango={range_a1}",
            func_name,
        )
...
def enviar_tangenciales_a_google_sheets(menciones_tangenciales_data):
    """
    Envía menciones tangenciales a la misma hoja configurada por cliente.
    Retorna lista de dicts con métricas por cliente.
    """
    ...
    for tangencial in menciones_tangenciales_data:
        ...
        cliente_id = str(cliente.get("id") or "default")
        cliente_nombre = cliente.get("nombre", cliente_id)
...
"""
        ...
        # Subir TRANSCRIPCIÓN COMPLETA a Google Drive (cualquier longitud si hay texto)
        if (transcripcion_completa or "").strip():
    ...
    try:
        ...
        for item in coincidencias_items:
            ...
            registro = {
                ...
                'url_video': url_vid_reg if url_vid_reg else '',
                'nombre_archivo': nombre_archivo if nombre_archivo else 'archivo_desconocido',
                'enlace_directo': enl_reg if enl_reg else '',
...
def verificar_todas_las_apis():
    ...
    # 6. Verificar Bunny Storage
    with st.spinner("🐰 Verificando Bunny Storage..."):
        try:
            bunny_config = cargar_bunny_config()
            if bunny_config.get('enabled') and bunny_config.get('storage_zone') and bunny_config.get('access_key'):
                archivo_prueba = os.path.join(CARPETA_PROCESADOS, "test_bunny_connection.txt")
                with open(archivo_prueba, 'w', encoding='utf-8') as f:
                    f.write("Prueba de conexión Bunny Storage")
                bunny_ok, bunny_msg = subir_video_bunny(archivo_prueba, "prueba_conexion", "", bunny_cfg=None)
                if os.path.exists(archivo_prueba):
                    try:
                        os.remove(archivo_prueba)
                    except OSError:
                        pass
                resultados['bunny'] = {
                    'activo': bool(bunny_ok),
                    'mensaje': bunny_msg,
                    'icono': '✅' if bunny_ok else '❌'
                }
                if bunny_ok:
                    st.success(f"🐰 **Bunny Storage**: {bunny_msg}")
                else:
                    st.error(f"🐰 **Bunny Storage**: {bunny_msg}")
            else:
                resultados['bunny'] = {
                    'activo': False,
                    'mensaje': "Bunny no configurado",
                    'icono': '⚠️'
                }
                st.warning("🐰 **Bunny Storage**: No configurado")
        except Exception as e:
            resultados['bunny'] = {
                'activo': False,
                'mensaje': f"Error verificando Bunny: {str(e)[:100]}",
                'icono': '❌'
            }
            st.error(f"🐰 **Bunny Storage**: Error verificando conexión")
...
# === INFORME GENERAL ACUMULATIVO (Escritorio/informes/informe_general.md) ===

def _informe_formato_tiempo_audio(ts_seg):
    """Posición en el archivo: XmYYs."""
    try:
...
def _informe_hora_deteccion(item):
...
def _informe_medio_desde_tangencial(item):
    medio = (item.get("medio") or "").strip()
    if medio:
        return medio
    arch = str(item.get("archivo") or "").strip()
    return os.path.splitext(os.path.basename(arch))[0] if arch else "?"


def construir_tangenciales_narrativo_informe(items_list):
    """Resumen agrupado por término para informe_general / reporte de sesión."""
    if not items_list:
        return ""
    ...
    intro = (
        ...
        "Estos términos se detectaron pero fueron rechazados para generación de clip "
    ...
    for termino in sorted(by_term.keys(), key=lambda x: x.lower()):
        ...
        archivos, visto = [], set()
        for it in occ:
            nombre = os.path.basename(str(it.get("archivo") or "?"))
            k = nombre.lower()
            if k not in visto:
                visto.add(k)
        ...
        lista = ", ".join(archivos)
        bloques.append(
            f"- **{termino}**: tangencial en **{len(archivos)}** archivo(s): {lista} "
            f"(**{len(occ)}** menciones sin clip). Detalle en la tabla.\n"
        )
    return intro + "".join(bloques) + "\n*Implicaciones:* revisar si el término merece monitoreo más estricto.\n"


def append_informe_general(titulo, contenido_md):
    """Añade una sección al informe general acumulativo."""
    func_name = "append_informe_general"
    try:
        informe_path = INFORME_GENERAL_PATH
        os.makedirs(os.path.dirname(informe_path), exist_ok=True)
        existe = os.path.exists(informe_path) and os.path.getsize(informe_path) > 0
        hora = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        with open(informe_path, "a", encoding="utf-8") as f:
            if not existe:
                f.write("# Informe General — Video Analyzer\n\n")
                f.write("> Coincidencias, tangenciales y reportes de sesión (acumulativo).\n\n")
            f.write("\n---\n\n")
            f.write(f"## {titulo}\n\n")
...
def escribir_informe_general(contenido_md):
    """Añade el reporte de sesión completo al informe general."""
    return append_informe_general("Reporte de sesión completo", contenido_md)


# === GENERAR MD DE COINCIDENCIAS EN CARPETA PROCESADOS ===

def generar_md_sesion_coincidencias(
    videos_procesados_data,
    clips_generados_en_sesion,
    estadisticas_escaneo=None,
    terminos_buscados=None,
    menciones_tangenciales_data=None,
):
    """
    ...
    try:
        ...
        if estadisticas_escaneo:
            ...
            md.append(f"| **Videos encontrados** | {estadisticas_escaneo.get('total_videos', 'N/A')} |")
        ...
        if videos_procesados_data:
            ...
            for vid_idx, video_data in enumerate(videos_procesados_data, 1):
                ...
                md.append(f"### 🎞️ Video {vid_idx}: `{nombre}`")
                md.append(f"")
                ...
                if coincidencias:
                    ...
                    for c_idx, item in enumerate(coincidencias, 1):
                        ...
                        url_bunny_md = item.get('url_bunny', None)
                        ...
                        if url_cloud:
                            md.append(f"   - URL Cloudinary: [{url_cloud}]({url_cloud})")
                        if url_bunny_md:
                            md.append(f"   - URL Bunny: [{url_bunny_md}]({url_bunny_md})")
                        if contexto:
                ...
                if clips:
                    ...
                    for cl_idx, clip in enumerate(clips, 1):
                        ...
                        clip_bunny = clip.get('url_bunny', None)
                        ...
                        if clip_bunny:
                            md.append(f"   - URL Bunny: [{clip_bunny}]({clip_bunny})")
                        md.append(f"")
                
                # Transcripción de este video (cualquier longitud si hay texto)
        ...
        if menciones_tangenciales_data:
            md_tangenciales = construir_tangenciales_narrativo_informe(menciones_tangenciales_data)
            ...
            for tang in menciones_tangenciales_data:
                ...
                ta = _informe_formato_tiempo_audio(ts_seg)
                medio = _informe_medio_desde_tangencial(tang).replace('|', '\\|')
                term = str(tang.get('termino', '')).replace('|', '\\|')
                motivo = str(tang.get('motivo', '')).replace('|', '\\|')
                hd = _informe_hora_deteccion(tang).replace('|', '\\|')
        ...
        ok_informe, msg_informe = escribir_informe_general(contenido_final)
        if not ok_informe:
...
def generar_archivo_coincidencias_md(termino_encontrado, resumen_completo, nombre_video, info_medio="", terminos_detectados=[], video_url=None, video_url_bunny=None, video_path=None):
    """
...
"""
        if video_url_bunny:
            contenido_md += f"""
**🐰 Video Bunny CDN:** [{video_url_bunny}]({video_url_bunny})
"""
...
def generar_analisishoy_md(nombre_archivo, termino_encontrado, contexto_termino="", resumen_ejecutivo="", transcripcion_completa="", video_url=None, video_url_bunny=None, video_url_r2=None, info_medio=""):
    """
    ...
    try:
        ...
        transcripcion_display = transcripcion_completa.strip() if (transcripcion_completa or "").strip() else "No disponible"
...
- **Video Bunny CDN:** {f'[{video_url_bunny}]({video_url_bunny})' if video_url_bunny else 'No disponible'}
- **Video Cloudflare R2:** {f'[{video_url_r2}]({video_url_r2})' if video_url_r2 else 'No disponible'}
...
def append_analisishoy_menciones_tangenciales(menciones_tangenciales_data):
    """
    Añade al Analisishoy_YYYYMMDD.md una sección de menciones tangenciales (mismo formato
    que la UI: archivo, término, motivo, tiempo). Si el archivo no existe, lo crea con el
    mismo encabezado que generar_analisishoy_md.
    """
    ...
    try:
        ...
        md_narr = construir_tangenciales_narrativo_informe(menciones_tangenciales_data)
        lineas_tabla = [
        ...
        for tang in menciones_tangenciales_data:
            ...
            medio = _informe_medio_desde_tangencial(tang).replace('|', '\\|')
            term = str(tang.get('termino', '')).replace('|', '\\|')
            motivo = motivo_display_tangencial(tang).replace('|', '\\|')
            hd = _informe_hora_deteccion(tang).replace('|', '\\|')
...
def crear_plantilla_email_html(termino_encontrado, resumen_completo, nombre_video, info_medio="", terminos_detectados=[], video_url=None, transcripcion_segmento="", video_url_bunny=None, video_url_r2=None):
    ...
    if _vid_ref:
        bloque_archivo_tecnico = (
            f'<div style="font-size: 13px; color: #868e96; margin-top: 6px; font-weight: 400;">'
            f'Nombre de archivo: <span style="word-break: break-all;">'
            f'{html_module.escape(_vid_ref)}</span></div>'
        )
    ...
    play_intro_section = ""
    _intro_primary = video_url or video_url_bunny or v_r2_raw or None
    if _intro_primary:
        play_intro_section = f"""
        <div style="text-align: center; margin: 30px 0;">
            <a href="{_intro_primary}" target="_blank" style="background: linear-gradient(135deg, #ff4757 0%, #ff3742 100%); border: none; border-radius: 50px; padding: 15px 30px; color: white; font-size: 18px; font-weight: 600; cursor: pointer; transition: all 0.3s ease; display: inline-flex; align-items: center; gap: 12px; box-shadow: 0 4px 15px rgba(255, 71, 87, 0.3); text-decoration: none;">
                ▶️ Reproducir Video de Coincidencia
            </a>
        </div>
        """
    if video_url_bunny and video_url and video_url_bunny != video_url:
        play_intro_section += f"""
        <div style="text-align: center; margin: 12px 0 30px 0;">
            <a href="{video_url_bunny}" target="_blank" style="background: linear-gradient(135deg, #ff9f43 0%, #ee5a24 100%); border: none; border-radius: 50px; padding: 12px 28px; color: white; font-size: 16px; font-weight: 600; cursor: pointer; text-decoration: none; display: inline-flex; align-items: center; gap: 10px; box-shadow: 0 4px 12px rgba(238, 90, 36, 0.35);">
                ▶️ Reproducir vía Bunny CDN
            </a>
        </div>
        """
    if v_r2_raw and v_r2_raw not in ((video_url or "").strip(), (video_url_bunny or "").strip()):
        play_intro_section += f"""
        <div style="text-align: center; margin: 12px 0 30px 0;">
            <a href="{vr2_esc}" target="_blank" style="background: linear-gradient(135deg, #f57c00 0%, #e65100 100%); border: none; border-radius: 50px; padding: 12px 28px; color: white; font-size: 16px; font-weight: 600; cursor: pointer; text-decoration: none; display: inline-flex; align-items: center; gap: 10px; box-shadow: 0 4px 12px rgba(230, 81, 0, 0.35);">
                ▶️ Reproducir copia Cloudflare R2
            </a>
        </div>
        """
    ...
    _badge_parts = []
    if video_url:
        _badge_parts.append('🌐 <span>Cloudinary CDN</span>')
    if video_url_bunny:
        _badge_parts.append('🐰 <span>Bunny CDN</span>')
    if v_r2_raw:
        _badge_parts.append('☁️ <span>Cloudflare R2</span>')
    _badges_join = '<span>·</span>'.join(_badge_parts)

    # Sección de video simplificada (reproductor usa Cloudinary si existe; si no, Bunny o R2)
    video_section = ""
    _src_player = video_url or video_url_bunny or v_r2_raw
    if _src_player:
        video_section = f"""
        <div style="margin: 30px 0; text-align: center;">
            ...
            <div style="position: relative; max-width: 650px; margin: 0 auto; background: #000; border-radius: 12px; overflow: hidden; box-shadow: 0 8px 25px rgba(0,0,0,0.3);">
                <video id="main-video" style="width: 100%; height: 400px; object-fit: cover; cursor: pointer;" controls preload="metadata">
                    <source src="{_src_player}" type="video/mp4">
                ...
                <div style="font-weight: 600; color: #495057; margin-bottom: 4px; font-size: 16px;">{nombre_video_display_esc}</div>
                {bloque_archivo_tecnico}
                <div style="color: #6c757d; font-size: 14px; display: flex; align-items: center; justify-content: center; gap: 8px; flex-wrap: wrap;">
                    {_badges_join}
                </div>
            </div>
        </div>
        """
    if v_r2_raw and _src_player and v_r2_raw != _src_player:
        video_section += f"""
        <div style="margin: 30px 0; text-align: center;">
            <h3 style="color: {primary_color}; margin: 0 0 20px 0; font-size: 18px;">
                ☁️ Copia en Cloudflare R2
            </h3>
            <div style="position: relative; max-width: 650px; margin: 0 auto; background: #000; border-radius: 12px; overflow: hidden; box-shadow: 0 8px 25px rgba(0,0,0,0.3);">
                <video style="width: 100%; height: 400px; object-fit: cover; cursor: pointer;" controls preload="metadata">
                    <source src="{vr2_esc}" type="video/mp4">
                    Tu navegador no soporta la reproducción de video.
                </video>
            </div>
        </div>
...
{transcripcion}
                ...
                <div style="display: flex; gap: 15px; justify-content: center; margin: 30px 0; flex-wrap: wrap;">
                    {f'<a href="{video_url}" target="_blank" style="padding: 15px 30px; border: none; border-radius: 30px; text-decoration: none; font-weight: 600; text-align: center; transition: all 0.3s ease; display: inline-flex; align-items: center; gap: 10px; cursor: pointer; font-size: 16px; box-shadow: 0 4px 15px rgba(0,0,0,0.1); background: linear-gradient(135deg, #007bff 0%, #0056b3 100%); color: white;">🔗 Ver en Nueva Pestaña (Cloudinary)</a>' if video_url else ''}
                    {f'<a href="{video_url_bunny}" target="_blank" style="padding: 15px 30px; border: none; border-radius: 30px; text-decoration: none; font-weight: 600; text-align: center; transition: all 0.3s ease; display: inline-flex; align-items: center; gap: 10px; cursor: pointer; font-size: 16px; box-shadow: 0 4px 15px rgba(0,0,0,0.1); background: linear-gradient(135deg, #e67e22 0%, #d35400 100%); color: white;">🔗 Ver en Bunny CDN</a>' if video_url_bunny and video_url_bunny != video_url else (f'<a href="{video_url_bunny}" target="_blank" style="padding: 15px 30px; border: none; border-radius: 30px; text-decoration: none; font-weight: 600; text-align: center; transition: all 0.3s ease; display: inline-flex; align-items: center; gap: 10px; cursor: pointer; font-size: 16px; box-shadow: 0 4px 15px rgba(0,0,0,0.1); background: linear-gradient(135deg, #007bff 0%, #0056b3 100%); color: white;">🔗 Ver en Nueva Pestaña</a>' if video_url_bunny and not video_url else '')}
                    {f'<a href="{vr2_esc}" target="_blank" style="padding: 15px 30px; border: none; border-radius: 30px; text-decoration: none; font-weight: 600; text-align: center; transition: all 0.3s ease; display: inline-flex; align-items: center; gap: 10px; cursor: pointer; font-size: 16px; box-shadow: 0 4px 15px rgba(0,0,0,0.1); background: linear-gradient(135deg, #ef6c00 0%, #e65100 100%); color: white;">🔗 Cloudflare R2</a>' if vr2_esc else ''}
...
def generar_html_resumen_diario(coincidencias, cliente_nombre, corte_label, fecha, color_cliente="#1E88E5"):
    ...
    for idx, coinc in enumerate(coincidencias, 1):
        ...
        resumen_limpio = coinc['resumen_ejecutivo'].replace('**', '').replace('*', '').replace('🤖 Análisis del segmento:', '').strip()
            ...
            <div style="display: flex; gap: 18px; align-items: flex-start; flex-wrap: wrap;">
                ...
                <div style="flex: 1; min-width: 250px;">
                    ...
                    <div style="font-size: 13px; color: #666; margin-bottom: 10px;">
                        📺 {coinc['medio'] if coinc['medio'] else coinc['nombre_archivo']}
                    </div>
    ...
    terminos_badges = " ".join(
        f'<span style="background: {color_cliente}; color: white; padding: 4px 12px; border-radius: 20px; font-size: 12px; font-weight: 600;">{t}</span>'
        for t in terminos_unicos
    )
    
    # Medios lista
    medios_lista = "<br>".join(f"📺 {m}" for m in medios_unicos) if medios_unicos else "Sin datos de medio"
...
def enviar_resumen_diario_clientes(corte="mañana"):
    ...
    for cliente_id, grupo in grupos.items():
        ...
        correos = brevo_config.get('correos_destinatarios', [])
        if not correos:
        ...
        try:
            ...
            for c in coincs_cliente:
                text_lines.append(f"---")
                text_lines.append(f"Término: {c['termino']} | {c['hora']}")
                text_lines.append(f"Medio: {c['medio']}")
                text_lines.append(f"Resumen: {c['resumen_ejecutivo'][:300]}")
                if c['video_url']:
                    text_lines.append(f"Video: {c['video_url']}")
                text_lines.append("")
...
def enviar_correo_brevo(termino_encontrado, resumen_completo, nombre_video, video_path=None, info_medio="", terminos_detectados=[], video_url_gdrive=None):
    ...
    try:
        ...
        if video_path and os.path.exists(video_path):
            bcfg = cargar_bunny_config()
            if bcfg.get('enabled'):
                try:
                    video_url_bunny, bunny_msg = subir_video_bunny(video_path, termino_encontrado, "")
                    if video_url_bunny:
                        log_info(f"✅ Video subido a Bunny (correo global): {video_url_bunny}", func_name)
                    else:
                        log_warning(f"❌ Bunny (correo global): {bunny_msg}", func_name)
                except Exception as e:
                    log_warning(f"❌ Excepción subiendo video a Bunny: {e}", func_name)
        ...
        _plano_arch_vid = archivo_broadcast_lineas_correo_plano(nombre_video)
        _plano_arch_vid_ind = "\n  ".join(_plano_arch_vid.split("\n"))
...
def enviar_video_telegram(video_path, caption="", chat_id=None, bot_token=None, usar_cloudinary=True):
    ...
    try:
        ...
        video_url_send = None
        response = None
        use_c = usar_cloudinary and config.get('use_cloudinary', True)
        if use_c:
            video_url_cloudinary, upload_msg = subir_video_cloudinary(video_path)
            u_bunny, msg_bunny = (None, "")
            if cargar_bunny_config().get('enabled'):
                u_bunny, msg_bunny = subir_video_bunny(video_path, "", "")
            # Telegram por URL: mismo comportamiento que antes → Cloudinary si existe; si no, Bunny
            video_url_send = video_url_cloudinary if video_url_cloudinary else u_bunny
            if not video_url_send:
                upload_msg = upload_msg or msg_bunny or "Cloudinary y Bunny no disponibles"
            
            if video_url_send:
                data = {
                    'chat_id': chat_id,
                    'video': video_url_send,
            ...
            elif file_size_mb <= TELEGRAM_SENDVIDEO_DIRECT_MAX_MB:
                log_info(
                    f"Cloudinary/Bunny URL falló ({upload_msg}); reintentando envío directo sendVideo ({file_size_mb:.1f}MB)",
                    "enviar_video_telegram"
                )
                cap = caption[:TELEGRAM_CAPTION_MAX] if len(caption) > TELEGRAM_CAPTION_MAX else caption
                with open(video_path, 'rb') as video_file:
                    files = {'video': video_file}
                    data = {
                        'chat_id': chat_id,
                        'caption': cap.replace("🌐 *Vía:* Cloudinary - 5.6MB", f"📹 Envío directo (fallback Cloudinary) - {file_size_mb:.1f}MB")
                    }
                    timeout = max(60, int(file_size_mb * 2))
                    response = requests.post(url, files=files, data=data, timeout=timeout)
            else:
                return False, f"Error subiendo video: {upload_msg}; {file_size_mb:.1f}MB > {TELEGRAM_SENDVIDEO_DIRECT_MAX_MB}MB sin URL", None
        ...
        if response is None:
            return False, "No se pudo enviar el video (sin respuesta HTTP)", None
        if response.status_code == 200:
            return True, f"Video enviado a Telegram ({file_size_mb:.1f}MB)", video_url_send or video_url_cloudinary
        else:
...
def enviar_video_telegram_inteligente(video_path, caption, chat_id=None, bot_token=None, parse_mode='Markdown', cloudinary_url=None):
    ...
    1. Si hay URL CDN previa (p. ej. Cloudinary) -> sendVideo con URL (hasta 2GB)
    2. Si video < límite -> envío directo
    3. Si video grande -> subir a Cloudinary y Bunny en paralelo; URL para Telegram = Cloudinary si hay, si no Bunny
    """
    ...
    try:
        ...
        # MÉTODO 1: Si ya tenemos URL CDN, usarla directamente
        if cloudinary_url:
            log_info(f"Usando URL CDN existente: {cloudinary_url}", func_name)
            return enviar_video_telegram_url(cloudinary_url, caption, chat_id, bot_token, parse_mode)
        
        # MÉTODO 2: Si video es pequeño (< límite), envío directo
        max_mb = config.get('max_file_size_mb', 8)
        if file_size_mb <= max_mb:
        ...
        # MÉTODO 3: Video grande — Cloudinary + Bunny en paralelo; enviar por URL como antes (Cloudinary primero)
        log_info(f"Video grande ({file_size_mb:.1f}MB) - subiendo a Cloudinary y Bunny si aplica", func_name)
        
        u_c, m_c = subir_video_cloudinary(video_path, "Video para Telegram")
        u_b, m_b = (None, "")
        if cargar_bunny_config().get('enabled'):
            u_b, m_b = subir_video_bunny(video_path, "Video para Telegram", "")
        send_u = u_c or u_b
        if send_u:
            log_info(f"Video listo en CDN para Telegram: {send_u}", func_name)
            return enviar_video_telegram_url(send_u, caption, chat_id, bot_token, parse_mode)
        return False, f"Error subida CDN: {m_c or m_b}", None
...
def probar_conexiones_cliente(cliente):
    ...
    bunny_res = _bunny_config_resuelta(cliente.get('bunny') or {})
    if bunny_res.get('enabled') and bunny_res.get('storage_zone') and bunny_res.get('access_key'):
        try:
            archivo_prueba = os.path.join(CARPETA_PROCESADOS, f"test_bunny_cliente_{cliente_nombre[:20].replace(' ', '_')}.txt")
            with open(archivo_prueba, 'w', encoding='utf-8') as f:
                f.write("ping")
            ok_b, msg_b = subir_video_bunny(archivo_prueba, "test", "", bunny_cfg=cliente.get('bunny') or {})
            if os.path.exists(archivo_prueba):
                try:
                    os.remove(archivo_prueba)
                except OSError:
                    pass
            resultados['bunny'] = (bool(ok_b), (msg_b[:80] if ok_b else msg_b[:80]))
        except Exception as e:
            resultados['bunny'] = (False, str(e)[:50])
    else:
        resultados['bunny'] = (None, "No configurado")
...
st.markdown("### 📤 **FLUJO DE ENVÍO DE COINCIDENCIAS (MULTI-CLIENTE):**")
st.markdown("""
**🆕 SISTEMA MULTI-CLIENTE:** Cada término está asociado a un cliente específico con sus propias credenciales.

**Cuando se detecta una coincidencia:**

1. **🔍 Detección del cliente:** El sistema identifica automáticamente a qué cliente pertenece el término
2. **📤 Envío a destinos del cliente:** Se envía a TODOS los destinos configurados para ese cliente:
   - 📱 **Telegram** → Mensaje + Video (credenciales del cliente)
   - 🌐 **Webhook** → URLs configuradas para el cliente
   - 📧 **Brevo** → Correo HTML a destinatarios del cliente
   - ☁️ **Google Drive** → Carpeta del cliente
   - 🗄️ **Supabase** → Base de datos del cliente

**📋 Gestión de Clientes:**
- Ir a la pestaña **"👥 Gestión de Clientes"** para:
  - ➕ Crear nuevos clientes
  - ⚙️ Configurar credenciales de cada servicio
  - 🏷️ Asociar términos a clientes
...
with st.sidebar:
    ...
    for _sc in obtener_clientes_activos():
        ...
        st.toggle(
            f"🔎 {_sc.get('nombre', _scid)}",
            key=_skey,
            help=f"Cliente `{_scid}`: desactivado = no se analizan sus términos.",
    ...
    loop_activo = st.checkbox(
        ...
        help="Al terminar de procesar, re-escanea automáticamente buscando videos nuevos"
    )
    ...
    st.session_state.mostrar_solo_actual_relevantes = st.checkbox(
        "Mostrar solo video actual + relevantes",
        value=st.session_state.get('mostrar_solo_actual_relevantes', True),
        help="Oculta detalle de videos sin coincidencias y mantiene en pantalla solo el actual, coincidencias y menciones tangenciales."
    )
    ...
    st.session_state.intro_clip_enabled = st.checkbox(
        ...
        value=st.session_state.get("intro_clip_enabled", INTRO_CLIP_ENABLED),
        help="Antepone un bumper con el logo del canal (carpeta logos/) y audio Mistral TTS (voz MISTRAL_TTS_VOICE_ID, p. ej. «mi voz»).",
    )
    ...
    if telegram_enabled:
        ...
        st.markdown("#### 🐰 Bunny.net Storage (paralelo a Cloudinary)")
        bunny_ui = cargar_bunny_config()
        bunny_enabled_ui = st.checkbox("Activar subida a Bunny Storage", value=bool(bunny_ui.get('enabled')))
        b_zone = st.text_input("Storage zone (nombre de zona):", value=bunny_ui.get('storage_zone', ''), placeholder="fgjmultimedios")
        b_key = st.text_input("Access key (password API):", value=bunny_ui.get('access_key', ''), type="password")
        b_folder = st.text_input("Carpeta en storage:", value=bunny_ui.get('folder', 'video_analyzer_clips'))
        b_cdn = st.text_input(
            "CDN base URL (hostname del Pull Zone):",
            value=bunny_ui.get('cdn_base_url', ''),
            placeholder="https://tu-pullzone.b-cdn.net",
            help="En panel.bunny.net: Pull Zones → el hostname debe estar enlazado a esta Storage Zone. Si lo dejas vacío se usa https://<storage_zone>.b-cdn.net (solo si ese Pull Zone existe y está activo).",
        )
        if st.button("💾 Guardar Bunny"):
            nueva_b = bunny_ui.copy()
            nueva_b.update({
                'enabled': bunny_enabled_ui,
                'storage_zone': b_zone.strip(),
                'access_key': b_key.strip(),
                'folder': b_folder.strip() or 'video_analyzer_clips',
                'cdn_base_url': b_cdn.strip()
            })
            if guardar_bunny_config(nueva_b):
                st.success("✅ Configuración Bunny guardada")
        st.warning(
            "Si al abrir el enlace del video ves **Domain suspended or not configured**: "
            "en Bunny crea o reactiva un **Pull Zone**, enlázalo a tu **Storage Zone** y copia aquí el hostname exacto (CDN base URL). "
            "El nombre de la storage zone no siempre coincide con el hostname del Pull Zone."
        )
...
with col2:
    ...
    if st.button("🎬 Probar Video", help="Probar envío inteligente de video a Telegram"):
        ...
        if videos_disponibles:
            ...
            ruta_video = os.path.join(CARPETA_PROCESADOS, video_prueba)
...
with st.expander("👁️ Preview: Coincidencias de hoy"):
    ...
    if coincs_hoy:
        ...
        for cid, grupo in grupos_hoy.items():
            nombre = grupo['cliente'].get('nombre', cid)
...
def cargar_archivos_fallidos():
    """Carga la lista de archivos fallidos desde fallidos.txt (en videos procesados)."""
    try:
        ruta_fallidos = FALLIDOS_TXT if os.path.exists(FALLIDOS_TXT) else os.path.join(str(_DIR_SCRIPT), "fallidos.txt")
        if os.path.exists(ruta_fallidos):
            with open(ruta_fallidos, "r", encoding="utf-8") as f:
...
def guardar_archivo_fallido(nombre_archivo, error_mensaje="", archivo_path=None):
    ...
    try:
        ...
        # 2. CREAR CARPETA archivos_fallidos/ dentro de videos procesados si no existe
        carpeta_fallidos = CARPETA_ARCHIVOS_FALLIDOS
...
"""
        ...
        # 5. REGISTRAR EN fallidos.txt (en videos procesados)
        with open(FALLIDOS_TXT, "a", encoding="utf-8") as f:
...
def es_archivo_fallido(nombre_archivo):
    """True si el archivo está en fallidos.txt (ruta relativa o solo nombre de archivo)."""
    try:
        archivos_fallidos = cargar_archivos_fallidos()
        if not nombre_archivo or not archivos_fallidos:
            return False
        na = nombre_archivo.replace("\\", "/").strip()
        if na in archivos_fallidos:
            return True
        base = os.path.basename(na)
        for af in archivos_fallidos:
            afn = (af or "").replace("\\", "/").strip()
            if not afn:
                continue
            if afn == na or os.path.basename(afn) == base:
                return True
        return False
    except Exception as e:
...
def limpiar_archivos_fallidos():
    ...
    try:
        eliminado = False
        for ruta in (FALLIDOS_TXT, os.path.join(str(_DIR_SCRIPT), "fallidos.txt")):
...
_ffprobe_exe_resuelto = None
_ffprobe_no_encontrado_aviso = False
_ffmpeg_exe_resuelto = None
_ffmpeg_no_encontrado_aviso = False


def _subprocess_no_window_kw():
    """En Windows evita ventana de consola al lanzar ffmpeg/ffprobe."""
    return {"creationflags": subprocess.CREATE_NO_WINDOW} if sys.platform == "win32" else {}


def _ruta_ejecutable_ffprobe():
    """
    Resuelve ffprobe: FFPROBE_PATH (.env), FFMPEG_BIN, PATH (incl. bin anteponido al inicio).
    """
    global _ffprobe_exe_resuelto
    if _ffprobe_exe_resuelto is not None:
        return _ffprobe_exe_resuelto
    directo = os.getenv("FFPROBE_PATH", "").strip()
    if directo and os.path.isfile(directo):
        _ffprobe_exe_resuelto = directo
        return _ffprobe_exe_resuelto
    fb = os.getenv("FFMPEG_BIN", "").strip()
    if fb:
        fb = os.path.normpath(fb.replace("/", os.sep))
        for name in ("ffprobe.exe", "ffprobe"):
            p_join = os.path.join(fb, name)
            if os.path.isfile(p_join):
                _ffprobe_exe_resuelto = p_join
                return _ffprobe_exe_resuelto
    for nombre in ("ffprobe", "ffprobe.exe"):
        w = shutil.which(nombre)
        if w:
            _ffprobe_exe_resuelto = w
            return _ffprobe_exe_resuelto
    _ffprobe_exe_resuelto = ""
    return ""


def _ruta_ejecutable_ffmpeg():
    """Resuelve ffmpeg: FFMPEG_PATH, FFMPEG_BIN, PATH."""
    global _ffmpeg_exe_resuelto
    if _ffmpeg_exe_resuelto is not None:
        return _ffmpeg_exe_resuelto
    directo = os.getenv("FFMPEG_PATH", "").strip()
    if directo and os.path.isfile(directo):
        _ffmpeg_exe_resuelto = directo
        return _ffmpeg_exe_resuelto
    fb = os.getenv("FFMPEG_BIN", "").strip()
    if fb:
        fb = os.path.normpath(fb.replace("/", os.sep))
        for name in ("ffmpeg.exe", "ffmpeg"):
            p_join = os.path.join(fb, name)
            if os.path.isfile(p_join):
                _ffmpeg_exe_resuelto = p_join
                return _ffmpeg_exe_resuelto
    for nombre in ("ffmpeg", "ffmpeg.exe"):
        w = shutil.which(nombre)
        if w:
            _ffmpeg_exe_resuelto = w
            return _ffmpeg_exe_resuelto
    _ffmpeg_exe_resuelto = ""
    return ""


def obtener_duracion(video_path):
    """
    Devuelve duración en segundos o None si ffprobe falla (no inventar 1.0:
    eso hacía que el video se tratara como muy corto y se saltara el análisis).
    """
    global _ffprobe_no_encontrado_aviso
    try:
        exe = _ruta_ejecutable_ffprobe()
        if not exe:
            if not _ffprobe_no_encontrado_aviso:
                _ffprobe_no_encontrado_aviso = True
                log_warning(
                    "ffprobe no está en PATH ni en FFPROBE_PATH. "
                    "Añade la carpeta bin de FFmpeg a PATH o define FFMPEG_BIN / FFPROBE_PATH en .env",
                    "obtener_duracion",
                )
            return None
        ruta = os.path.abspath(os.path.normpath(video_path))
        if not os.path.isfile(ruta):
            return None
        # -i antes del archivo es la forma habitual; evita ambigüedad con rutas raras en Windows.
        res = subprocess.run(
            [
                exe,
                "-v",
        ...
        if res.returncode != 0:
            ...
            if err:
                log_warning(
                    f"ffprobe rc={res.returncode} ({os.path.basename(ruta)}): {err[:400]}",
                    "obtener_duracion",
                )
            return None
    ...
    try:
        ...
        try:
            with st.spinner("🎵 Convirtiendo audio a MP3 (64kbps, 16kHz, mono)..."):
                _ff = _ruta_ejecutable_ffmpeg()
                if not _ff:
                    raise FileNotFoundError(
                        "ffmpeg no encontrado. Configura FFMPEG_BIN o FFMPEG_PATH o instala FFmpeg."
                    )
                subprocess.run(
                    [
                        _ff,
...
def _normalizar_termino_sin_acentos_va(s):
    """Minúsculas y sin tildes para comparar términos de monitoreo."""
    if not s:
        return ""
...
def termino_requiere_educacion_escolar_rd_exclusiva(termino):
    """
    Términos MINERD donde «educación» debe interpretarse solo como sistema escolar
    formal en República Dominicana (no vial/tránsito, moral, ciudadana/cívica).
    """
    t = re.sub(r"\s+", " ", _normalizar_termino_sin_acentos_va(termino)).strip()
    if not t:
        return False
    if t == "educacion":
        return True
    if t in ("ministro de educacion", "ministerio de educacion", "secretario de estado de educacion"):
        return True
    return False


def _suffix_prompt_educacion_escolar_rd():
    """Bloque de instrucciones para modelos al evaluar coincidencias de educación escolar RD."""
    return """
═══════════════════════════════════════════════════════════════════
🏫 REGLA OBLIGATORIA — EDUCACIÓN ESCOLAR DOMINICANA (MINERD / sistema formal en RD)
═══════════════════════════════════════════════════════════════════

Este monitoreo solo debe dar por válido el tema cuando se trata de **educación escolar formal en República Dominicana**: escuelas públicas/privadas, liceos, politécnicos/tecnológicos, estudiantes y docentes en ese nivel, MINERD, matrícula, infraestructura escolar, calendario o presupuesto escolar, desayuno/comedor escolar (INABIE), pruebas nacionales **en contexto escolar dominicano**, reformas o política del sistema educativo **del país**, texto escolar, aulas, director/regional/distrito escolar, etc.

❌ DEBES RECHAZAR (rechazar: true / es_relevante: false / relevancia: "baja") cuando el eje del contenido sea cualquiera de estos usos de «educación» (u homónimos), aunque se mencione el ministerio o el ministro:

• Educación **vial**, de **tránsito** o seguridad en la vía (peatones, multas, cultura vial, DIGESETT en ese rol, etc.)
• Educación **moral**, en **valores**, convivencia o desarrollo socioemocional **sin** articulación clara al sistema escolar formal RD
• Educación **ciudadana**, **cívica**, formación ciudadana, democrática participativa, etc., cuando NO sea claramente política pública **escolar** en RD (currículo, escuela, MINERD)

También rechaza clichés corporativos/deportivos («nos educamos», «educación al cliente»), educación financiera/del consumidor, sexual o de salud **como tema aislado**, o modelos solo de otros países **sin** vínculo explícito con escuelas o política educativa escolar en República Dominicana.

✅ Solo aprueba si queda claro que el desarrollo es sobre **la educación escolar dominicana** según el párrafo inicial.

Si hay duda razonable entre «escolar RD» y «vial/moral/ciudadana», RECHAZA.
═══════════════════════════════════════════════════════════════════
"""

# Último motivo de rechazo del modelo (Gemini / GPT-4o) cuando rechaza segmento por tangencial — lo lee el flujo de tangenciales/correo.
_SEG_RECHAZO_SEGMENTO_IA = ""


def _guardar_razon_rechazo_segmento_ia(texto: str) -> None:
    global _SEG_RECHAZO_SEGMENTO_IA
    _SEG_RECHAZO_SEGMENTO_IA = (texto or "").strip()


def _tomar_y_limpiar_razon_rechazo_segmento_ia() -> str:
    global _SEG_RECHAZO_SEGMENTO_IA
    t = (_SEG_RECHAZO_SEGMENTO_IA or "").strip()
    _SEG_RECHAZO_SEGMENTO_IA = ""
    return t
...
{f'{_suffix_prompt_educacion_escolar_rd()}IMPORTANTE: Respuesta «NO_RELEVANTE» obligatoria cuando el uso sea educación vial/tránsito, moral o ciudadana/cívica sin foco en escolar formal RD.' if termino_requiere_educacion_escolar_rd_exclusiva(termino_encontrado) else ''}
...
{f'{_suffix_prompt_educacion_escolar_rd()}IMPORTANTE: Si aplica esta regla, cualquier foco en educación vial/tránsito, moral o ciudadana/cívica (sin articulación escolar formal RD) debe llevar a es_relevante: false y relevancia: «baja».' if termino_requiere_educacion_escolar_rd_exclusiva(termino_encontrado) else ''}
...
def crear_archivo_consolidado(video_path, nombre_video, coincidencias, transcripcion_completa, resumen, terminos_buscados, clips_info=None, video_url=None, video_url_bunny=None, video_url_r2=None):
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
...
PROGRAMACION_TV_JSON = str(_DIR_SCRIPT / "programacion_tv.json")
_programacion_tv_cache = None

CANAL_ALIASES = {
    "TELEANTILLAS": "Teleantillas",
    "TELE ANTILLAS": "Teleantillas",
    "TELE ANTILLA": "Teleantillas",
    "TELEMICRO": "Telemicro",
    "TELEMICRO CANAL 5": "Telemicro",
    "ANTENA7": "Antena Latina",
    "ANTENA 7": "Antena Latina",
    "ANTENA LATINA": "Antena Latina",
    "COLORVISION": "Color Vision",
    "COLOR VISION": "Color Vision",
    "TELESISTEMA": "Telesistema",
    "TELESISTEMA 11": "Telesistema",
    "TELECENTRO": "Telecentro",
    "DIGITAL15": "Digital 15",
    "DIGITAL 15": "Digital 15",
    "CDN": "CDN",
    "CDN37": "CDN",
    "CDN 37": "CDN",
    "CDN EN VIVO": "CDN",
    ...
    "TELE UNIVERSO": "Teleuniverso",
    "ACENTO": "Acento",
    "ACENTO TV": "Acento",
    "CINEVISION": "Cinevision",
    "PANORAMA": "Panorama",
    "TELE SISEMA": "Telesistema",
    "TELEMEDIOS": "Telemedios",
    "CANAL SEIS": "Canal Seis",
    "EL SEIS": "Canal Seis",
    "MICROVISION": "Microvision",
    "RNN": "RNN",
    "FGJ MEDIOS": "FGJ Medios",
}


def cargar_programacion_tv():
    global _programacion_tv_cache
    if _programacion_tv_cache is not None:
        return _programacion_tv_cache
    try:
        if os.path.isfile(PROGRAMACION_TV_JSON):
            with open(PROGRAMACION_TV_JSON, "r", encoding="utf-8") as f:
                data = json.load(f)
            if isinstance(data, dict):
                _programacion_tv_cache = data
                return data
    except Exception as e:
        log_warning(f"No se pudo cargar programacion_tv.json: {e}", "cargar_programacion_tv")
    _programacion_tv_cache = {}
    return _programacion_tv_cache


def _config_canal_programacion(canal):
    prog = cargar_programacion_tv()
    if not canal or not prog:
        return None
    if canal in prog:
        return prog[canal]
    canal_upper = canal.upper()
    for k, v in prog.items():
        if k.upper() == canal_upper:
            return v
    return None


def normalizar_canal_programacion(canal_raw):
    if not canal_raw:
        return ""
    s = str(canal_raw).replace("_", " ").strip()
    s = re.sub(r"\s+", " ", s)
    key = s.upper()
    key_compact = key.replace(" ", "")
    if key in CANAL_ALIASES:
        return CANAL_ALIASES[key]
    if key_compact in CANAL_ALIASES:
        return CANAL_ALIASES[key_compact]
    prog = cargar_programacion_tv()
    for k in prog:
        ku = k.upper()
        if ku == key or ku.replace(" ", "") == key_compact:
            return k
    return s.title() if s else str(canal_raw)


def metadata_emision_archivo(nombre_o_ruta):
    """Extrae canal y datetime del nombre del archivo (p. ej. Acento_TV_2026-07-19_08-40-59.mp4)."""
    base = os.path.basename(str(nombre_o_ruta or ""))
    if base.lower().endswith(".mp4"):
        base = base[:-4]
    dt = None
    canal_raw = ""
    m = re.match(
        r"^(?P<canal>.+?)_(?P<fecha>\d{4}-\d{2}-\d{2})_(?P<hora>\d{2}-\d{2}-\d{2})$",
        base,
        re.I,
    )
    if m:
        canal_raw = m.group("canal")
        try:
            dt = datetime.strptime(
                f"{m.group('fecha')} {m.group('hora').replace('-', ':')}",
                "%Y-%m-%d %H:%M:%S",
            )
        except ValueError:
            dt = None
    if not dt:
        m2 = re.match(
            r"^(?P<canal>.+?)_(?P<fecha>\d{8})_(?P<hora>\d{6})$",
            base,
            re.I,
        )
        if m2:
            canal_raw = m2.group("canal")
            try:
                dt = datetime.strptime(
                    f"{m2.group('fecha')} {m2.group('hora')}",
                    "%Y%m%d %H%M%S",
                )
            except ValueError:
                dt = None
    canal = normalizar_canal_programacion(canal_raw)
    return {
        "canal": canal,
        "canal_raw": canal_raw,
        "datetime": dt,
        "basename": base + ".mp4" if base else "",
    }


def _dia_en_rango_escaneo(dt, modo_dias):
    if not dt:
        return False
    wd = dt.weekday()
    modo = (modo_dias or "").lower().replace("-", "_")
    if modo in ("lunes_a_viernes", "lun_vie", "weekdays"):
        return wd < 5
    return True


def _hora_en_slot(dt, slot):
    if not dt or not slot:
        return False
    try:
        hi = datetime.strptime(slot.get("inicio", "00:00"), "%H:%M").time()
        hf = datetime.strptime(slot.get("fin", "23:59"), "%H:%M").time()
    except ValueError:
        return False
    t = dt.time()
    if hi <= hf:
...
def obtener_horarios_escaneo_canal(canal):
    cfg = _config_canal_programacion(canal) or {}
    slots = cfg.get("escaneo_solo_horarios") or []
    return slots if isinstance(slots, list) else []


def video_pasa_filtro_escaneo_horario(nombre_o_ruta):
    """
    Filtro de escaneo por canal/horario (p. ej. Teleantillas solo mañana).
    ...
    if not slots:
        if canal:
            return True, f"{canal} — sin horario de escaneo; se procesan todos los videos"
        return True, "canal sin horario de escaneo; se procesan todos los videos"
    canal_cfg = _config_canal_programacion(canal) or {}
    dt = meta.get("datetime")
    if not dt:
        return False, f"Video fuera de horario — {canal}: sin fecha/hora en el nombre (no se procesa)"
    modo_dias = canal_cfg.get("escaneo_solo_dias") or "lunes_a_viernes"
    if not _dia_en_rango_escaneo(dt, modo_dias):
        return False, f"Video fuera de horario — {canal} {dt.strftime('%H:%M')} (solo lun–vie)"
    for slot in slots:
        if _hora_en_slot(dt, slot):
            prog = slot.get("programa") or ""
            return True, f"horario OK — {slot.get('inicio')}-{slot.get('fin')} {prog}".strip()
    lista = ", ".join(
        f"{s.get('inicio', '?')}-{s.get('fin', '?')} {s.get('programa', '')}".strip()
        for s in slots
    )
    hora_txt = dt.strftime("%H:%M")
    return False, f"Video fuera de horario — {canal} {hora_txt} (permitidos: {lista})"


def _es_carpeta_ignorada_escaneo(rel_root):
    rel_root = (rel_root or ".").replace("\\", "/").lower()
    if rel_root in ("videoscheck", "archivos_fallidos", "procesados"):
        return True
    parts = [p for p in rel_root.split("/") if p and p != "."]
    if "videoscheck" in parts or "archivos_fallidos" in parts or "procesados" in parts:
        return True
    if rel_root.startswith("videoscheck/") or rel_root.startswith("archivos_fallidos/"):
        return True
    if rel_root.startswith("procesados/") or "/procesados/" in f"/{rel_root}/":
        return True
    return False


def es_error_archivo_en_uso(texto_o_exc):
    """True si el error indica que el archivo está bloqueado (p. ej. grabando)."""
    if isinstance(texto_o_exc, PermissionError):
        return True
    if isinstance(texto_o_exc, OSError):
        if getattr(texto_o_exc, "winerror", None) == 32:
            return True
        if getattr(texto_o_exc, "errno", None) in (13, 16):
            txt_os = str(texto_o_exc).lower()
            if any(x in txt_os for x in ("denied", "denegado", "being used", "en uso")):
                return True
    txt = str(texto_o_exc or "").lower()
    markers = (
...
def encolar_video_en_uso_fin_ciclo(archivo_path, rel, motivo, cola, mostrar_ui=True):
    """Anota un video en uso para reprocesarlo al final del ciclo (sigue grabando)."""
    if archivo_path and archivo_path not in cola:
        cola.append(archivo_path)
    msg = (
        f"🔴 **En grabación / en uso:** `{rel}` — se reintentará al **final del ciclo**. "
        f"({motivo})"
    )
    if mostrar_ui:
        st.warning(msg)
    log_info(f"Encolado fin de ciclo (en uso): {rel} — {motivo}", "encolar_video_en_uso_fin_ciclo")
    ui_list = st.session_state.setdefault("videos_en_uso_fin_ciclo_ui", [])
    ui_list.append(
        {
            "rel": rel,
            "motivo": motivo,
...
def mover_video_origen_a_carpeta_procesados(archivo_path, mostrar_ui=True):
    """
    Tras procesar un video, lo mueve a una subcarpeta `procesados` en el mismo directorio.
    Ej.: .../Teleantillas/video.mp4 → .../Teleantillas/procesados/video.mp4
    """
    func_name = "mover_video_origen_a_carpeta_procesados"
    try:
        if not archivo_path or not os.path.isfile(archivo_path):
            return False, "Archivo no existe o ya no está en origen"
        parent = os.path.dirname(os.path.abspath(archivo_path))
        if os.path.basename(parent).lower() == "procesados":
            return True, archivo_path
        dest_dir = os.path.join(parent, "procesados")
        os.makedirs(dest_dir, exist_ok=True)
        nombre = os.path.basename(archivo_path)
        dest = os.path.join(dest_dir, nombre)
        if os.path.abspath(archivo_path) == os.path.abspath(dest):
            return True, dest
        if os.path.exists(dest):
            stem, ext = os.path.splitext(nombre)
            ts = datetime.now().strftime("%Y%m%d_%H%M%S")
            dest = os.path.join(dest_dir, f"{stem}_{ts}{ext}")
        shutil.move(archivo_path, dest)
        rel_orig = os.path.relpath(archivo_path, CARPETA_VIDEOS)
        rel_dest = os.path.relpath(dest, CARPETA_VIDEOS)
        msg = f"📦 Movido a procesados/: `{rel_orig}` → `{rel_dest}`"
        if mostrar_ui:
            st.info(msg)
        log_info(msg, func_name)
        return True, dest
    except Exception as e:
        log_warning(f"No se pudo mover a procesados/: {archivo_path} — {e}", func_name)
        if mostrar_ui:
            st.warning(f"⚠️ No se pudo mover `{os.path.basename(str(archivo_path))}` a procesados/: {e}")
        return False, str(e)


def borrar_video_origen_permanente(archivo_path, motivo="", mostrar_ui=True, registrar=True):
    """
    Borra el video de origen de forma definitiva (os.remove — no pasa por la Papelera).
    También elimina audios auxiliares del mismo nombre (.wav/.mp3/.m4a/.aac).
    Solo permite borrar si el archivo está bajo CARPETA_VIDEOS.
    """
    func_name = "borrar_video_origen_permanente"
    try:
        if not archivo_path:
            return False, "Sin ruta"
        path = archivo_path
        if not os.path.isabs(str(path)):
            path = os.path.join(CARPETA_VIDEOS, str(path))
        path = os.path.abspath(path)
        raiz_permitida = os.path.abspath(CARPETA_VIDEOS)
        if not path.startswith(raiz_permitida + os.sep) and path != raiz_permitida:
            msg = f"Bloqueado: ruta fuera de videos procesados — {path}"
            log_warning(msg, func_name)
            if mostrar_ui:
                st.error(f"🛑 {msg}")
            return False, msg
        if not os.path.isfile(path):
            return False, "Archivo no encontrado"
        rel = os.path.relpath(path, CARPETA_VIDEOS)
        stem, _ext = os.path.splitext(path)
        borrados = []
        for candidato in [path] + [stem + ext for ext in (".wav", ".mp3", ".m4a", ".aac")]:
            if os.path.isfile(candidato):
                os.remove(candidato)
                borrados.append(os.path.basename(candidato))
        if registrar:
            registrar_archivo_procesado(
                os.path.basename(rel),
                [],
                f"Borrado permanente: {motivo or 'sin motivo'}",
                "mp4",
            )
        msg_ui = f"🗑️ **Eliminado permanentemente:** `{rel}`"
        if motivo:
            msg_ui += f" — {motivo}"
        if mostrar_ui:
            st.warning(msg_ui)
        ui_list = st.session_state.setdefault("videos_borrados_ui", [])
        ui_list.append({"rel": rel, "motivo": motivo or "sin motivo", "hora": datetime.now().isoformat(timespec="seconds")})
        log_info(f"Borrado permanente: {rel} — {motivo} — archivos: {borrados}", func_name)
        return True, rel
    except Exception as e:
        log_warning(f"Error borrando {archivo_path}: {e}", func_name)
        if mostrar_ui:
            st.warning(f"⚠️ No se pudo borrar `{os.path.basename(str(archivo_path))}`: {e}")
        return False, str(e)


def eliminar_video_fuera_horario(archivo_path, motivo_horario, mostrar_ui=True):
    """Borra un video fuera de horario y actualiza contadores de ciclo."""
    ok, _msg = borrar_video_origen_permanente(
        archivo_path,
        motivo=f"fuera de horario — {motivo_horario}",
        mostrar_ui=mostrar_ui,
        registrar=True,
    )
    if ok:
        st.session_state.ciclo_borrados_horario = int(st.session_state.get("ciclo_borrados_horario") or 0) + 1
    return ok
...
def _video_tuvo_tangenciales(rel, menciones_tangenciales_data):
    ...
    for t in menciones_tangenciales_data or []:
        ar = (t.get("archivo") or "").replace("\\", "/")
        if ar == rel_norm or os.path.basename(ar) == base:
            return True
    return False


def post_procesar_video_origen(archivo_path, rel, coincidencias_items, menciones_tangenciales_data, mostrar_ui=True):
    """Tras analizar: mover a procesados/ si hubo coincidencias o tangenciales; si no, borrar."""
    tiene_coinc = bool(coincidencias_items)
    tiene_tang = _video_tuvo_tangenciales(rel, menciones_tangenciales_data)
    if tiene_coinc or tiene_tang:
        ok, _msg = mover_video_origen_a_carpeta_procesados(archivo_path, mostrar_ui=mostrar_ui)
        if ok:
            st.session_state.ciclo_movidos_procesados = int(st.session_state.get("ciclo_movidos_procesados") or 0) + 1
        return ok
    ok, _msg = borrar_video_origen_permanente(
        archivo_path,
        motivo="sin coincidencias ni tangenciales",
        mostrar_ui=mostrar_ui,
        registrar=True,
    )
    if ok:
        st.session_state.ciclo_borrados_sin_match = int(st.session_state.get("ciclo_borrados_sin_match") or 0) + 1
    return ok


def _mostrar_resumen_borrados_ciclo():
    ...
    if n_u:
        st.warning(f"🔴 **{n_u}** video(s) detectados **en grabación / en uso** (reintentados al final del ciclo)")
...
def buscar_videos_nuevos_optimizado(procesados, func_name):
    ...
    try:
        ...
        for root, dirs, files in os.walk(CARPETA_VIDEOS):
            rel_root = os.path.relpath(root, CARPETA_VIDEOS).replace("\\", "/").lower()
            if _es_carpeta_ignorada_escaneo(rel_root):
                continue
            ...
            for file in files:
                if file.lower().endswith(".mp4"):
                    ...
                    try:
                        rel_path_early = os.path.relpath(path_full, CARPETA_VIDEOS)
                        nombre_solo_early = os.path.basename(rel_path_early)
                        salto_por_cache_valido = False
                        ...
                        if path_full in archivos_cache:
                            ...
                            if (file_stat.st_mtime == cache_info.get('mtime', 0) and 
                                ...
                                # Caché dice "procesado": debe coincidir con procesados.log / .txt
                                if cache_info.get('procesado', False):
                                    if rel_path_early in procesados or nombre_solo_early in procesados:
                                        salto_por_cache_valido = True
                                    else:
                                        log_warning(
                                            f"Caché desincronizada (procesado en cache, no en procesados): {rel_path_early} — reanalizando",
                                            func_name,
                                        )
                                        try:
                                ...
                                # Caché dice "muy pequeño": revalidar tamaño real
                                    ...
                                    if fs_now >= TAMANO_MINIMO_BYTES:
                                        log_info(
                                            f"Caché obsoleta (antes pequeño, ahora {fs_now / (1024*1024):.1f} MB): {rel_path_early}",
                                            func_name,
                                        )
                                        try:
                                            del archivos_cache[path_full]
                        ...
                        if es_archivo_fallido(rel_path) or es_archivo_fallido(nombre_archivo_solo):
                            file_stat = os.stat(path_full)
                            archivos_cache[path_full] = {
                                'mtime': file_stat.st_mtime,
                                'size': file_size,
                                'procesado': True,
                                'omitido_fallido': True
                            }
                            log_debug(f"Omitido (fallidos): {rel_path}", func_name)
                            continue

                        ok_horario, motivo_horario = video_pasa_filtro_escaneo_horario(path_full)
                        if not ok_horario:
                            eliminar_video_fuera_horario(path_full, motivo_horario, mostrar_ui=False)
                            file_stat = os.stat(path_full) if os.path.exists(path_full) else None
                            archivos_cache[path_full] = {
                                'mtime': file_stat.st_mtime if file_stat else 0,
                                'size': file_size,
                                'procesado': False,
                                'omitido_horario': True,
                                'motivo_omitido': motivo_horario,
                            }
                            log_info(f"⏭️ Video fuera de horario (eliminado): {rel_path} — {motivo_horario}", func_name)
                            continue
...
def buscar_videos_tradicional(procesados, func_name):
    ...
    for f in archivos:
        ...
        if rel_path not in procesados and nombre_archivo_solo not in procesados:
            ok_horario, motivo_horario = video_pasa_filtro_escaneo_horario(f)
            if not ok_horario:
                eliminar_video_fuera_horario(f, motivo_horario, mostrar_ui=False)
                continue
            nuevos.append(f)
        else:
...
def escanear_carpeta_completa():
    ...
    archivos_fuera_horario = 0
    archivos_fuera_horario_lista = []
    ...
    try:
                ...
                if file.lower().endswith(".mp4"):
                    ...
                    if rel_path in procesados or nombre_archivo_solo in procesados:
                        ...
                        if ok_horario:
                            ...
                            archivos_fuera_horario += 1
                            archivos_fuera_horario_lista.append((rel_path, motivo_horario))
                            eliminar_video_fuera_horario(path_full, motivo_horario, mostrar_ui=False)
            ...
            for file in files:
                if file.lower().endswith(".mp4"):
                    ...
                    try:
                        ...
                        nombre_archivo_solo = os.path.basename(rel_path)
                        if (
                            file_size >= TAMANO_MINIMO_BYTES
                            and rel_path not in procesados
                            and nombre_archivo_solo not in procesados
                            and not es_archivo_fallido(rel_path)
                            and not es_archivo_fallido(nombre_archivo_solo)
                        ):
                            ok_horario, _motivo_h = video_pasa_filtro_escaneo_horario(path_full)
                            if ok_horario:
        ...
        return {
            ...
            'archivos_fuera_horario': archivos_fuera_horario,
            'archivos_fuera_horario_lista': archivos_fuera_horario_lista,
...
def buscar_y_procesar_videos(duracion_clip=90, buffer_anterior=30):
    ...
    # Contadores de borrados/movimientos/en-uso de este ciclo (para resumen UI)
    st.session_state.ciclo_borrados_horario = 0
    st.session_state.ciclo_borrados_sin_match = 0
    st.session_state.ciclo_movidos_procesados = 0
    st.session_state.videos_borrados_ui = []
    st.session_state.videos_en_uso_fin_ciclo_ui = []
    ...
    terminos = filtrar_terminos_por_analisis_clientes(st.session_state.terminos_continuos)
    log_debug(f"Términos efectivos para análisis (tras filtro por entidad): {terminos}", func_name)
    
    if not terminos:
        log_info("No hay términos configurados para buscar", func_name)
        st.warning("⚠️ No hay términos para analizar (configura términos o activa entidades en el sidebar: 🏢 Análisis por entidad).")
    ...
    if estadisticas['archivos_nuevos'] > 0:
        ...
        # Mostrar lista de videos nuevos solo cuando no estamos en modo "solo relevantes"
        ...
        if st.session_state.get('loop_continuo', True):
            ...
            sesion_viva = _countdown_seguro(
                ...
                "💤 Sin videos nuevos. Re-escaneando en {i}s...",
                func_name
            )
            
            if not sesion_viva:
                log_warning("Sesión cerrada durante espera sin videos nuevos. Saliendo limpiamente.", func_name)
                return
    ...
    if not nuevos and estadisticas.get("archivos_nuevos", 0) > 0 and not forzar_escaneo:
        st.warning(
            "⚠️ **Caché de escaneo desincronizada:** el dashboard mostró videos nuevos pero la lista optimizada "
            "quedó vacía. Reintentando con **búsqueda completa** (sin depender del caché)."
        )
        log_warning(
            "nuevos vacíos pese a archivos_nuevos > 0; fallback buscar_videos_tradicional",
            func_name,
        )
        nuevos = buscar_videos_tradicional(procesados, func_name)
        log_info(f"Fallback tradicional tras desincronización: {len(nuevos)} archivos", func_name)
    
    if not nuevos:
        _mostrar_resumen_borrados_ciclo()
        n_bh = int(st.session_state.get("ciclo_borrados_horario") or 0)
        n_sm = int(st.session_state.get("ciclo_borrados_sin_match") or 0)
        if n_bh or n_sm:
            st.info(f"🧹 Ciclo sin videos nuevos: {n_bh} borrados por horario, {n_sm} borrados sin match.")
    ...
    def _ui_transient(level, message):
        """Muestra mensajes temporales por video cuando se usa modo solo relevantes."""
    ...
    for pasada in ("inicial", "reintento"):
        if pasada == "reintento":
            if not cola_en_uso:
                break
            st.info(
                f"🔁 **Reintento fin de ciclo:** {len(cola_en_uso)} video(s) que estaban "
                f"**en grabación / en uso** — probablemente ya terminaron de grabarse."
            )
            batch_videos = list(cola_en_uso)
            cola_en_uso = []
        else:
            batch_videos = nuevos

        for i, archivo_path in enumerate(batch_videos):
            rel = os.path.relpath(archivo_path, CARPETA_VIDEOS)
            total_pasada = len(batch_videos)
            etiqueta_pasada = " (reintento)" if pasada == "reintento" else ""
        
            # BLOQUE TRY-EXCEPT GENERAL PARA CADA ARCHIVO
            try:
                ok_horario_loop, motivo_horario_loop = video_pasa_filtro_escaneo_horario(archivo_path)
                if not ok_horario_loop:
                    eliminar_video_fuera_horario(archivo_path, motivo_horario_loop)
                    continue
                ...
                with current_video_container.container():
                    st.info(f"**{i+1}/{total_pasada}**{etiqueta_pasada} — {icono} Procesando: `{rel}`")
                    st.markdown(f"### {icono} Procesando {tipo_archivo} ({i+1}/{total_pasada}){etiqueta_pasada}: `{rel}`")
                    progress_bar = st.progress(0, text=f"Iniciando... ({i+1}/{total_pasada})")
                ...
                # === FILTRO opcional por duración (VIDEO_DURACION_MINIMA_SEGUNDOS; 0 = desactivado) ===
                if DURACION_MINIMA_SEGUNDOS_PROCESAR > 0 and dur_total is not None:
                    if dur_total < DURACION_MINIMA_SEGUNDOS_PROCESAR:
                        minutos = int(dur_total // 60)
                        segundos = int(dur_total % 60)
                        req_m = DURACION_MINIMA_SEGUNDOS_PROCESAR // 60
                        req_s = DURACION_MINIMA_SEGUNDOS_PROCESAR % 60
                        msg = (
                            f"⏭️ **SALTANDO video corto:** `{rel}` — Duración: {minutos}:{segundos:02d} "
                            f"(mínimo configurado: {req_m}:{req_s:02d}). "
                            f"Para analizar todos los videos: `VIDEO_DURACION_MINIMA_SEGUNDOS=0` en `.env`."
                        )
                        st.warning(msg)
                        log_info(
                            f"Video saltado por duración ({dur_total:.0f}s < {DURACION_MINIMA_SEGUNDOS_PROCESAR}s): {rel}",
                            func_name,
                        )
                        registrar_archivo_procesado(
                            os.path.basename(archivo_path),
                            [],
                            f"Video saltado: duración menor a {DURACION_MINIMA_SEGUNDOS_PROCESAR}s",
                            "mp4",
                        )
                        st.session_state.videos_procesados += 1
                        continue
                elif dur_total is None:
                    st.warning(
                        f"⚠️ No se pudo leer la duración con ffprobe: `{rel}`. "
                        "Se continúa el análisis. Si **ffmpeg** no está instalado, añade su carpeta `bin` al PATH o define "
                        "**FFMPEG_BIN** o **FFPROBE_PATH** en `.env`. Si el archivo es un segmento `.mp4` incompleto o dañado, "
                        "ffprobe también fallará (revisa el log `debug_*.log`)."
                    )
                    log_warning(f"duración desconocida (ffprobe), se analiza igual: {rel}", func_name)
                ...
                def procesar_audio_con_reintentos(archivo_path, audio_path, max_reintentos=3):
                    for intento in range(max_reintentos):
                        try:
                            ...
                            # Verificar acceso al archivo (detectar grabación en curso)
                            try:
                                with open(archivo_path, "rb") as _probe:
                                    _probe.read(4096)
                            except Exception as e_open:
                                if es_error_archivo_en_uso(e_open):
                                    return "en_uso"
                                if intento < max_reintentos - 1:
                                    time.sleep(5)
                                    continue
                            ...
                            _ffm = _ruta_ejecutable_ffmpeg()
                            if not _ffm:
                                _ui_transient(
                                    "error",
                                    "❌ ffmpeg no encontrado. Revisa FFMPEG_BIN / PATH en .env o instala FFmpeg.",
                                )
                                return False
                            cmd = [
                                _ffm,
                            ...
                            result = subprocess.run(
                                cmd, capture_output=True, text=True, **_subprocess_no_window_kw()
                            )
                    
                            if result.returncode != 0:
                                stderr_txt = f"{result.stderr or ''}\n{result.stdout or ''}"
                                if es_error_archivo_en_uso(stderr_txt):
                                    return "en_uso"
                                if intento < max_reintentos - 1:
                                    ...
                                    if result_alt.returncode != 0:
                                        stderr_alt = f"{result_alt.stderr or ''}\n{result_alt.stdout or ''}"
                                        if es_error_archivo_en_uso(stderr_alt):
                                            return "en_uso"
                                        _ui_transient("error", "❌ Error persistente incluso con parámetros alternativos")
                                        return False
                                    else:
                ...
                resultado_audio = procesar_audio_con_reintentos(archivo_path, audio_path)
                if resultado_audio == "en_uso":
                    encolar_video_en_uso_fin_ciclo(
                        archivo_path,
                        rel,
                        "archivo en uso — probablemente aún grabando",
                        cola_en_uso,
                    )
                    continue
                ...
                progress_bar.progress(40, text=f"🧠 Transcribiendo ({i+1}/{total_pasada})...")
                start = time.time()
                try:
                    ...
                    if es_error_archivo_en_uso(e):
                        encolar_video_en_uso_fin_ciclo(
                            archivo_path, rel, f"transcripción bloqueada: {e}", cola_en_uso
                        )
                        continue
                    ...
                    try:
                        ...
                        with open(transcripcion_completa_path, "w", encoding="utf-8") as f:
                            ...
                            if dur_total is not None:
                                f.write(
                                    f"⏱️ DURACIÓN TOTAL: {dur_total:.1f} segundos "
                                    f"({int(dur_total // 60)}:{int(dur_total % 60):02d})\n"
                                )
                            else:
                                f.write("⏱️ DURACIÓN TOTAL: no disponible (ffprobe no devolvió duración)\n")
                ...
                # No aplicar sufijos automáticos (-s/-es/-a/-o) a estos términos: p. ej. "miner"+"a"/"o"
                # coincide con "minera"/"minero" (minería) y genera falsos positivos frente a MINERD.
                TERMINOS_SIN_VARIACION_MORFOLOGICA = frozenset({'miner', 'minerd'})
                ...
                def buscar_termino_flexible(termino, texto):
                    ...
                    if termino_lower not in TERMINOS_SIN_VARIACION_MORFOLOGICA:
                        variaciones = [
                            termino + "s",  # plural
                ...
                for termino_item in terminos:
                    ...
                    if encontrado:
                        ...
                        try:
                            _pro = _ruta_ejecutable_ffprobe()
                            if not _pro:
                                raise RuntimeError("ffprobe no disponible")
                            _vid = os.path.abspath(os.path.normpath(archivo_path))
                            cmd_duracion = [
                                _pro,
                                "-v",
                                "quiet",
                        ...
                        _ffcut = _ruta_ejecutable_ffmpeg()
                        if not _ffcut:
                            raise RuntimeError("ffmpeg no encontrado; no se puede generar el clip.")
                        cmd = [
                            _ffcut,
                            "-y",
                            "-ss",
                        ...
                        try:
                            ...
                            try:
                                ...
                                if not es_relevante_gemini or relevancia_gemini == 'baja':
                                    if es_prioritario:
                                        ...
                                        razon_ia = f"Relevancia baja ({relevancia_gemini})"
                                        det = (contexto_gemini or idea_general_clip or "").strip()
                                        if det:
                                            razon_ia = f"{razon_ia}. {det}"
                                        ctx_drive = (
                                            f"Motivo (relevancia del segmento):\n{razon_ia}\n\n"
                                            f"Transcripción del segmento analizado:\n{(transcripcion_segmento or '')[:12000]}"
                                        )
                                        _destino_video_check, _txt_video_check, estado_videoscheck = guardar_video_y_minuto_coincidencia(
                                            video_path=archivo_path,
                                            minuto_detectado=momento_termino,
                                        ...
                                        if estado_videoscheck in ("copiado", "movido", "ya_existia"):
                                            ...
                                            log_warning(f"⚠️ No se pudo enviar a videoscheck (baja relevancia): {estado_videoscheck}", func_name)
                                        te_rel = (transcripcion_segmento or "").strip()[:12000]
                                        _item_tang = crear_item_tangencial(
                                                rel,
                                                termino,
                                                razon_ia,
                                                momento_termino,
                                                texto_evidencia=te_rel,
                                                url_drive_video=drive_url or "",
                                                transcripcion_extracto=(transcripcion_segmento or "")[:2500],
                                            )
                                        menciones_tangenciales_data.append(_item_tang)
                                        try:
                                            enriquecer_motivos_tangenciales_sesion(
                            ...
                            try:
                                _intro_on = st.session_state.get("intro_clip_enabled", INTRO_CLIP_ENABLED)
                                clip_path, _ok_intro, _msg_intro = prefijar_intro_coincidencia_si(
                                    clip_path,
                                    termino,
                                    rel,
                                    mistral_api_key=mistral_api_key,
                                    enabled=_intro_on,
                                    log_fn=lambda m, lvl="info": (
                                        log_warning(m, func_name) if lvl == "warning" else log_info(m, func_name)
                                    ),
                                    ui_info=st.info,
                                    ui_warning=st.warning,
                                )
                            ...
                            with st.spinner("🐰 Subiendo clip a Bunny Storage..."):
                                try:
                                    if cargar_bunny_config().get('enabled'):
                                        u_b, msg_b = subir_video_bunny(clip_path, termino, "")
                                        if u_b:
                                            url_bunny_clip = u_b
                                            st.success(f"🐰 ✅ **CLIP subido a Bunny**: {u_b}")
                                            log_info(f"Clip subido a Bunny: {u_b}", func_name)
                                        else:
                                            st.warning(f"⚠️ Error subiendo clip a Bunny: {msg_b}")
                                            log_warning(f"Error subiendo clip a Bunny: {msg_b}", func_name)
                                except Exception as e:
                            ...
                            if clip_path and os.path.exists(clip_path):
                                try:
                                    _pro_clip = _ruta_ejecutable_ffprobe()
                                    if _pro_clip and os.path.isfile(clip_path):
                                        _clip_abs = os.path.abspath(clip_path)
                                        _cmd_dur = [
                                            _pro_clip,
                                            "-v",
                                            "quiet",
                                        ...
                                        _res_dur = subprocess.run(
                                            _cmd_dur,
                                            capture_output=True,
                                            text=True,
                                            **_subprocess_no_window_kw(),
                                        )
                                        if _res_dur.returncode == 0 and (_res_dur.stdout or "").strip():
                                            ...
                                            if _dur_clip < MIN_DURACION_CLIP_ENVIO:
                                                ...
                                                st.warning(
                                                    ...
                                                    "Telegram y correo llevarán solo el **resumen de texto**, sin adjunto de clip."
                                                )
                                                log_info(
                                                    "Envío: solo texto (sin video) porque el clip es < "
                                                    f"{MIN_DURACION_CLIP_ENVIO}s — término seguirá notificándose por texto.",
                                                    func_name,
                                                )
                                                clip_path = None  # No enviar video pero sí el texto
                                                _clip_ok_para_enviar = False
                                except Exception as _e_dur:
                                    log_warning(f"⚠️ No se pudo verificar duración del clip: {_e_dur}", func_name)

                            exito_envio, mensaje_envio, url_cloudinary_clip, url_r2_clip = enviar_coincidencia_inmediata(
                                ...
                                video_url_bunny=url_bunny_clip,
                                transcripcion_segmento=transcripcion_segmento, # Segmento resaltado
                                video_url_r2_precargada=url_r2_clip,
                            )
        ...
        """)
                        ...
                        # Guardar video original para revisión posterior + minuto de coincidencia.
                        _destino_video_check, _txt_video_check, _estado_video_check = guardar_video_y_minuto_coincidencia(
                            archivo_path,
                            timestamp_actual,
                            termino_detectado=termino,
                            mover_video=False,
                            prefijo_carpeta="_coincidencia_"
                        )
                        if _estado_video_check not in ("copiado", "movido", "ya_existia"):
                            log_warning(
                                f"No se pudo registrar video en videoscheck ({rel}): {_estado_video_check}",
                                func_name
                            )
                ...
                if coincidencias_items:
                    try:
                        ...
                        try:
                            ...
                            video_url_bunny_consolidado = None
                            video_url_r2_consolidado = None
                            if coincidencias_items:
                                ...
                                video_url_r2_consolidado = coincidencias_items[0].get('url_r2')
                            if clips_info:
                                for _c in clips_info:
                                    if _c.get('url_bunny'):
                                        video_url_bunny_consolidado = _c.get('url_bunny')
                                        break

                            archivo_completo = crear_archivo_consolidado(
                                archivo_path, rel, coincidencias_items, transcripcion_mistral, resumen_archivo, terminos, clips_info, video_url_consolidado, video_url_bunny_consolidado, video_url_r2_consolidado
                            )
                        except TypeError:
                        ...
                        post_procesar_video_origen(
                            archivo_path, rel, coincidencias_items, menciones_tangenciales_data
                        )
                ...
                if es_error_archivo_en_uso(e):
                    encolar_video_en_uso_fin_ciclo(archivo_path, rel, error_mensaje, cola_en_uso)
                    continue
    ...
    if videos_procesados_data:
        ...
        for i, archivo_data in enumerate(videos_procesados_data, 1):
            ...
            with envio_actual_container.container():
                st.markdown("---")
                st.markdown(f"### 📤 Enviando {archivo_data.get('tipo_archivo', 'Archivo')} {i}/{len(videos_procesados_data)}: {icono_archivo} `{archivo_data['nombre_archivo']}`")
            ...
            if i < len(videos_procesados_data):
                ...
                with envio_actual_container.container(), st.spinner(f"⏳ FINAL: Esperando 60s antes del siguiente video ({i+1}/{len(videos_procesados_data)})..."):
                    time.sleep(60)
                with envio_actual_container.container():
    ...
    if menciones_tangenciales_data:
        try:
            ...
            log_exception("buscar_y_procesar_videos_post_ciclo_tang_deepseek", e)

    # Mostrar solo resultados relevantes de la sesion: coincidencias y tangenciales
    if videos_procesados_data or menciones_tangenciales_data:
        ...
        if menciones_tangenciales_data:
            ...
            for i, tangencial in enumerate(menciones_tangenciales_data, 1):
                ts_seg = tangencial.get('timestamp', 0) or 0
                mm = int(ts_seg // 60)
                ss = int(ts_seg % 60)
                _udv = (tangencial.get('url_drive_video') or tangencial.get('video_url_drive') or '').strip()
                partes = [
                    f"- **{i}.** `{tangencial.get('archivo', '')}` | término: **{tangencial.get('termino', '')}** | "
                    f"motivo: {motivo_display_tangencial(tangencial)} | tiempo: {mm}m{ss:02d}s"
                ]
                if _udv:
                    partes.append(f" | [Abrir en Drive]({_udv})")
    ...
    try:
        if not menciones_tangenciales_data:
            ...
            resumen_tang_sheets = enviar_tangenciales_a_google_sheets(menciones_tangenciales_data)
            if not resumen_tang_sheets:
                st.warning("No se obtuvo resultado del envío de tangenciales a Sheets.")
            for rs in resumen_tang_sheets:
                nombre_c = rs.get("cliente_nombre", "Cliente")
                ok_n = int(rs.get("ok", 0) or 0)
                fail_n = int(rs.get("fail", 0) or 0)
                if fail_n == 0:
                    st.success(f"✅ {nombre_c}: tangenciales enviadas a Sheets ({ok_n})")
                elif ok_n > 0:
                    st.warning(f"⚠️ {nombre_c}: tangenciales en Sheets ok={ok_n}, fallos={fail_n}")
                else:
                    st.warning(f"⏭️ {nombre_c}: no se pudo enviar tangenciales a Sheets (fallos={fail_n})")
                errores = rs.get("errores") or []
                if errores:
                    st.caption(f"{nombre_c} — último detalle: {errores[-1]}")
    except Exception as e_gs_t:
        st.warning(f"⚠️ Error Google Sheets (tangenciales): {e_gs_t}")
        log_warning(f"Error envío tangenciales a Sheets: {e_gs_t}", func_name)
    ...
    try:
        ...
        if md_sesion_ok:
            ...
            st.caption(f"Informe general actualizado: `{INFORME_GENERAL_PATH}`")
    ...
    st.caption(
        "Un correo **resumen** por entidad con todas las tangenciales del ciclo (además de los avisos inmediatos). "
        "Las coincidencias fuertes ya se enviaron al momento; esto es solo para tangenciales."
    )
    try:
        _tang_list = menciones_tangenciales_data or []
        n_tang = len(_tang_list)
        if n_tang == 0:
            st.info(
                "**No se envía** correo de cierre de tangenciales: en este ciclo **no hubo menciones tangenciales** "
                "(no se acumularon descartes por relevancia baja / mención sin desarrollo). "
                "Si hubo solo coincidencias con clip, es normal no ver este correo aquí."
            )
        else:
            res_correos_tang = enviar_correos_tangenciales_fin_ciclo(_tang_list)
            st.markdown(f"- Tangenciales acumuladas en el ciclo: **{n_tang}**")
            if not res_correos_tang:
                st.warning(
                    "Hay tangenciales en memoria pero **no se obtuvo resultado de envío por cliente** "
                    "(estado anómalo; revisa `logs/app_*.log` y la función enviar_correos_tangenciales_fin_ciclo)."
                )
            else:
                any_ok = False
                for nombre_c, ok_c, msg_c in res_correos_tang:
                    if ok_c:
                        any_ok = True
                        st.success(f"✅ **Correo de cierre enviado** — **{nombre_c}**: {msg_c}")
                    else:
                        st.warning(
                            f"⏭️ **Correo de cierre no enviado** — **{nombre_c}**: {msg_c}"
                        )
                if not any_ok:
                    st.info(
                        "**Ninguna entidad recibió** el correo de cierre. Motivos típicos por cliente en `clientes_config.json`: "
                        "**brevo.enabled** en false, falta **api_key** o **sender_email**, o lista vacía **correos_destinatarios**. "
                        "El texto de cada entidad arriba indica el motivo concreto."
                    )
    except Exception as e_mail_tang:
        st.warning(f"⚠️ **Error** al procesar correo tangenciales (cierre de ciclo): {e_mail_tang}")
    ...
    if loop_activo:
        st.info("♻️ **REINICIO SUAVE**: recargando procesados y verificando nuevos...")
        log_info("Loop activo: reinicio suave post-ciclo", func_name)

        # Liberar flag antes de reiniciar para evitar bloqueo.
        st.session_state.procesamiento_en_curso = False

        # Verificar contra procesados.log/procesados.txt antes del rerun.
        try:
            ...
            if not nuevos_encontrados:
                nuevos_encontrados = buscar_videos_tradicional(procesados_actualizados, func_name)
                if nuevos_encontrados:
                    log_info(
                        f"Loop: búsqueda tradicional tras optimizada vacía → {len(nuevos_encontrados)} archivo(s)",
                        func_name,
                    )
            if nuevos_encontrados:
                st.success(f"🆕 Detectados {len(nuevos_encontrados)} nuevos. Reiniciando para procesar...")
                # Forzar siguiente ciclo inmediato.
                st.session_state.ultimo_procesamiento_continuo = 0
            else:
                st.info("✅ Sin nuevos por ahora. Reiniciando y esperando próximo intervalo...")
                # Mantener cadence normal cuando no hay nuevos.
                st.session_state.ultimo_procesamiento_continuo = time.time()
        except Exception as e:
            log_exception(func_name, e, "Error validando procesados antes de reinicio suave")
            st.warning(f"⚠️ Error validando procesados: {e}. Reiniciando de todos modos.")

        try:
            st.rerun()
        except Exception as e:
            log_warning(f"st.rerun() falló durante reinicio suave: {e}", func_name)
            st.warning("⚠️ No se pudo reiniciar automáticamente. Pulsa continuar.")
            if st.button("🔄 **CONTINUAR**", key="continuar_loop_reinicio_suave"):
                st.rerun()
    else:
...
if st.session_state.running and filtrar_terminos_por_analisis_clientes(st.session_state.terminos_continuos):
...
with tab4:
    ...
    if st.session_state.get('mostrar_form_nueva', False):
        ...
        if st.button("✅ Crear Entidad", type="primary", key="btn_crear_ent"):
            if _new_nombre.strip():
                ...
                if _ncl:
                    _nc_cl = cargar_cloudinary_config()
                    nc['cloudinary'] = {
                        'enabled': True,
                        'cloud_name': _nc_cl.get('cloud_name', ''),
                        'api_key': _nc_cl.get('api_key', ''),
                        'api_secret': _nc_cl.get('api_secret', ''),
                        'folder': _nc_cl.get('folder', 'video_analyzer_clips'),
                    }
    ...
    for _ent in _clientes:
        ...
        if _ent.get('brevo', {}).get('enabled'): _destinos.append(f"📧 {len(_ent['brevo'].get('correos_destinatarios', []))} correos")
        ...
        with st.expander(f"**{_enombre}** — {len(_eterms)} términos — {len(_destinos)}/6 destinos", expanded=False):
            ...
            with _d2:
                ...
                if _br_on:
                    _correos = _br.get('correos_destinatarios', [])
                    st.caption(f"{len(_correos)} destinatarios")
            ...
            with _d5:
                ...
                if _cl_on and not _cl.get('cloud_name'):
                    if st.button("Activar", key=f"sv_cl_{_eid}"):
                        _cl_cfg = cargar_cloudinary_config()
                        _ent['cloudinary'] = {
                            'enabled': True,
                            'cloud_name': _cl_cfg.get('cloud_name', ''),
                            'api_key': _cl_cfg.get('api_key', ''),
                            'api_secret': _cl_cfg.get('api_secret', ''),
                            'folder': _cl_cfg.get('folder', 'video_analyzer_clips'),
                        }
                        actualizar_cliente(_eid, _ent); st.success("OK")
                elif _cl_on:
            ...
            with _d6:
                ...
                if _sb_on:
                    ...
                    if st.button("💾", key=f"sv_sb_{_eid}"):
                        _ent['supabase'] = {
                            'enabled': True,
                            'url': _sb.get('url', SUPABASE_URL or ''),
                            'anon_key': _sb.get('anon_key', SUPABASE_ANON_KEY or ''),
                            'tabla_nombre': _sb_t.strip(),
                        }
                        actualizar_cliente(_eid, _ent); st.success("OK")
            if _eid != 'default':
                st.markdown("---")
