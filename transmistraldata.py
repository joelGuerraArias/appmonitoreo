...
def enviar_coincidencia_inmediata(nombre_archivo, termino_encontrado, contexto_termino, tipo_archivo, clip_path=None, transcripcion_completa=""):
    """
    Envía una coincidencia inmediatamente tan pronto se encuentra:
    1. PRIMERO: Genera resumen ejecutivo del contexto
    ...
    try:
        ...
        # PASO 0: Generar resumen ejecutivo usando IA si tenemos transcripción
        resumen_ejecutivo = ""
        if transcripcion_completa and len(transcripcion_completa.strip()) > 100:
            try:
...
{resumen_ejecutivo}"""
        ...
        if clip_path and os.path.exists(clip_path):
            ...
            # También enviar clip al webhook si está configurado
            if webhook_config['enabled'] and webhook_config['url']:
                ...
                if clip_path and os.path.exists(clip_path):
                    ...
                    with st.spinner("🎬 Subiendo video clip a Google Drive..."):
                        ...
                        if resultado_clip:
                            ...
                            # Actualizar el clip en clips_generados_en_sesion con la URL de Drive
                            if 'clips_generados_en_sesion' in st.session_state:
                                for clip in st.session_state.clips_generados_en_sesion:
                                    if clip['path'] == clip_path and clip['termino'] == termino_encontrado:
                                        clip['url_gdrive'] = video_url_gdrive
                                        log_info(f"✅ URL Drive actualizada en clip: {termino_encontrado}", func_name)
                                        break
                        ...
                        try:
                            ...
                            if cloudinary_configurado:
                                ...
                                if video_url_cloudinary:
                                    ...
                                    # Actualizar el clip en clips_generados_en_sesion con la URL de Cloudinary
                                    if 'clips_generados_en_sesion' in st.session_state:
                                        for clip in st.session_state.clips_generados_en_sesion:
                                            if clip['path'] == clip_path and clip['termino'] == termino_encontrado:
                                                clip['url_cloudinary'] = video_url_cloudinary
                                                log_info(f"✅ URL Cloudinary actualizada en clip: {termino_encontrado}", func_name)
                                                break
                                else:
...
{resumen_ejecutivo}"""
        ...
        return True, "Coincidencia enviada: resumen + video clip + Google Drive + archivo MD (con pausas)"
...
def cargar_cloudinary_config():
    ...
    default_config = {
        'cloud_name': 'dhzxzbkmc',  # Tu cloud name
        'api_key': '149663287387673',  # Tu API key
        'api_secret': 'YOUR_CLOUDINARY_API_SECRET',  # Tu API secret
...
def extraer_info_del_nombre_archivo(nombre_archivo):
    ...
    Args:
        nombre_archivo: Nombre del archivo (ej: "Parnorama TV_720p_2025-09-26_13-55-11_seg049.mp4")
    
    Returns:
        tuple: (nombre_medio, fecha_programa, hora_programa)
    """
    import re
    ...
    # Extraer nombre del medio (antes de la fecha o al inicio)
    ...
    for medio in medios_conocidos:
        ...
        # antes de la fecha o caracteres especiales
...
def crear_fecha_detencion(fecha_programa, hora_programa):
    ...
    Args:
        fecha_programa: Fecha del programa (date object o None)
        hora_programa: Hora del programa (time object o None)
    
    Returns:
        str: Fecha de detección en formato ISO
    """
...
def enviar_coincidencia_inmediata_a_supabase(nombre_archivo, termino_encontrado, contexto_termino, transcripcion_completa="", url_video=None, enlace_directo=None):
    """
    Envía una coincidencia inmediata a Supabase (para envío individual)
    
    Args:
        nombre_archivo: Nombre del archivo procesado
        termino_encontrado: Término detectado
        contexto_termino: Contexto donde se encontró el término
    ...
    """
    func_name = "enviar_coincidencia_inmediata_a_supabase"
    ...
    try:
        ...
        nombre_medio, fecha_programa, hora_programa = extraer_info_del_nombre_archivo(nombre_archivo)
        
        # Crear fecha_detencion
        fecha_detencion = crear_fecha_detencion(fecha_programa, hora_programa)
        
        # Crear resumen ejecutivo básico
        resumen_ejecutivo = f"""Tema principal: Se detectó una mención del término "{termino_encontrado}" en el contenido.
...
Relevancia: Esta mención es significativa para el monitoreo de contenido y puede requerir seguimiento adicional."""
        ...
        registro = {
            ...
            'termino_detectado': termino_encontrado,
            'nombre_medio': nombre_medio,
            'hora_programa': hora_programa.isoformat() if hora_programa else None,
            'fecha_programa': fecha_programa.isoformat() if fecha_programa else None,
            'url_video': url_video,
            'nombre_archivo': nombre_archivo,
            'enlace_directo': enlace_directo,
            'contexto': contexto_termino,
            'resumen_ejecutivo': resumen_ejecutivo,
            'transcripcion': transcripcion_completa or contexto_termino,
            'relevancia': 'Alta'
        }
        
        log_info(f"📝 Enviando coincidencia inmediata a Supabase: {termino_encontrado} en {nombre_medio} - URL Cloudinary: {url_video is not None}, URL Drive: {enlace_directo is not None}", func_name)
        
        # Insertar en Supabase
        result = supabase.table('alertas_medios').insert(registro).execute()
        
        if result.data:
            log_info(f"✅ Coincidencia inmediata enviada a Supabase: {termino_encontrado} - ID: {result.data[0].get('id')}", func_name)
            return True, f"✅ Coincidencia enviada a Supabase (ID: {result.data[0].get('id')})"
        else:
            log_error_critico(func_name, f"❌ Error al insertar en Supabase: {result}")
            return False, f"❌ Error al insertar en Supabase: {result}"
            
    except Exception as e:
        error_msg = f"Error enviando coincidencia inmediata a Supabase: {str(e)}"
...
def enviar_coincidencias_a_supabase(coincidencias_items, nombre_archivo, tipo_archivo, resumen_archivo="", transcripcion_completa="", url_video=None, enlace_directo=None):
    ...
    try:
        ...
        for item in coincidencias_items:
            # Crear fecha_detencion: usar fecha_programa + hora_programa si están disponibles, sino hora actual
            fecha_detencion = crear_fecha_detencion(fecha_programa, hora_programa)
            
            # Crear registro para cada coincidencia
            registro = {
                'fecha_detencion': fecha_detencion,
                'termino_detectado': item.get('termino', ''),
                'nombre_medio': nombre_medio,
                'hora_programa': hora_programa.isoformat() if hasattr(hora_programa, 'isoformat') and hora_programa else None,
                'fecha_programa': fecha_programa.isoformat() if hasattr(fecha_programa, 'isoformat') and fecha_programa else None,
                'url_video': url_video,  # URL de Cloudinary - SIEMPRE enviar
                'nombre_archivo': nombre_archivo,
                'enlace_directo': enlace_directo,  # Enlace directo (URL de Drive) - SIEMPRE enviar
                'contexto': item.get('texto', ''),
                'resumen_ejecutivo': resumen_archivo,
                'transcripcion': transcripcion_completa,
                'relevancia': 'Alta'  # Por defecto alta, se puede calcular
            }
            datos_supabase.append(registro)
            
            log_info(f"📝 Registro preparado para Supabase: {item.get('termino', '')} en {nombre_medio} - URL Cloudinary: {url_video is not None}, URL Drive: {enlace_directo is not None}, Transcripción: {len(transcripcion_completa)} caracteres", func_name)
...
with st.sidebar:
    ...
    st.markdown("---")
    st.header("⚠️ Archivos Fallidos")
    try:
        # Cargar archivos fallidos
        if os.path.exists("fallidos.txt"):
            with open("fallidos.txt", "r", encoding="utf-8") as f:
                archivos_fallidos = [line.strip() for line in f.readlines() if line.strip()]
...
def cargar_procesados():
    """
    Carga la lista de archivos ya procesados desde procesados.log
    """
    procesados = set()
    if os.path.exists(PROCESADOS_LOG):
        try:
            ...
            st.warning(f"⚠️ Error leyendo archivo procesados.log: {e}")
    return procesados
...
# === FUNCIONES DE ARCHIVOS FALLIDOS ===
...
def guardar_archivo_fallido(nombre_archivo, error_mensaje=""):
    """Guarda un archivo en la lista de fallidos"""
    try:
        archivos_fallidos = cargar_archivos_fallidos()
        
        # Evitar duplicados
        if nombre_archivo not in archivos_fallidos:
            with open("fallidos.txt", "a", encoding="utf-8") as f:
                timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                f.write(f"{nombre_archivo}|{timestamp}|{error_mensaje}\n")
            
            log_warning(f"❌ Archivo agregado a fallidos: {nombre_archivo} - Error: {error_mensaje}", "guardar_archivo_fallido")
            return True
        else:
            log_info(f"ℹ️ Archivo ya está en fallidos: {nombre_archivo}", "guardar_archivo_fallido")
            return False
    except Exception as e:
        log_error_critico("guardar_archivo_fallido", f"Error guardando archivo fallido: {e}")
        return False
...
def registrar_archivo_procesado(nombre_archivo, coincidencias, resumen, tipo_archivo):
    """
    Registra un archivo procesado en el log (tanto videos como audios)
    """
    try:
        with open(PROCESADOS_LOG, "a", encoding="utf-8") as f:
            timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
...
def escanear_carpeta_completa():
    ...
    try:
        ...
        with col4:
            # Información del archivo procesados.log
            if os.path.exists(PROCESADOS_LOG):
                try:
                    with open(PROCESADOS_LOG, 'r', encoding='utf-8') as f:
                        lineas = f.readlines()
                    lineas_procesadas = len([l for l in lineas if not l.startswith('#') and l.strip()])
                    st.metric("📝 En Log", lineas_procesadas)
                except Exception:
                    st.metric("📝 En Log", "Error")
            else:
                st.metric("📝 En Log", "No existe")
...
def buscar_y_procesar_videos(duracion_clip=60, buffer_anterior=30):
    ...
    for i, archivo_path in enumerate(nuevos):
        ...
        # Verificar si el archivo está en la lista de fallidos
        if es_archivo_fallido(rel):
            st.warning(f"⚠️ **OMITIENDO ARCHIVO FALLIDO**: `{rel}` (está en la lista de fallidos)")
            log_info(f"Archivo omitido por estar en fallidos: {rel}", func_name)
            continue
        ...
        for termino in terminos:
            ...
            if encontrado:
                ...
                # ========== CONTROL DE DUPLICADOS POR TIMESTAMP ==========
                # Verificar si ya procesamos un clip para este timestamp (tolerancia de ±5 segundos)
                ...
                for ts_procesado in timestamps_procesados:
                    ...
                    if diferencia <= 5:  # Tolerancia de 5 segundos
                        es_duplicado = True
                        log_info(f"⏭️ Término '{termino}' OMITIDO - Ya existe clip para timestamp similar ({diferencia:.1f}s de diferencia)", func_name)
                        st.info(f"⏭️ Término '{termino}' omitido - Ya existe clip para momento similar")
                        break
                
                if es_duplicado:
                    continue  # ❌ NO GENERAR CLIP DUPLICADO - SALTAR AL SIGUIENTE TÉRMINO
                
                # Agregar timestamp a la lista de procesados
                ...
                inicio = max(0, mejor_timestamp['start'] - buffer_anterior)
                
                # VERIFICACIÓN ADICIONAL: Asegurar que el término esté cerca del centro del clip
                momento_termino = mejor_timestamp['start']
                centro_clip = inicio + (duracion_clip / 2)
                distancia_centro = abs(momento_termino - centro_clip)
                
                log_info(f"🎯 CONFIGURACIÓN DE CLIP para '{termino}':", func_name)
                log_info(f"  - Duración total configurada: {duracion_clip}s", func_name)
                log_info(f"  - Buffer anterior: {buffer_anterior}s", func_name)
                log_info(f"  - Momento del término: {momento_termino:.2f}s", func_name)
                log_info(f"  - Inicio del clip: {inicio:.2f}s", func_name)
                log_info(f"  - Centro del clip: {centro_clip:.2f}s", func_name)
                log_info(f"  - Distancia al centro: {distancia_centro:.2f}s", func_name)
                
                # CENTRAR PERFECTAMENTE: Asegurar que la coincidencia esté en el centro del clip
                if distancia_centro > 5:  # Si está a más de 5 segundos del centro
                    log_warning(f"⚠️ Término '{termino}' no está centrado, recentrando clip", func_name)
                    # Centrar perfectamente la coincidencia
                    inicio = max(0, momento_termino - (duracion_clip / 2))
                    log_info(f"  - Nuevo inicio centrado: {inicio:.2f}s", func_name)
                    
                    # Recalcular valores después del centrado
                    centro_clip = inicio + (duracion_clip / 2)
                    distancia_centro = abs(momento_termino - centro_clip)
                    log_info(f"  - Centro recalculado: {centro_clip:.2f}s", func_name)
                    log_info(f"  - Nueva distancia al centro: {distancia_centro:.2f}s", func_name)
                ...
                try:
                    ...
                    if fin_clip > duracion_video:
                        ...
                        # Ajustar el inicio para que quepa todo el clip
                        nuevo_inicio = max(0, duracion_video - duracion_clip)
                        log_info(f"  - Ajustando inicio de {inicio:.2f}s a {nuevo_inicio:.2f}s", func_name)
                        inicio = nuevo_inicio
                        
                        # Recalcular el centrado después del ajuste
                        centro_clip = inicio + (duracion_clip / 2)
                        distancia_centro = abs(momento_termino - centro_clip)
                        log_info(f"  - Nuevo centro ajustado: {centro_clip:.2f}s", func_name)
                        log_info(f"  - Distancia al centro: {distancia_centro:.2f}s", func_name)
                        
                        # Si aún no está centrado, centrar lo mejor posible
                        if distancia_centro > 10:  # Si está muy lejos del centro
                            log_warning(f"⚠️ Coincidencia muy lejos del centro después del ajuste", func_name)
                            # Intentar centrar lo mejor posible dentro de los límites
                            inicio_ideal = max(0, momento_termino - (duracion_clip / 2))
                            if inicio_ideal + duracion_clip <= duracion_video:
                                inicio = inicio_ideal
                                log_info(f"  - Recentrado a inicio ideal: {inicio:.2f}s", func_name)
                            else:
                                log_info(f"  - Manteniendo inicio ajustado: {inicio:.2f}s (no se puede centrar más)", func_name)
                ...
                # VERIFICACIÓN FINAL: Asegurar duración correcta
                if duracion_clip < 60:
                    log_warning(f"⚠️ Duración del clip es {duracion_clip}s, ajustando a 60s", func_name)
                    duracion_clip = 60
                ...
                st.info(f"🎯 Generando clip de {duracion_clip}s para '{termino}' en {momento_termino:.1f}s")
                st.info(f"📊 Clip: {inicio:.1f}s - {fin_clip:.1f}s | Antes: {buffer_anterior_real:.1f}s | Después: {buffer_posterior_real:.1f}s")
                # Comando para recortar video
                cmd = [
                    "ffmpeg", "-y", "-ss", str(inicio),
                    "-t", str(duracion_clip), "-i", archivo_path,
                    "-c:v", "libx264", "-c:a", "aac",
                    clip_path
                ]
                ...
                try:
                    ...
                    # Subir clip a Cloudinary inmediatamente después de generarlo
                    ...
                    clips_generados_en_sesion.append({
                        ...
                        'verificado': True,
                        'url_cloudinary': url_cloudinary_clip,  # URL de Cloudinary del clip
                        'url_gdrive': None       # Se llenará después de subir a Drive
                    })
                    ...
                    exito_envio, mensaje_envio = enviar_coincidencia_inmediata(
                        ...
                        clip_path,  # ruta del clip generado
                        transcripcion_mistral  # transcripción completa
                    )
                    
                    # 🗄️ ENVÍO INMEDIATO A SUPABASE - SIEMPRE OBLIGATORIO
                    with st.spinner(f"🗄️ Enviando '{termino}' a Supabase..."):
                        # Obtener URLs de Cloudinary y Drive del clip generado
                        url_cloudinary = None
                        url_gdrive = None
                        
                        # Buscar URLs en el clip recién agregado
                        for clip in clips_generados_en_sesion:
                            if clip['path'] == clip_path and clip['termino'] == termino:
                                url_cloudinary = clip.get('url_cloudinary')
                                url_gdrive = clip.get('url_gdrive')
                                break
                        
                        # Enviar a Supabase inmediatamente
                        try:
                            supabase_success, supabase_msg = enviar_coincidencia_inmediata_a_supabase(
                                rel,  # nombre del archivo
                                termino,  # término detectado
                                mejor_texto_contexto,  # contexto del término
                                transcripcion_mistral,  # transcripción completa
                                url_cloudinary,  # URL de Cloudinary
                                url_gdrive  # URL de Google Drive
                            )
                            if supabase_success:
                                st.success(f"🗄️ ✅ SUPABASE: {supabase_msg}")
                                log_info(f"✅ Coincidencia inmediata enviada a Supabase: {termino}", "buscar_y_procesar_videos")
                            else:
                                st.error(f"🗄️ ❌ SUPABASE FALLÓ: {supabase_msg}")
                                log_error_critico("buscar_y_procesar_videos", f"❌ Supabase falló para coincidencia inmediata: {supabase_msg}")
                        except Exception as e:
                            st.error(f"🗄️ ❌ ERROR CRÍTICO SUPABASE: {str(e)}")
                            log_error_critico("buscar_y_procesar_videos", f"❌ Error crítico Supabase coincidencia inmediata: {str(e)}")
...
""")
                ...
                coincidencias_items.append({
                    ...
                    "texto": mejor_texto_contexto,
                    "transcripcion_completa": transcripcion_mistral
                })
        ...
        if coincidencias_items:
            try:
                ...
                # Enviar coincidencias a Supabase - SIEMPRE OBLIGATORIO
                with st.spinner("🗄️ Enviando coincidencias a Supabase..."):
                    # Buscar URLs de Cloudinary y Drive para este archivo
                    url_cloudinary = None
                    url_gdrive = None
                    
                    # Buscar en clips generados en sesión
                    for clip in clips_generados_en_sesion:
                        if clip['archivo_origen'] == rel:
                            if clip.get('url_cloudinary'):
                                url_cloudinary = clip['url_cloudinary']
                            if clip.get('url_gdrive'):
                                url_gdrive = clip['url_gdrive']
                            break
                    
                    # Si no se encontraron URLs en clips, buscar en el archivo original
                    if not url_cloudinary or not url_gdrive:
                        # Buscar si el archivo original fue subido a Cloudinary o Drive
                        for archivo_procesado in st.session_state.get('archivos_subidos', []):
                            if archivo_procesado.get('nombre_archivo') == rel:
                                if not url_cloudinary and archivo_procesado.get('url_cloudinary'):
                                    url_cloudinary = archivo_procesado.get('url_cloudinary')
                                if not url_gdrive and archivo_procesado.get('url_gdrive'):
                                    url_gdrive = archivo_procesado.get('url_gdrive')
                                break
                    
                    log_info(f"🔗 URLs para Supabase - Cloudinary: {url_cloudinary is not None}, Drive: {url_gdrive is not None}, Transcripción: {len(transcripcion_mistral)} caracteres", "buscar_y_procesar_videos")
                    
                    # ENVÍO OBLIGATORIO A SUPABASE - NO PUEDE FALLAR SILENCIOSAMENTE
                    try:
                        supabase_success, supabase_msg = enviar_coincidencias_a_supabase(
                            coincidencias_items, rel, tipo_archivo, resumen_archivo, transcripcion_mistral, url_cloudinary, url_gdrive
                        )
                        if supabase_success:
                            st.success(f"🗄️ ✅ SUPABASE: {supabase_msg}")
                            log_info(f"✅ Supabase: {supabase_msg}", "buscar_y_procesar_videos")
                        else:
                            st.error(f"🗄️ ❌ SUPABASE FALLÓ: {supabase_msg}")
                            log_error_critico("buscar_y_procesar_videos", f"❌ Supabase falló: {supabase_msg}")
                    except Exception as e:
                        st.error(f"🗄️ ❌ ERROR CRÍTICO SUPABASE: {str(e)}")
                        log_error_critico("buscar_y_procesar_videos", f"❌ Error crítico Supabase: {str(e)}")
                        # NO CONTINUAR SIN ENVIAR A SUPABASE
                        st.error("🚨 **ERROR CRÍTICO**: No se pudo enviar a Supabase. Revisar configuración.")
                ...
                st.info(f"📦 {tipo_archivo} almacenado para envío posterior: {rel}")
                    ...
                    # Guardar archivo como fallido
                    guardar_archivo_fallido(rel, f"Error en resumen: {str(e)[:100]}")
                    log_error_critico("buscar_y_procesar_videos", f"Error procesando archivo {rel}: {e}")
                else:
                    ...
                    # Registro detallado en procesados.log
                    registrar_archivo_procesado(rel, coincidencias_items, resumen_archivo, tipo_archivo)
                    
        except Exception as e:
            st.error(f"❌ **ERROR CRÍTICO** procesando archivo `{rel}`: {str(e)[:200]}")
            log_error_critico("buscar_y_procesar_videos", f"Error crítico procesando archivo {rel}: {e}")
            
            # Guardar archivo como fallido
            guardar_archivo_fallido(rel, f"Error crítico: {str(e)[:100]}")
            
            # Limpieza de archivos temporales en caso de error
            try:
                if 'audio_path' in locals() and os.path.exists(audio_path) and audio_path != archivo_path:
                    os.remove(audio_path)
            except:
                pass
