...
def enviar_telegram_cliente(cliente, mensaje, video_path=None, parse_mode='Markdown', video_url=None):
    ...
    # Función de escape completa para Telegram (igual que transmistral2.py)
    ...
    if video_path and os.path.exists(video_path) and telegram_config.get('send_clips', True):
        ...
        if video_url and use_cloudinary:
            ...
            caption_limpio = escape_telegram_text(mensaje[:200]) if mensaje else ''
            # Enviar video usando URL existente (sin parse_mode para evitar errores)
            ...
            # Limpiar caption del video también
            caption_limpio = escape_telegram_text(mensaje[:200]) if mensaje else ''
...
def enviar_brevo_cliente(cliente, termino_encontrado, resumen_completo, nombre_video, video_path=None, info_medio="", terminos_detectados=[], video_url=None):
...
def enviar_gdrive_cliente(cliente, archivo_path, nombre_archivo=None):
    """Sube archivo a Google Drive usando credenciales del cliente"""
    ...
    cliente_nombre = cliente.get('nombre', 'Desconocido')
    log_info(f"Subiendo a Google Drive para cliente: {cliente_nombre}", func_name)
    
    try:
        # Usar las credenciales globales o del cliente si están configuradas
        # (Por ahora usa las credenciales globales ya que OAuth2 es más complejo)
        exito, mensaje, file_id = subir_archivo_google_drive(archivo_path, nombre_archivo)
        return exito, mensaje, file_id
    except Exception as e:
        log_exception(func_name, e)
    ...
    try:
        ...
        for item in coincidencias_items:
            try:
                ...
                cliente_supabase.table(tabla_nombre).insert(data).execute()
                insertadas += 1
...
def enviar_coincidencia_a_cliente(cliente, nombre_archivo, termino_encontrado, contexto_termino, tipo_archivo, clip_path=None, transcripcion_completa="", timestamp=None, idea_general=None, video_url=None):
    """
    ...
    if idea_general:
        ...
        try:
            ...
            resumen_ejecutivo = f"Detección de término: {termino_encontrado}\nContexto: {contexto_termino[:300]}"
    else:
        resumen_ejecutivo = f"Detección de término: {termino_encontrado}\nContexto: {contexto_termino[:300]}"
    
    mensaje_completo = f"📺 Medio: {info_medio_hora}\n\nTÉRMINOS DETECTADOS: {termino_encontrado}\n\n{resumen_ejecutivo}"
    ...
    if cliente.get('webhook', {}).get('enabled'):
        with st.spinner(f"🌐 Enviando a Webhook de {cliente_nombre}..."):
            exito, msg = enviar_webhook_cliente(cliente, clip_path or nombre_archivo, resumen_ejecutivo, [termino_encontrado])
    ...
    if cliente.get('brevo', {}).get('enabled'):
        with st.spinner(f"📧 Enviando correo de {cliente_nombre}..."):
            exito, msg = enviar_brevo_cliente(
                cliente, termino_encontrado, resumen_ejecutivo, 
    ...
    if cliente.get('google_drive', {}).get('enabled'):
        with st.spinner(f"☁️ Subiendo a Google Drive de {cliente_nombre}..."):
            # Subir resumen como TXT
            try:
                txt_path = f"temp_resumen_{cliente_nombre.replace(' ', '_')}.txt"
                with open(txt_path, 'w', encoding='utf-8') as f:
                    f.write(mensaje_completo)
                exito_txt, msg_txt, _ = enviar_gdrive_cliente(cliente, txt_path, f"{termino_encontrado}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt")
                os.remove(txt_path)
            except:
            ...
            if clip_path and os.path.exists(clip_path):
                exito_video, msg_video, _ = enviar_gdrive_cliente(cliente, clip_path)
...
def enviar_coincidencia_inmediata(nombre_archivo, termino_encontrado, contexto_termino, tipo_archivo, clip_path=None, transcripcion_completa="", timestamp=None, idea_general=None):
    ...
    try:
        ...
        # 🆕 SUBIR VIDEO A CLOUDINARY PRIMERO (para tener el URL disponible para todos los servicios)
        video_url_cloudinary = None
        if clip_path and os.path.exists(clip_path):
            cloudinary_config = cliente.get('cloudinary', {})
            if cloudinary_config.get('enabled') and cloudinary_config.get('cloud_name'):
                try:
                    # Configurar Cloudinary del cliente
                    cloudinary.config(
                        cloud_name=cloudinary_config['cloud_name'],
                        api_key=cloudinary_config['api_key'],
                        api_secret=cloudinary_config['api_secret']
                    ...
                    st.info("☁️ Subiendo video a Cloudinary...")
                    video_url_cloudinary, upload_msg = subir_video_cloudinary(clip_path, termino_encontrado)
                    
                    if video_url_cloudinary:
                        log_info(f"✅ Video subido a Cloudinary: {video_url_cloudinary}", func_name)
                        st.success(f"☁️ Video subido a Cloudinary exitosamente")
                    else:
                        log_warning(f"⚠️ Error subiendo a Cloudinary: {upload_msg}", func_name)
                except Exception as e:
                    log_warning(f"⚠️ Error configurando Cloudinary: {e}", func_name)
        ...
        resultados = enviar_coincidencia_a_cliente(
            ...
            video_url=video_url_cloudinary  # ✅ Ahora pasa el URL de Cloudinary
        )
        ...
        return True, f"Enviado a cliente: {cliente_nombre}"
...
def probar_conexiones_cliente(cliente):
    ...
    if supabase_config.get('enabled') and supabase_config.get('url') and supabase_config.get('anon_key'):
        try:
            ...
            tabla = supabase_config.get('tabla_nombre', 'alertasmediosintrant')
...
tab1, tab2, tab3, tab4 = st.tabs(["🎬 Todos los Clips", "🆕 Clips de Sesión", "📊 Análisis", "👥 Gestión de Clientes"])
...
with tab4:
    # === GESTIÓN DE CLIENTES ===
    st.markdown("### 👥 Gestión de Clientes Multi-Destino")
    st.info("📌 Configura diferentes clientes con sus propias credenciales de envío. Cada cliente puede tener su propio Webhook, Telegram, Email, Google Drive, etc.")
    
    # Cargar clientes existentes
    clientes_actuales = obtener_clientes_activos()
    
    # Crear columnas para el resumen
    st.markdown("#### 📊 Resumen de Clientes")
    
    if clientes_actuales:
        cols = st.columns(min(len(clientes_actuales), 4))
        for i, cliente in enumerate(clientes_actuales):
            with cols[i % 4]:
                color = cliente.get('color', '#4CAF50')
                nombre = cliente.get('nombre', 'Sin nombre')
                st.markdown(f"""
                <div style="background-color: {color}20; padding: 10px; border-radius: 10px; border-left: 4px solid {color};">
                    <h4 style="margin: 0; color: {color};">{nombre}</h4>
                    <small>ID: {cliente.get('id', 'N/A')}</small>
                </div>
                """, unsafe_allow_html=True)
                
                # Contar servicios activos
                servicios_activos = []
                if cliente.get('webhook', {}).get('enabled'): servicios_activos.append("🌐")
                if cliente.get('telegram', {}).get('enabled'): servicios_activos.append("📱")
                if cliente.get('brevo', {}).get('enabled'): servicios_activos.append("📧")
                if cliente.get('google_drive', {}).get('enabled'): servicios_activos.append("☁️")
                if cliente.get('cloudinary', {}).get('enabled'): servicios_activos.append("🖼️")
                if cliente.get('supabase', {}).get('enabled'): servicios_activos.append("🗄️")
                
                st.caption(f"Servicios: {' '.join(servicios_activos) if servicios_activos else '❌ Ninguno'}")
    else:
        st.warning("No hay clientes configurados")
    
    st.markdown("---")
    
    # === SECCIÓN: CREAR NUEVO CLIENTE ===
    with st.expander("➕ Crear Nuevo Cliente", expanded=False):
        st.markdown("#### Información Básica")
        
        col1, col2, col3 = st.columns([2, 1, 1])
        with col1:
            nuevo_nombre = st.text_input("📛 Nombre del Cliente:", placeholder="Ej: EDENOR, Cliente ABC", key="nuevo_cliente_nombre")
        with col2:
            nuevo_color = st.color_picker("🎨 Color:", value="#4CAF50", key="nuevo_cliente_color")
        with col3:
            st.write("")  # Espaciador
            st.write("")
            if st.button("✅ Crear Cliente", type="primary", key="btn_crear_cliente"):
                if nuevo_nombre.strip():
                    nuevo_cliente = crear_cliente_nuevo(nuevo_nombre.strip(), nuevo_color)
                    exito, mensaje = agregar_cliente(nuevo_cliente)
                    if exito:
                        st.success(mensaje)
                        st.rerun()
                    else:
    ...
    # === SECCIÓN: CONFIGURAR CLIENTE EXISTENTE ===
    st.markdown("#### ⚙️ Configurar Cliente")
    
    # Selector de cliente
    nombres_clientes = [f"{c.get('nombre', 'Sin nombre')} ({c.get('id', 'N/A')})" for c in clientes_actuales]
    ...
    if nombres_clientes:
        cliente_seleccionado_idx = st.selectbox(
            ...
            key="selector_cliente_config"
        )
        
        cliente_actual = clientes_actuales[cliente_seleccionado_idx]
        cliente_id = cliente_actual.get('id')
        
        # Sub-tabs para cada tipo de configuración
        config_tab1, config_tab2, config_tab3, config_tab4, config_tab5, config_tab6 = st.tabs([
            "🌐 Webhook", "📱 Telegram", "📧 Brevo", "☁️ Google Drive", "🖼️ Cloudinary", "🗄️ Supabase"
        ])
        
        # === TAB: WEBHOOK ===
        with config_tab1:
            webhook_config = cliente_actual.get('webhook', {})
            
            webhook_enabled = st.checkbox(
                "🟢 Activar Webhook", 
                value=webhook_config.get('enabled', False),
                key=f"webhook_enabled_{cliente_id}"
            )
            ...
            if webhook_enabled:
                ...
                with col1:
                    webhook_url = st.text_input(
                        "URL Principal (Make.com):", 
                        value=webhook_config.get('url', ''),
                        placeholder="https://hook.us1.make.com/...",
                        key=f"webhook_url_{cliente_id}"
                    )
                with col2:
                    webhook_url2 = st.text_input(
                        "URL Secundario (N8N):", 
                        value=webhook_config.get('url_secundario', ''),
                        placeholder="https://n8n.tudominio.com/webhook/...",
                        key=f"webhook_url2_{cliente_id}"
                    )
                
                webhook_url3 = st.text_input(
                    "URL Terciario (opcional):", 
                    value=webhook_config.get('url_terciario', ''),
                    placeholder="https://otro-webhook.com/...",
                    key=f"webhook_url3_{cliente_id}"
                )
                
                if st.button("💾 Guardar Webhook", key=f"save_webhook_{cliente_id}"):
                    cliente_actual['webhook'] = {
                        'enabled': webhook_enabled,
                        'url': webhook_url.strip(),
                        'url_secundario': webhook_url2.strip(),
                        'url_terciario': webhook_url3.strip()
                    }
                    exito, msg = actualizar_cliente(cliente_id, cliente_actual)
                    if exito:
                        st.success("✅ Webhook guardado")
                    else:
                        st.error(msg)
            else:
                st.info("Webhook desactivado para este cliente")
        
        # === TAB: TELEGRAM ===
        with config_tab2:
            telegram_config = cliente_actual.get('telegram', {})
            
            telegram_enabled = st.checkbox(
                "🟢 Activar Telegram", 
                value=telegram_config.get('enabled', False),
                key=f"telegram_enabled_{cliente_id}"
            )
            ...
            if telegram_enabled:
                ...
                with col1:
                    bot_token = st.text_input(
                        ...
                        type="password",
                        key=f"telegram_token_{cliente_id}"
                    )
                with col2:
                    chat_id = st.text_input(
                        ...
                        placeholder="-1001234567890",
                        key=f"telegram_chatid_{cliente_id}"
                    )
                
                col1, col2, col3 = st.columns(3)
                with col1:
                    send_clips = st.checkbox("📹 Enviar clips", value=telegram_config.get('send_clips', True), key=f"tg_clips_{cliente_id}")
                with col2:
                    send_summary = st.checkbox("📝 Enviar resumen", value=telegram_config.get('send_summary', True), key=f"tg_summary_{cliente_id}")
                with col3:
                    use_cloudinary = st.checkbox("☁️ Usar Cloudinary", value=telegram_config.get('use_cloudinary', True), key=f"tg_cloud_{cliente_id}")
                
                if st.button("💾 Guardar Telegram", key=f"save_telegram_{cliente_id}"):
                    cliente_actual['telegram'] = {
                        'enabled': telegram_enabled,
                        'bot_token': bot_token.strip(),
                        'chat_id': chat_id.strip(),
                        'send_clips': send_clips,
                ...
                st.info("Telegram desactivado para este cliente")
        
        # === TAB: BREVO (EMAIL) ===
        with config_tab3:
            brevo_config = cliente_actual.get('brevo', {})
            
            brevo_enabled = st.checkbox(
                "🟢 Activar Brevo (Email)", 
                value=brevo_config.get('enabled', False),
                key=f"brevo_enabled_{cliente_id}"
            )
            
            if brevo_enabled:
                st.markdown("##### 🔐 Credenciales SMTP")
                
                api_key = st.text_input(
                    "🔑 API Key SMTP (xsmtpsib-...):", 
                    value=brevo_config.get('api_key', ''),
                    placeholder="xsmtpsib-...",
                    type="password",
                    key=f"brevo_apikey_{cliente_id}"
                )
                ...
                with col1:
                    smtp_user = st.text_input(
                        "👤 Usuario SMTP:", 
                        value=brevo_config.get('smtp_user', '951480002@smtp-brevo.com'),
                        placeholder="123456@smtp-brevo.com",
                        key=f"brevo_smtpuser_{cliente_id}"
                    )
                with col2:
                    smtp_server = st.text_input(
                        "🌐 Servidor SMTP:", 
                        value=brevo_config.get('smtp_server', 'smtp-relay.brevo.com'),
                        key=f"brevo_smtpserver_{cliente_id}"
                    )
                
                smtp_port = st.number_input(
                    "🔌 Puerto SMTP:", 
                    value=brevo_config.get('smtp_port', 587),
                    min_value=1,
                    max_value=65535,
                    key=f"brevo_smtpport_{cliente_id}"
                )
                ...
                with col1:
                    sender_email = st.text_input(
                        "📧 Email Remitente:", 
                        value=brevo_config.get('sender_email', ''),
                        placeholder="alertas@tudominio.com",
                        key=f"brevo_sender_{cliente_id}"
                    )
                with col2:
                    sender_name = st.text_input(
                        "👤 Nombre Remitente:", 
                        value=brevo_config.get('sender_name', f'Sistema {cliente_actual.get("nombre", "")}'),
                        key=f"brevo_sendername_{cliente_id}"
                    )
                
                # Lista de correos destinatarios
                st.markdown("##### 📨 Destinatarios")
                correos_actuales = brevo_config.get('correos_destinatarios', [])
                
                if correos_actuales:
                    for i, correo in enumerate(correos_actuales):
                        col1, col2 = st.columns([3, 1])
                        with col1:
                            st.text(f"📧 {correo}")
                        with col2:
                            if st.button("🗑️", key=f"del_correo_{cliente_id}_{i}"):
                                correos_actuales.pop(i)
                                cliente_actual['brevo']['correos_destinatarios'] = correos_actuales
                                actualizar_cliente(cliente_id, cliente_actual)
                                st.rerun()
                else:
                    st.caption("No hay destinatarios configurados")
                
                col1, col2 = st.columns([3, 1])
                with col1:
                    nuevo_correo = st.text_input("Agregar correo:", placeholder="nuevo@email.com", key=f"brevo_nuevo_{cliente_id}")
                with col2:
                    st.write("")
                    if st.button("➕ Agregar", key=f"add_correo_{cliente_id}"):
                        if nuevo_correo.strip() and "@" in nuevo_correo:
                            if nuevo_correo.strip() not in correos_actuales:
                                correos_actuales.append(nuevo_correo.strip())
                                cliente_actual['brevo']['correos_destinatarios'] = correos_actuales
                                actualizar_cliente(cliente_id, cliente_actual)
                                st.success(f"Correo {nuevo_correo} agregado")
                                st.rerun()
                            else:
                ...
                if st.button("💾 Guardar Brevo", key=f"save_brevo_{cliente_id}"):
                    cliente_actual['brevo'] = {
                        'enabled': brevo_enabled,
                        'api_key': api_key.strip(),
                        ...
                        'sender_email': sender_email.strip(),
                        'sender_name': sender_name.strip(),
                        'correos_destinatarios': correos_actuales
                    }
                    exito, msg = actualizar_cliente(cliente_id, cliente_actual)
                    if exito:
                        st.success("✅ Brevo guardado")
                    else:
                        st.error(msg)
            else:
                st.info("Brevo (Email) desactivado para este cliente")
        
        # === TAB: GOOGLE DRIVE ===
        with config_tab4:
            gdrive_config = cliente_actual.get('google_drive', {})
            
            gdrive_enabled = st.checkbox(
                "🟢 Activar Google Drive", 
                value=gdrive_config.get('enabled', False),
                key=f"gdrive_enabled_{cliente_id}"
            )
            
            if gdrive_enabled:
                folder_id = st.text_input(
                    "📁 ID de Carpeta de Google Drive:", 
                    value=gdrive_config.get('folder_id', ''),
                    placeholder="1ABC...xyz",
                    key=f"gdrive_folder_{cliente_id}"
                )
                
                st.markdown("##### 🔑 Credenciales OAuth2")
                col1, col2 = st.columns(2)
                with col1:
                    client_id = st.text_input(
                        "Client ID:", 
                        value=gdrive_config.get('client_id', ''),
                        type="password",
                        key=f"gdrive_clientid_{cliente_id}"
                    )
                with col2:
                    client_secret = st.text_input(
                        "Client Secret:", 
                        value=gdrive_config.get('client_secret', ''),
                        type="password",
                        key=f"gdrive_secret_{cliente_id}"
                    )
                
                refresh_token = st.text_input(
                    "Refresh Token:", 
                    value=gdrive_config.get('refresh_token', ''),
                    type="password",
                    key=f"gdrive_refresh_{cliente_id}"
                )
                
                if st.button("💾 Guardar Google Drive", key=f"save_gdrive_{cliente_id}"):
                    cliente_actual['google_drive'] = {
                    ...
                    exito, msg = actualizar_cliente(cliente_id, cliente_actual)
                    if exito:
                        st.success("✅ Google Drive guardado")
                    else:
                        st.error(msg)
            else:
                st.info("Google Drive desactivado para este cliente")
        
        # === TAB: CLOUDINARY ===
        with config_tab5:
            cloudinary_config = cliente_actual.get('cloudinary', {})
            
            cloudinary_enabled = st.checkbox(
                "🟢 Activar Cloudinary", 
                value=cloudinary_config.get('enabled', False),
                key=f"cloudinary_enabled_{cliente_id}"
            )
            
            if cloudinary_enabled:
                cloud_name = st.text_input(
                    "☁️ Cloud Name:", 
                    value=cloudinary_config.get('cloud_name', ''),
                    placeholder="tu-cloud-name",
                    key=f"cloudinary_name_{cliente_id}"
                )
                
                col1, col2 = st.columns(2)
                with col1:
                    cl_api_key = st.text_input(
                        "🔑 API Key:", 
                        value=cloudinary_config.get('api_key', ''),
                        type="password",
                        key=f"cloudinary_apikey_{cliente_id}"
                    )
                with col2:
                    cl_api_secret = st.text_input(
                        "🔒 API Secret:", 
                        value=cloudinary_config.get('api_secret', ''),
                        type="password",
                        key=f"cloudinary_secret_{cliente_id}"
                    )
                
                cl_folder = st.text_input(
                    "📁 Carpeta:", 
                    value=cloudinary_config.get('folder', f'clips_{cliente_actual.get("nombre", "cliente").lower().replace(" ", "_")}'),
                    key=f"cloudinary_folder_{cliente_id}"
                )
                
                if st.button("💾 Guardar Cloudinary", key=f"save_cloudinary_{cliente_id}"):
                    cliente_actual['cloudinary'] = {
                        'enabled': cloudinary_enabled,
                        'cloud_name': cloud_name.strip(),
                        'api_key': cl_api_key.strip(),
                        'api_secret': cl_api_secret.strip(),
                        'folder': cl_folder.strip()
                    }
                    exito, msg = actualizar_cliente(cliente_id, cliente_actual)
                    if exito:
                        st.success("✅ Cloudinary guardado")
                    else:
                        st.error(msg)
            else:
                st.info("Cloudinary desactivado para este cliente")
        
        # === TAB: SUPABASE ===
        with config_tab6:
            supabase_config = cliente_actual.get('supabase', {})
            
            supabase_enabled = st.checkbox(
                "🟢 Activar Supabase", 
                value=supabase_config.get('enabled', False),
                key=f"supabase_enabled_{cliente_id}"
            )
            
            if supabase_enabled:
                supabase_url = st.text_input(
                    "🌐 Supabase URL:", 
                    value=supabase_config.get('url', ''),
                    placeholder="https://xxxxx.supabase.co",
                    key=f"supabase_url_{cliente_id}"
                )
                
                supabase_key = st.text_input(
                    "🔑 Anon Key:", 
                    value=supabase_config.get('anon_key', ''),
                    type="password",
                    key=f"supabase_key_{cliente_id}"
                )
                
                st.markdown("##### 📋 Configuración de Tabla")
                
                # Lista de tablas disponibles
                tabla_cliente_custom = f"alertas_{cliente_actual.get('nombre', 'cliente').lower().replace(' ', '_')}"
                tablas_disponibles = [
                    "alertasmediosintrant",
                    "alertas_medios", 
                    tabla_cliente_custom,
                    "📝 Personalizada..."
                ]
                
                # Obtener tabla guardada
                tabla_guardada = supabase_config.get('tabla_nombre', 'alertasmediosintrant')
                
                # Si la tabla guardada no está en la lista, es personalizada
                if tabla_guardada not in tablas_disponibles[:-1]:
                    tablas_disponibles.insert(-1, tabla_guardada)
                
                # Encontrar índice de la tabla guardada
                try:
                    idx_tabla = tablas_disponibles.index(tabla_guardada)
                except ValueError:
                    idx_tabla = 0
                ...
                tabla_seleccionada = st.selectbox(
                    ...
                    index=idx_tabla,
                    help="Selecciona la tabla donde se guardarán las coincidencias de este cliente.",
                    key=f"supabase_tabla_select_{cliente_id}"
                )
                
                # Si eligió personalizada, mostrar input
                if tabla_seleccionada == "📝 Personalizada...":
                    supabase_tabla = st.text_input(
                        "Nombre de tabla personalizada:",
                        value="",
                        placeholder="mi_tabla_custom",
                        key=f"supabase_tabla_custom_{cliente_id}"
                    )
                    if not supabase_tabla:
                        st.warning("⚠️ Ingresa un nombre de tabla")
                else:
                    supabase_tabla = tabla_seleccionada
                
                # Mostrar tabla seleccionada
                if supabase_tabla and supabase_tabla != "📝 Personalizada...":
                    st.success(f"📋 Tabla seleccionada: **{supabase_tabla}**")
                
                # Mostrar estructura de la tabla
                with st.expander("📐 Ver estructura de la tabla"):
                    st.code(f"""
CREATE TABLE {supabase_tabla} (
    id BIGSERIAL PRIMARY KEY,
    termino_detectado TEXT,
    nombre_archivo TEXT,
    tipo_archivo TEXT,
    ...
    url_video TEXT,
    enlace_directo TEXT,
    cliente TEXT DEFAULT '{cliente_actual.get('nombre', '')}',
    transcripcion TEXT,
    created_at TIMESTAMPTZ DEFAULT NOW()
);
                    """, language="sql")
                    st.caption("⚠️ Si la tabla no existe, copia este SQL y ejecútalo en Supabase Dashboard")
                
                if st.button("💾 Guardar Supabase", key=f"save_supabase_{cliente_id}"):
                    cliente_actual['supabase'] = {
                        'enabled': supabase_enabled,
                        'url': supabase_url.strip(),
                        'anon_key': supabase_key.strip(),
                        'tabla_nombre': supabase_tabla.strip() if supabase_tabla.strip() else tabla_default
                    }
                    exito, msg = actualizar_cliente(cliente_id, cliente_actual)
                    if exito:
                        st.success(f"✅ Supabase guardado (tabla: {supabase_tabla})")
                    else:
                        st.error(msg)
                
                # Botón para probar conexión
                if supabase_url and supabase_key:
                    if st.button("🧪 Probar Conexión", key=f"test_supabase_{cliente_id}"):
                        with st.spinner("Probando conexión..."):
                            try:
                                test_client = create_client(supabase_url.strip(), supabase_key.strip())
                                # Intentar consultar la tabla
                                try:
                                    test_client.table(supabase_tabla).select('id').limit(1).execute()
                                    st.success(f"✅ Conexión exitosa. Tabla '{supabase_tabla}' existe.")
                                except Exception as tabla_error:
                                    if 'does not exist' in str(tabla_error).lower() or '42P01' in str(tabla_error):
                                        st.warning(f"⚠️ Conexión OK pero tabla '{supabase_tabla}' no existe. Se creará automáticamente al enviar.")
                                    else:
                                        st.warning(f"⚠️ Conexión OK. Error tabla: {str(tabla_error)[:100]}")
                            except Exception as e:
                                st.error(f"❌ Error de conexión: {str(e)[:100]}")
            else:
                st.info("Supabase desactivado para este cliente")
        
        st.markdown("---")
        
        # Botón para eliminar cliente (excepto default)
        if cliente_id != 'default':
            with st.expander("⚠️ Zona de Peligro", expanded=False):
                st.warning(f"Esta acción eliminará permanentemente el cliente '{cliente_actual.get('nombre')}'")
                if st.button("🗑️ Eliminar Cliente", type="secondary", key=f"delete_cliente_{cliente_id}"):
                    exito, msg = eliminar_cliente(cliente_id)
                    if exito:
                        st.success(msg)
                        st.rerun()
                    else:
            ...
            st.info("ℹ️ El cliente 'Sistema Principal' no puede ser eliminado")
    else:
        st.warning("No hay clientes configurados. Crea uno nuevo arriba.")
    
    st.markdown("---")
    
    # === SECCIÓN: ASOCIAR TÉRMINOS A CLIENTES ===
    st.markdown("#### 🏷️ Asociar Términos a Clientes")
    st.info("Configura qué términos de búsqueda enviarán coincidencias a cada cliente")
    
    # Cargar términos actuales
    config_terminos = cargar_configuracion_completa()
    terminos_actuales = config_terminos.get('terminos', [])
    
    if terminos_actuales:
        # Convertir términos legacy a formato nuevo si es necesario
        terminos_normalizados = []
        for t in terminos_actuales:
            if isinstance(t, str):
                terminos_normalizados.append({'termino': t, 'cliente_id': 'default'})
            else:
                terminos_normalizados.append(t)
        
        st.markdown("**Términos Configurados:**")
        
        # Crear opciones de clientes para el selector
        opciones_clientes = {c.get('id'): c.get('nombre', 'Sin nombre') for c in clientes_actuales}
        
        cambios_pendientes = False
        for i, termino_data in enumerate(terminos_normalizados):
            col1, col2, col3 = st.columns([2, 2, 1])
            
            with col1:
                st.text_input(
                    "Término:", 
                    value=termino_data.get('termino', ''),
                    disabled=True,
                    key=f"termino_txt_{i}"
                )
            
            with col2:
                cliente_actual_id = termino_data.get('cliente_id', 'default')
                ids_clientes = list(opciones_clientes.keys())
                idx_actual = ids_clientes.index(cliente_actual_id) if cliente_actual_id in ids_clientes else 0
                
                nuevo_cliente_id = st.selectbox(
                    "Cliente asociado:",
                    ids_clientes,
                    index=idx_actual,
                    format_func=lambda x: opciones_clientes.get(x, x),
                    key=f"termino_cliente_{i}"
                )
                
                if nuevo_cliente_id != cliente_actual_id:
                    terminos_normalizados[i]['cliente_id'] = nuevo_cliente_id
                    cambios_pendientes = True
            
            with col3:
                # Mostrar color del cliente asociado
                cliente_asoc = next((c for c in clientes_actuales if c.get('id') == nuevo_cliente_id), None)
                if cliente_asoc:
                    color = cliente_asoc.get('color', '#666')
                    st.markdown(f'<span style="background-color: {color}; padding: 5px 10px; border-radius: 5px; color: white;">●</span>', unsafe_allow_html=True)
        
        if cambios_pendientes:
            if st.button("💾 Guardar Asociaciones de Términos", type="primary"):
                config_terminos['terminos'] = terminos_normalizados
                try:
                    with open(TERMINOS_CONFIG, 'w', encoding='utf-8') as f:
                        json.dump(config_terminos, f, indent=2, ensure_ascii=False)
                    st.success("✅ Asociaciones guardadas")
                    st.rerun()
                except Exception as e:
                    st.error(f"Error guardando: {e}")
    else:
        st.info("No hay términos configurados. Agrega términos en la sección de configuración del sidebar.")
