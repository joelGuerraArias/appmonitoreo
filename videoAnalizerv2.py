...
# === CONFIGURACIÓN ===
# Carpeta donde buscar videos a analizar y donde guardar resultados
_DIR_SCRIPT = Path(__file__).parent.resolve()
_CARPETA_BASE = _DIR_SCRIPT / "videos procesados"
CARPETA_VIDEOS = os.getenv('CARPETA_VIDEOS', str(_CARPETA_BASE))
CARPETA_PROCESADOS = os.getenv('CARPETA_PROCESADOS', str(_CARPETA_BASE))
...
def guardar_video_y_minuto_coincidencia(video_path, minuto_detectado, termino_detectado="", mover_video=False):
    """
    Guarda/copia (o mueve) el video con coincidencia en `videoscheck`
    ...
    try:
        ...
        carpeta_destino = os.path.join(_DIR_SCRIPT, "videoscheck")
        ...
        if not os.path.exists(destino_video):
            if mover_video:
                ...
                accion = "copiado"
            log_info(f"Video {accion} a videoscheck: {destino_video}", func_name)
        ...
        return destino_video, txt_minutos, "ok"
    except Exception as e:
        log_warning(f"No se pudo guardar en videoscheck: {e}", func_name)
...
{resumen_ejecutivo}"""
        ...
        return True, "Coincidencia enviada: resumen + video clip + Google Drive + archivo MD + Supabase (con pausas)", video_url_cloudinary
...
def generar_md_sesion_coincidencias(videos_procesados_data, clips_generados_en_sesion, estadisticas_escaneo=None, terminos_buscados=None):
    ...
    try:
        ...
        md.append(f"")
        md.append(f"> **Video Analyzer IA v2.0** | Sesión: {fecha_legible}")
        md.append(f"")
        md.append(f"---")
        md.append(f"")
        ...
        # PIE
        # ============================
        md.append(f"_Reporte generado automáticamente por Video Analyzer IA v2.0 - {fecha_legible}_")
...
def generar_analisishoy_md(nombre_archivo, termino_encontrado, contexto_termino="", resumen_ejecutivo="", transcripcion_completa="", video_url=None, info_medio="", timestamp=None):
    """
    ...
    try:
        ...
        ctx_raw = (contexto_termino or "").strip()
        if len(ctx_raw) > 8000:
            contexto_display = ctx_raw[:8000] + "\n\n…(texto truncado para el MD; el análisis completo sigue en el resumen ejecutivo)"
        else:
            contexto_display = ctx_raw if ctx_raw else "No disponible"
        ...
        info_medio_str = info_medio if isinstance(info_medio, str) else str(info_medio)
        linea_tiempo = ""
        if timestamp is not None:
            try:
                ...
                ss = int(ts % 60)
                tv = f"{mm}m{ss:02d}s"
                linea_tiempo = f"- **Tiempo en el video:** {tv} ({ts:.1f}s)\n"
...
- **Medio:** {info_medio_str}
- **Término detectado:** **{termino_encontrado}**
"""
            + linea_tiempo
            + f"- **Video Cloudinary:** {f'[{video_url}]({video_url})' if video_url else 'No disponible'}\n\n"
            + "## 💡 Contexto / idea del segmento (motivo del hallazgo)\n\n"
            + contexto_display
            + "\n\n"
    ...
    <div style="max-width: 700px; margin: 0 auto;">
        ...
        <div style="background: #343a40; color: white; padding: 20px; border-radius: 0 0 16px 16px; text-align: center;">
            <p style="margin: 4px 0; opacity: 0.8; font-size: 13px;">
                🤖 Resumen generado automáticamente por Video Analyzer IA v2.0
            </p>
...
def transcribir_audio_hibrido(audio_path, indice_actual=None, total=None):
    ...
    try:
        ...
        st.info(f"🔄 {progreso}🎯 **ANÁLISIS DE ARCHIVO:** {os.path.basename(audio_path)} ({tamaño_mb:.1f}MB)")
        ...
        st.info(f"📊 **DECISIÓN:** Archivo {tamaño_mb:.1f}MB → Intentando **faster-whisper GPU** (local, sin costo)")
        ...
        try:
            ...
            st.success(f"✅ **{motor}** completó transcripción + timestamps — **SIN COSTO**")
            ...
            if es_cuda:
                st.warning(f"⚠️ **faster-whisper GPU falló** → Intentando **faster-whisper CPU**")
                ...
                try:
                    ...
                    st.success(f"✅ **faster-whisper CPU** completó transcripción + timestamps — **SIN COSTO**")
                    log_info(f"Transcripción exitosa con faster-whisper CPU", func_name)
                    return result, "faster-whisper CPU", segments
                except Exception as cpu_error:
                    st.warning(f"⚠️ **faster-whisper CPU falló** → Activando **Voxtral (Mistral)**")
                    log_info(f"faster-whisper CPU falló ({cpu_error}) - Activando Voxtral", func_name)
            else:
                st.warning(f"⚠️ **faster-whisper falló** ({err_str}...) → Activando **Voxtral (Mistral)**")
        ...
        st.info(f"📊 **FALLBACK:** Usando **Voxtral** (Mistral) con timestamps")
        try:
            result, segments = transcribir_audio_mistral(audio_path, con_timestamps=True)
            st.success(f"✅ **Mistral (Voxtral)** completó transcripción + timestamps")
        ...
        st.info(f"📊 **FALLBACK 2:** Usando **OpenAI Whisper** (último recurso)")
        try:
            result = transcribir_con_openai(audio_path)
            st.success(f"✅ **OpenAI Whisper** completó la transcripción (sin timestamps - se obtendrán con whisper local)")
            ...
            st.error(f"❌ **Las 3 opciones fallaron** — faster-whisper, Voxtral y OpenAI Whisper")
...
def buscar_y_procesar_videos(duracion_clip=90, buffer_anterior=30):
    ...
    for i, archivo_path in enumerate(nuevos):
        ...
        try:
            ...
            # === FILTRO: Saltar videos menores a 5 minutos ===
            ...
            if dur_total and dur_total < DURACION_MINIMA_SEGUNDOS:
                ...
                st.warning(f"⏭️ **SALTANDO video corto:** `{rel}` — Duración: {minutos}:{segundos:02d} (mínimo requerido: 5:00 minutos)")
                log_info(f"Video saltado por duración corta ({dur_total:.0f}s < 300s): {rel}", func_name)
                registrar_archivo_procesado(os.path.basename(archivo_path), [], "Video saltado: duración menor a 5 minutos", "mp4")
                ...
                if encontrado:
                    ...
                    try:
                        ...
                        if segmento_gemini is None:
                            ...
                            _, _, estado_videoscheck = guardar_video_y_minuto_coincidencia(
                                video_path=archivo_path,
                                minuto_detectado=momento_termino,
                                termino_detectado=termino,
                                mover_video=False
                            )
                            if estado_videoscheck == "ok":
                                st.info("📁 Video enviado a `videoscheck` por rechazo tangencial")
                            else:
                        ...
                        try:
                            ...
                            if not es_relevante_gemini or relevancia_gemini == 'baja':
                                if es_prioritario:
                                    ...
                                    if estado_videoscheck == "ok":
                                        st.info("📁 Video enviado a `videoscheck` por descarte de relevancia")
                                    else:
                        ...
                        if clip_path and os.path.exists(clip_path):
                            try:
                                ...
                                if _dur_clip < MIN_DURACION_CLIP_ENVIO:
                                    log_warning(f"⚠️ Clip descartado por requerimiento de usuario: duración {_dur_clip:.1f}s < {MIN_DURACION_CLIP_ENVIO}s", func_name)
                                    st.warning(f"⚠️ Video descartado por ser menor a {MIN_DURACION_CLIP_ENVIO}s (según tu requerimiento).")
    ...
    if loop_activo:
        ...
        st.info("🔍 **RE-ESCANEANDO** - Buscando videos nuevos...")
        ...
        if nuevos_encontrados:
            ...
            log_info(f"Loop: {len(nuevos_encontrados)} videos nuevos, iniciando nuevo ciclo", func_name)
            time.sleep(2)
            try:
            ...
            sesion_viva = _countdown_seguro(
                ...
                "💤 Sin videos nuevos. Re-escaneando en {i}s... (Desactiva el loop en sidebar)",
                func_name
            )
            
            if not sesion_viva:
                log_warning("Sesión cerrada durante espera sin videos nuevos (post-ciclo). Saliendo limpiamente.", func_name)
                return
