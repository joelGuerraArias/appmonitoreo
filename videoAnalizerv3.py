# -*- coding: utf-8 -*-
"""
Video Analyzer Pro v3 - Analizador de Videos con IA (2 Fases)
=============================================================
Versión: 3.0.0
Autor: Video Analyzer Team

Sistema de análisis de videos con procesamiento en 2 FASES:
  FASE 1 (PRE-FILTRADO): Extrae audio + faster-whisper (local, gratis) + busca términos
          → Solo identifica qué videos tienen coincidencias (rápido y sin costo API)
  FASE 2 (PROCESAMIENTO): Solo videos con coincidencias → Mistral/Voxtral (calidad)
          + Gemini (análisis) + clips + notificaciones

Ventaja: ~50-60% más rápido, ~85% menos costo API vs v2
"""
...
st.set_page_config(page_title="🧠 Video Analyzer Pro v3 - 2 Fases", layout="wide")
...
def enviar_telegram_cliente(cliente, mensaje, video_path=None, parse_mode='Markdown', video_url=None):
    ...
    if video_path and os.path.exists(video_path) and telegram_config.get('send_clips', True):
        ...
        if video_url and use_cloudinary:
            ...
            caption_limpio = escape_telegram_text(mensaje[:1024]) if mensaje else ''
...
def enviar_coincidencia_a_cliente(cliente, nombre_archivo, termino_encontrado, contexto_termino, tipo_archivo, clip_path=None, transcripcion_completa="", timestamp=None, idea_general=None, video_url=None):
    ...
    # === CONSTRUIR RESUMEN EJECUTIVO COMPLETO ===
    # Formato unificado que se envía TAL CUAL a todos los canales
    mensaje_completo = f"RESUMEN EJECUTIVO COMPLETO\n\n"
    mensaje_completo += f"📺 Medio: {info_medio_hora}\n\n"
    mensaje_completo += f"TÉRMINOS DETECTADOS: {termino_encontrado}\n\n"
    mensaje_completo += f"{resumen_ejecutivo}"
    ...
    if cliente.get('webhook', {}).get('enabled'):
        with st.spinner(f"🌐 Enviando a Webhook de {cliente_nombre}..."):
            exito, msg = enviar_webhook_cliente(cliente, clip_path or nombre_archivo, mensaje_completo, [termino_encontrado])
...
def transcribir_audio_hibrido(audio_path):
    """
    Sistema de transcripción con Voxtral (Mistral) como motor principal.
    Voxtral Mini Transcribe soporta hasta ~30 minutos de audio.
    - Archivos normales: Voxtral directo
    - Archivos muy grandes (>100MB): Convierte a MP3 primero para reducir tamaño
    - Fallback a OpenAI Whisper si Voxtral falla
    """
    ...
    try:
        ...
        if tamaño_mb > 100:
            # Archivo muy grande - convertir a MP3 primero para reducir tamaño
            st.info(f"📊 **DECISIÓN:** Archivo {tamaño_mb:.1f}MB (>100MB) → **Convirtiendo a MP3** antes de transcribir")
        ...
        # Archivo normal - usar Voxtral directo
        st.info(f"📊 **DECISIÓN:** Archivo {tamaño_mb:.1f}MB → Usando **Voxtral** (Mistral Transcription API)")
        log_info(f"Archivo {tamaño_mb:.1f}MB - Usando Voxtral API", func_name)
        
        try:
            result = transcribir_audio_mistral(audio_path)
            st.success(f"✅ **Voxtral** completó la transcripción exitosamente")
            log_info(f"Transcripción exitosa con Voxtral", func_name)
            return result, "Voxtral (Mistral)"
            
        except Exception as mistral_error:
            # Fallback a OpenAI Whisper
            st.warning(f"⚠️ **Voxtral falló** → Activando **FALLBACK a OpenAI Whisper**")
            ...
            try:
                ...
                st.success(f"✅ **FALLBACK exitoso:** OpenAI Whisper completó la transcripción")
                log_info(f"FALLBACK exitoso: Transcripción completada con OpenAI", func_name)
                return result, "OpenAI Whisper (Fallback desde Voxtral)"
            except Exception as openai_error:
                st.error(f"❌ **Ambas APIs fallaron** - Voxtral: {mistral_error}, OpenAI: {openai_error}")
                ...
                raise Exception(f"Ambas APIs fallaron - Voxtral: {mistral_error}, OpenAI: {openai_error}")
...
def transcribir_archivo_grande_mp3(audio_path, func_name):
    """Convierte archivos grandes a MP3 y los transcribe con Voxtral"""
    try:
        ...
        # Intentar transcripción con Voxtral primero
        st.info(f"🧠 **Intentando transcripción con Voxtral** (archivo MP3 de {tamaño_mp3_mb:.1f}MB)")
        try:
            result = transcribir_audio_mistral(audio_mp3)
            st.success(f"✅ **Voxtral** completó la transcripción del archivo MP3 exitosamente")
            log_info(f"Transcripción exitosa con archivo MP3 usando Voxtral", func_name)
            ...
            return result, f"Voxtral (MP3 comprimido de {tamaño_mp3_mb:.1f}MB, reducción: {reduccion:.1f}%)"
            
        except Exception as mistral_error:
            st.warning(f"⚠️ **Voxtral falló** con archivo MP3 → Activando **FALLBACK a OpenAI**")
            log_info(f"Voxtral falló con archivo MP3 - Intentando OpenAI", func_name)
            ...
            try:
                ...
                raise Exception(f"Error con archivo MP3 - Voxtral: {mistral_error}, OpenAI: {openai_error}")
...
# ============================================================================
# === ALIASES Y BÚSQUEDA FLEXIBLE DE TÉRMINOS (v3: extraído a nivel superior)
# ============================================================================
ALIASES_TERMINOS = {
...
def buscar_termino_flexible(termino, texto):
    ...
    # Buscar variaciones comunes (plurales, conjugaciones)
    variaciones = [
        termino + "s",
        termino + "es",
        termino + "a",
    ...
    for idx_p in range(len(palabras)):
        for j in range(idx_p+1, min(idx_p+6, len(palabras)+1)):
            fragmento_sin_espacios = "".join(palabras[idx_p:j]).lower()
            if termino_sin_espacios == fragmento_sin_espacios:
                termino_encontrado = " ".join(palabras[idx_p:j])
...
def buscar_y_procesar_videos(duracion_clip=60, buffer_anterior=30):
    ...
    if estadisticas['archivos_nuevos'] > 0:
        ...
        if st.session_state.get('loop_continuo', True):
            ...
            for i in range(espera, 0, -1):
                wait_container.info(f"💤 Sin videos nuevos. Re-escaneando en {i}s...")
                time.sleep(1)
            wait_container.empty()
    ...
    # ============================================================================
    # === FASE 1: PRE-FILTRADO RÁPIDO (faster-whisper local, sin costo API) =====
    # ============================================================================
    # Solo extrae audio + transcribe con whisper local + busca términos
    # Los videos SIN coincidencias se registran y saltan (ahorro masivo de tiempo/API)
    
    st.info(f"⚡ **FASE 1: PRE-FILTRADO RÁPIDO** — Escaneando {len(nuevos)} videos con faster-whisper (local, sin costo)")
    
    videos_con_coincidencias = []  # Lista de (archivo_path, rel, audio_path, transcripcion_whisper, segments, terminos_encontrados)
    videos_sin_coincidencias = []  # Lista de (archivo_path, rel) para registrar como procesados
    
    fase1_container = st.container()
    fase1_progress = st.progress(0, text="Iniciando pre-filtrado...")
    
    for i, archivo_path in enumerate(nuevos):
        rel = os.path.relpath(archivo_path, CARPETA_VIDEOS)
        
        try:
            fase1_progress.progress((i + 1) / len(nuevos), text=f"⚡ Pre-filtrado ({i+1}/{len(nuevos)}): {os.path.basename(rel)}")
            
            # --- Paso 1.1: Extraer audio ---
            audio_path = archivo_path.replace(".mp4", ".wav")
            
            if not os.path.exists(archivo_path):
                st.warning(f"⚠️ Archivo no encontrado: {rel}")
                continue
            
            file_size = os.path.getsize(archivo_path)
            if file_size == 0:
                st.warning(f"⚠️ Archivo vacío: {rel}")
                continue
            
            cmd = ["ffmpeg", "-y", "-i", archivo_path, "-ac", "1", "-ar", "16000", "-f", "wav", audio_path]
            result = subprocess.run(cmd, capture_output=True, text=True)
            
            if result.returncode != 0:
                # Reintentar con parámetros alternativos
                cmd_alt = ["ffmpeg", "-y", "-i", archivo_path, "-ac", "1", "-ar", "16000", "-f", "wav", "-avoid_negative_ts", "make_zero", audio_path]
                result_alt = subprocess.run(cmd_alt, capture_output=True, text=True)
                if result_alt.returncode != 0:
                    st.warning(f"⚠️ No se pudo extraer audio: {rel}")
                    continue
            
            # --- Paso 1.2: Transcribir con faster-whisper (LOCAL, gratis, rápido) ---
            try:
                segments_timestamps = obtener_timestamps_whisper(audio_path)
                
                # Reconstruir transcripción completa desde los segmentos de whisper
                transcripcion_whisper = " ".join([seg['text'] for seg in segments_timestamps]).strip()
                
                if not transcripcion_whisper or len(transcripcion_whisper) < 20:
                    st.warning(f"⚠️ Transcripción vacía o muy corta: {rel}")
                    # Registrar como procesado (sin coincidencias posibles)
                    registrar_archivo_procesado(rel, [], "", "Video")
                    videos_sin_coincidencias.append((archivo_path, rel))
                ...
                st.warning(f"⚠️ Error en whisper para {rel}: {str(e)[:80]}")
                continue
            
            # --- Paso 1.3: Buscar términos en la transcripción whisper ---
            text_lower = transcripcion_whisper.lower()
            terminos_encontrados_fase1 = []
            ...
            for termino_item in terminos:
                ...
                # Búsqueda flexible (con aliases y variaciones)
                encontrado, termino_match = buscar_termino_flexible(termino, text_lower)
                if encontrado:
                    terminos_encontrados_fase1.append(termino)
            
            # --- Paso 1.4: Clasificar video ---
            if terminos_encontrados_fase1:
                # HAY COINCIDENCIAS → guardar para Fase 2
                videos_con_coincidencias.append({
                    'archivo_path': archivo_path,
                    'rel': rel,
                    'audio_path': audio_path,
                    'transcripcion_whisper': transcripcion_whisper,
                    'segments_timestamps': segments_timestamps,
                    'terminos_encontrados': terminos_encontrados_fase1,
                    'dur_total': obtener_duracion(archivo_path)
                })
                with fase1_container:
                    st.success(f"🎯 **{rel}** → {len(terminos_encontrados_fase1)} términos: {', '.join(terminos_encontrados_fase1)}")
            else:
                # SIN COINCIDENCIAS → registrar y saltar
                registrar_archivo_procesado(rel, [], "", "Video")
                videos_sin_coincidencias.append((archivo_path, rel))
                # Limpiar audio temporal (no se necesita más)
                if os.path.exists(audio_path):
                    os.remove(audio_path)
                with fase1_container:
                    st.caption(f"⏭️ {rel} — Sin coincidencias, saltado")
        
        except Exception as e:
            st.warning(f"⚠️ Error en pre-filtrado de {rel}: {str(e)[:80]}")
            continue
    
    fase1_progress.progress(1.0, text="✅ Pre-filtrado completado")
    
    # === RESUMEN FASE 1 ===
    st.markdown("---")
    col_f1a, col_f1b, col_f1c = st.columns(3)
    with col_f1a:
        st.metric("📁 Videos escaneados", len(nuevos))
    with col_f1b:
        st.metric("🎯 Con coincidencias", len(videos_con_coincidencias))
    with col_f1c:
        st.metric("⏭️ Sin coincidencias (saltados)", len(videos_sin_coincidencias))
    
    ahorro_pct = (len(videos_sin_coincidencias) / max(1, len(nuevos))) * 100
    if videos_sin_coincidencias:
        st.success(f"💰 **Ahorro Fase 1:** {len(videos_sin_coincidencias)} videos saltados = {ahorro_pct:.0f}% menos llamadas API Mistral")
    
    if not videos_con_coincidencias:
        st.success("✅ **Pre-filtrado completado. No hay coincidencias que procesar.**")
        # Saltar a la sección de loop/fin
        st.session_state.procesamiento_en_curso = False
        ...
        if loop_activo:
            espera = st.session_state.get('intervalo_loop_vacio', 120)
            st.info(f"🔄 **LOOP ACTIVO** — Re-escaneando en {espera}s...")
            wait_container = st.empty()
            for countdown_i in range(espera, 0, -1):
                wait_container.info(f"💤 Sin coincidencias. Re-escaneando en {countdown_i}s...")
                time.sleep(1)
            wait_container.empty()
            st.rerun()
        return
    ...
    # ============================================================================
    # === FASE 2: PROCESAMIENTO COMPLETO (solo videos con coincidencias) =========
    # ============================================================================
    # Ahora SÍ usa Mistral/Voxtral (calidad) + Gemini (análisis) + clips + envíos
    
    st.markdown("---")
    st.info(f"🧠 **FASE 2: PROCESAMIENTO COMPLETO** — {len(videos_con_coincidencias)} videos con coincidencias")
    
    for i, video_info in enumerate(videos_con_coincidencias):
        archivo_path = video_info['archivo_path']
        rel = video_info['rel']
        audio_path = video_info['audio_path']
        transcripcion_whisper = video_info['transcripcion_whisper']
        segments_timestamps = video_info['segments_timestamps']
        dur_total = video_info['dur_total']
        ...
        try:
            ...
            with progress_container:
                st.markdown(f"---\n### {icono} Procesando {tipo_archivo} ({i+1}/{len(videos_con_coincidencias)}): `{rel}`")
                progress_bar = st.progress(0, text="Iniciando procesamiento completo...")
            
            md_path = archivo_path.replace(".mp4", "_streaming.md")

            # --- Paso 2.1: Transcribir con Mistral/Voxtral (alta calidad) ---
            progress_bar.progress(30, text="🧠 Transcribiendo con Mistral/Voxtral (calidad)...")
            start = time.time()
            try:
                transcripcion_mistral, api_usada = transcribir_audio_hibrido(audio_path)
                st.info(f"🎯 Transcripción de calidad completada con {api_usada}")
            except Exception as e:
                st.warning(f"⚠️ Error Mistral, usando transcripción whisper como fallback")
                transcripcion_mistral = transcripcion_whisper
                api_usada = "faster-whisper (fallback)"
            elapsed_mistral = time.time() - start

            # Timestamps ya los tenemos de Fase 1 (no repetir whisper)
            elapsed_whisper = 0

            progress_bar.progress(60, text="🔍 Buscando coincidencias...")
            ...
            # Buscar términos usando función global buscar_termino_flexible (definida a nivel superior en v3)
                ...
                if encontrado:
                    ...
                    try:
                        ...
                        if idea_general_clip and idea_general_clip != _que_dice:
                            resumen_estructurado += f"Relevancia: {idea_general_clip}"
                        elif _relevancia_g:
                            resumen_estructurado += f"Relevancia: {_relevancia_g.capitalize()}"
    ...
    # FASE 3: ENVIAR TODOS LOS ARCHIVOS PROCESADOS AL WEBHOOK (MASIVO)
    if videos_procesados_data:
        st.success(f"✅ FASE 2 COMPLETADA: {len(videos_procesados_data)} archivos procesados exitosamente")
        st.info("🌐 FASE 3: Enviando todos los archivos al webhook con pausas de 60s")
    ...
    if loop_activo:
        st.info(f"🔄 **LOOP ACTIVO** (Ciclo #{st.session_state.loop_ciclo_numero}) - Re-escaneando en {intervalo_loop}s para buscar videos nuevos...")
        log_info(f"Loop activo - Esperando {intervalo_loop}s antes de re-escanear", func_name)
        
        # Liberar flag ANTES de la espera para que el UI sea responsive
        st.session_state.procesamiento_en_curso = False
        
        # Countdown visual
        countdown_container = st.empty()
        for i in range(intervalo_loop, 0, -1):
            countdown_container.warning(f"⏳ Re-escaneando en {i}s... (Ciclo #{st.session_state.loop_ciclo_numero} completado)")
            time.sleep(1)
        countdown_container.empty()
        
        # Re-escanear: cargar procesados actualizados (incluye los que acabamos de procesar)
        st.info("🔍 **RE-ESCANEANDO** - Buscando videos nuevos no procesados...")
        ...
        if nuevos_encontrados:
            st.success(f"🆕 **{len(nuevos_encontrados)} VIDEOS NUEVOS** encontrados - Iniciando nuevo ciclo...")
            log_info(f"Loop: {len(nuevos_encontrados)} videos nuevos encontrados, iniciando nuevo ciclo", func_name)
            time.sleep(2)
            st.rerun()  # Relanzar para procesar los nuevos
        else:
            st.success("✅ **NO HAY VIDEOS NUEVOS** - Todos los videos están procesados")
            log_info("Loop: No hay videos nuevos, entrando en espera", func_name)
            
            # Esperar y volver a intentar
            wait_container = st.empty()
            espera_sin_nuevos = st.session_state.get('intervalo_loop_vacio', 120)  # 2 min si no hay nuevos
            for i in range(espera_sin_nuevos, 0, -1):
                wait_container.info(f"💤 Sin videos nuevos. Re-escaneando en {i}s... (Desactiva el loop desde el sidebar)")
                time.sleep(1)
            wait_container.empty()
        ...
        if st.button("✅ Crear Entidad", type="primary", key="btn_crear_ent"):
            if _new_nombre.strip():
                ...
                if _nem and _nem_correos:
                    ...
                    nc['brevo'] = {'enabled': True, 'api_key': 'YOUR_BREVO_SMTP_KEY', 'smtp_user': '951480002@smtp-brevo.com', 'smtp_server': 'smtp-relay.brevo.com', 'smtp_port': 587, 'sender_email': 'info@fgjmedios.com', 'sender_name': f'FGJ Medios - {_new_nombre.strip()}', 'correos_destinatarios': correos}
                ...
                if _nsb and _nsb_tabla:
                    nc['supabase'] = {'enabled': True, 'url': 'https://sfvbcprhfmwglqpyyfxz.supabase.co', 'anon_key': 'YOUR_JWT_TOKEN', 'tabla_nombre': _nsb_tabla.strip()}
                if _ncl:
                    nc['cloudinary'] = {'enabled': True, 'cloud_name': 'dhzxzbkmc', 'api_key': '149663287387673', 'api_secret': 'YOUR_CLOUDINARY_API_SECRET', 'folder': 'video_analyzer_clips'}
                ...
                if _br_on:
                    ...
                    if st.button("➕", key=f"br_btn_{_eid}") and _new_c.strip() and '@' in _new_c:
                        ...
                        if not _ent['brevo'].get('api_key'):
                            _ent['brevo'].update({'api_key': 'YOUR_BREVO_SMTP_KEY', 'smtp_user': '951480002@smtp-brevo.com', 'smtp_server': 'smtp-relay.brevo.com', 'smtp_port': 587, 'sender_email': 'info@fgjmedios.com', 'sender_name': f'FGJ Medios - {_enombre}'})
            ...
            with _d5:
                ...
                if _cl_on and not _cl.get('cloud_name'):
                    if st.button("Activar", key=f"sv_cl_{_eid}"):
                        _ent['cloudinary'] = {'enabled': True, 'cloud_name': 'dhzxzbkmc', 'api_key': '149663287387673', 'api_secret': 'YOUR_CLOUDINARY_API_SECRET', 'folder': 'video_analyzer_clips'}
                        actualizar_cliente(_eid, _ent); st.success("OK")
                elif _cl_on:
